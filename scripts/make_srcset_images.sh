...
if [[ "$found" -eq 1 ]]; then
  ...
  if [[ "$JOBS" -le 1 ]]; then
    ...
    export -f process_one make_thumb make_primary should_process_work_id record_success_id record_processed_source
    export OUTPUT_DIR WEBP_PRESET PRIMARY_Q THUMB_Q COMPRESSION_LEVEL HAS_SIPS HAS_HEIF_CONVERT WORK_IDS_FILE SUCCESS_IDS_FILE DRY_RUN REPORT_FILE PROCESSED_SOURCES_FILE
...
echo "  written total: $written_total (thumb=$written_thumbs, p800=$written_p800, p1200=$written_p1200, p1600=$written_p1600)"
echo "  dry-run total: $dry_total (thumb=$dry_thumbs, p800=$dry_p800, p1200=$dry_p1200, p1600=$dry_p1600)"
