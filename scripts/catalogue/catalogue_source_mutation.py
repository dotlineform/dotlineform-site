...
@dataclass(frozen=True)
class WorkDetailPlan(SourceRecordPlan):
    work_id: str
...
def validate_detail_records(
    source_records: CatalogueSourceRecords,
    detail_uid: str,
    detail_record: Dict[str, Any],
) -> list[str]:
    errors = validate_work_detail_media_section_record(detail_uid, detail_record)
    work_details = dict(source_records.work_details)
    work_details[detail_uid] = detail_record
    errors.extend(validate_work_detail_section_metadata_consistency(work_details))
    normalized_records = CatalogueSourceRecords(
...
def plan_work_detail_save(
    source_records: CatalogueSourceRecords,
    work_details: Mapping[str, Dict[str, Any]],
    detail_uid: str,
    current_record: Mapping[str, Any],
    update: Mapping[str, Any],
) -> WorkDetailPlan:
    current_section_id = normalize_text(current_record.get("section_id"))
    requested_section_id = normalize_text(update.get("section_id"))
    ...
    updated_record = normalize_work_detail_update(detail_uid, current_record, update)
    ...
    return WorkDetailPlan(
        baseline_record=dict(current_record),
        updated_record=updated_record,
        changed_fields=changed_fields(current_record, updated_record),
        validation_errors=validate_detail_records(source_records, detail_uid, updated_record),
        payload=payload_for_map("work_details", updated_details),
        work_id=work_id,
    )


def plan_work_detail_create(
    source_records: CatalogueSourceRecords,
    work_details: Mapping[str, Dict[str, Any]],
    detail_uid: str,
    work_id: str,
    ...
    blank_record = {field: None for field in DETAIL_FIELDS}
    blank_record["detail_uid"] = detail_uid
    blank_record["work_id"] = work_id
    blank_record["detail_id"] = detail_id
    normalized_update = dict(update)
    normalized_update["detail_uid"] = detail_uid
    normalized_update["work_id"] = work_id
    normalized_update["detail_id"] = detail_id
    if normalize_text(normalized_update.get("section_id")):
    ...
    return WorkDetailPlan(
        ...
        updated_record=created_record,
        changed_fields=changed_fields(blank_record, created_record),
        validation_errors=validate_detail_records(source_records, detail_uid, created_record),
        payload=payload_for_map("work_details", updated_details),
        work_id=work_id,
    )
