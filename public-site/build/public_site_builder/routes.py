...
from .render import docs_viewer_shell, render_page, script_tag, stylesheet_tag
...
def render_about(config: PublicSiteConfig) -> str:
    hero_base = _join_url(config.home_media["base"], config.home_media["prefix"])
    body = f"""
...
<div
  ...
  data-thumb-works-base="{escape(_join_url(config.thumbs['base'], config.thumbs['works']), quote=True)}"
  data-thumb-moments-base="{escape(_join_url(config.thumbs['base'], config.thumbs['moments']), quote=True)}"
...
<article
  ...
  id="selectedWorkRoot"
  data-baseurl="{_baseurl(config)}"
  data-works-img-base="{escape(_join_url(config.media['base'], config.media['image_works']), quote=True)}"
  data-works-files-base="{escape(_join_url(config.media['base'], config.media['files_works']), quote=True)}"
  data-details-thumb-base="{escape(_join_url(config.thumbs['base'], config.thumbs['work_details']), quote=True)}"
...
<article
  ...
  id="detailPageRoot"
  data-baseurl="{_baseurl(config)}"
  data-img-base="{escape(_join_url(config.media['base'], config.media['image_work_details']), quote=True)}"
  data-primary-render-widths="{_json_attr(primary_render_widths)}"
  data-primary-display-width="{primary_display_width}"
  data-primary-full-width="{primary_variant.get('preferred_width', primary_display_width)}"
  data-primary-suffix="{escape(str(primary_variant.get('suffix', 'primary')), quote=True)}"
...
<article
  ...
  id="momentPageRoot"
  data-baseurl="{_baseurl(config)}"
  data-img-base="{escape(_join_url(config.media['base'], config.media['image_moments']), quote=True)}"
  data-primary-render-widths="{_json_attr(primary_render_widths)}"
  data-primary-display-width="{primary_display_width}"
  data-primary-suffix="{escape(str(primary_variant.get('suffix', 'primary')), quote=True)}"
  data-asset-format="{escape(_asset_format(pipeline), quote=True)}"
