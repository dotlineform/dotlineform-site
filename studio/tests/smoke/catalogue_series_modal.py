#!/usr/bin/env python3
"""Smoke-check the catalogue series confirmation modals."""
...
ROOT_SELECTOR = "#catalogueSeriesRoot"
SERIES_ID = "009"
WORK_ID = "00001"
...
def main() -> int:
    ...
    def handle_catalogue(route: Route) -> None:
        ...
        if request.method == "GET" and path == "/catalogue/read":
            ...
            if key == "catalogue_lookup_series_base":
                fulfil_json(route, {
                    ...
                    "record_hash": "hash-series-current",
                    "member_works": [{"work_id": WORK_ID, "series_ids": [SERIES_ID]}],
                })
                return
        ...
        if request.method == "POST" and path == "/catalogue/publication-preview":
            fulfil_json(route, {
                ...
                "preview": {
                    "blocked": False,
                    "blockers": [],
                    "summary": f"Unpublish series {SERIES_ID} and clean public output.",
                },
            })
            return
        if request.method == "POST" and path == "/catalogue/publication-apply":
            ...
            series_record = {**series_record, "status": "draft"}
            fulfil_json(route, {
                "ok": True,
                "status": "ok",
                "record": series_record,
                "record_hash": "hash-series-draft",
            })
            return
        if request.method == "POST" and path == "/catalogue/delete-preview":
            fulfil_json(route, {
                ...
                "preview": {
                    ...
                    "blockers": [],
                    "validation_errors": [],
                    "summary": f"Delete source record {SERIES_ID} and remove generated series output.",
                },
            })
            return
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.goto(route_url(base_url, f"/studio/catalogue-series/?series={SERIES_ID}"), wait_until="domcontentloaded")
            ...
            if f"Unpublish series {SERIES_ID}" not in unpublish_modal["bodyText"]:
            ...
            if publication_request.get("kind") != "series" or publication_request.get("action") != "unpublish":
                raise AssertionError(f"unexpected publication apply request: {publication_request!r}")
            if publication_request.get("series_id") != SERIES_ID:
            ...
            if delete_request.get("kind") != "series" or delete_request.get("series_id") != SERIES_ID:
