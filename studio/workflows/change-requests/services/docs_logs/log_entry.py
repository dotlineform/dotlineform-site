...
def maybe_rebuild_generated(root: Path, dry_run: bool, skip: bool) -> None:
    if skip:
        return
    script = root / "studio" / "workflows" / "change-requests" / "services" / "docs_logs" / "build_indexes.py"
    if not script.exists():
        print(
            "generated log rebuild skipped: studio/workflows/change-requests/services/docs_logs/build_indexes.py is not implemented yet",
            file=sys.stderr,
        )
        return
