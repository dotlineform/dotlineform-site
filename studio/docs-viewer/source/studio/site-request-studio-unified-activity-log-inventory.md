...
| Validate activity contract registry | `done` | Implemented in `studio/checks/verify_activity_contract.py` and `studio/tests/python/test_activity_contract.py`; wired into the quick check profile. |
...
| Series tags | `/studio/analytics/series-tags/` | import modal `apply-import` | `import series tag assignments` | tag assignment import apply; tag assignment writes; backup; conflict resolution summary | `done` | Batch C coverage now runs through the local Studio analytics API. |
...
| Tag aliases | `/studio/analytics/tag-aliases/` | `data-role="open-import-modal"` then import action | `import tag aliases` | alias import preview/apply; alias writes; backups; validation results | `done` | Batch C coverage now runs through the local Studio analytics API. |
