...
from studio_data_sharing_api import data_sharing_get_payload, data_sharing_post_response  # noqa: E402
...
class StudioAppRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith("/studio/api/catalogue/"):
            ...
            return
        if path.startswith("/studio/api/data-sharing/"):
            self.send_data_sharing_api_json(path.removeprefix("/studio/api/data-sharing"), query)
            return
    ...
    def do_POST(self) -> None:
        ...
        if path.startswith("/studio/api/data-sharing/"):
            if not self.origin_allowed_for_local_api():
                self.send_json({"ok": False, "error": "Origin not allowed"}, HTTPStatus.FORBIDDEN)
                return
            self.send_data_sharing_api_post_json(path.removeprefix("/studio/api/data-sharing"))
            return
    ...
    def do_OPTIONS(self) -> None:
        request = urlsplit(self.path)
        path = unquote(request.path)
        if not path.startswith(("/studio/api/analytics/", "/studio/api/audits/", "/studio/api/catalogue/", "/studio/api/data-sharing/")):
            self.send_error(HTTPStatus.NOT_FOUND, "Not found")
            return
        if not self.origin_allowed_for_local_api():
