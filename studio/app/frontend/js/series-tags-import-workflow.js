...
export async function previewSeriesTagsImport(options = {}) {
  ...
  try {
    const importPreview = await postJson(getStudioTagWriteEndpoint("importTagAssignmentsPreview", config), {
...
export async function applySeriesTagsImport(options = {}) {
  ...
  try {
    ...
    const response = await postJson(getStudioTagWriteEndpoint("importTagAssignments", config), {
      import_assignments: importPayload,
