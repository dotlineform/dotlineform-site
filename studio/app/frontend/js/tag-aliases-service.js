...
export async function submitAliasEdit(options) {
  ...
  if (saveMode === "post") {
    try {
      const response = await postJson(getStudioTagWriteEndpoint("mutateTagAlias", config), {
        alias: originalAlias,
...
export async function previewAliasPromote(options) {
  ...
  try {
    const response = await postJson(getStudioTagWriteEndpoint("promoteTagAliasPreview", config), {
      alias: aliasKey,
      group,
      client_time_utc: utcTimestamp()
    });
