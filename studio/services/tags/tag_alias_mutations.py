...
def create_alias(
    ...
    *,
    ...
    tag_source.enforce_alias_group_constraints(normalized_tags, registry_payload, "tags")
    ...
    updated_payload.setdefault("tag_aliases_version", tag_source.TAG_ALIASES_VERSION)
...
def rewrite_aliases_for_targets(
    ...
    for idx, (raw_key, raw_value) in enumerate(existing_aliases.items()):
        ...
        updated_value, changed, replaced = replace_alias_entry_refs(
            alias_value,
            old_tag_id,
            replacement_tag_ids,
            registry_payload,
...
def rewrite_aliases_for_tag(
    ...
    for idx, (raw_key, raw_value) in enumerate(existing_aliases.items()):
        alias_key = tag_source.sanitize_alias_key(raw_key, idx)
        alias_value = tag_source.sanitize_alias_entry(raw_value, alias_key, "tag_aliases.aliases")
        updated_value, changed = update_alias_entry_for_tag(
            alias_value,
            old_tag_id,
            new_tag_id,
            registry_payload,
...
def extract_registry_tag_ids(registry_payload: Dict[str, Any]) -> set[str]:
    return set(tag_source.extract_registry_tag_groups(registry_payload))
...
def mutate_alias_entry(
    ...
    tag_source.enforce_alias_group_constraints(tags, registry_payload, "tags")
