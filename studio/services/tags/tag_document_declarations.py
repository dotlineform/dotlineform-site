...
CONCEPTS_SCOPE = "analysis"
CONCEPTS_SUB_SCOPE = "concepts"
CONCEPTS_STAGE = "working"


def _concepts_config(repo_root: Path) -> tuple[Any, Any]:
    parent_config = load_docs_scope_stage(repo_root, CONCEPTS_SCOPE, CONCEPTS_STAGE)
    matching = [
        candidate
        for candidate in parent_config.sub_scopes
        if candidate.sub_scope == CONCEPTS_SUB_SCOPE
    ]
    if len(matching) != 1:
        raise ValueError("Analysis Working Concepts document collection is not configured")
...
def load_tag_document_association_payload(repo_root: Path) -> dict[str, Any]:
    ...
    _parent_config, concepts_config = _concepts_config(repo_root)
    path = (
        resolve_scope_path(repo_root, generated_documents_path(concepts_config))
    ...
    if (
        ...
        or payload.get("scope") != CONCEPTS_SCOPE
        or payload.get("stage") != CONCEPTS_STAGE
        or payload.get("sub_scope") != CONCEPTS_SUB_SCOPE
    ...
    for association in payload["associations"]:
        ...
        for document in documents:
            ...
            if (
                set(target) != {"scope", "stage", "sub_scope", "doc_id"}
                or target.get("stage") != CONCEPTS_STAGE
                or target_key[0] != CONCEPTS_SCOPE
                or target_key[1] != CONCEPTS_SUB_SCOPE
...
def current_tag_document_associations(
    ...
    parent_config, concepts_config = _concepts_config(repo_root)
    ...
    for document in documents:
        ...
        associations.append(
            {
                "target": {
                    "scope": CONCEPTS_SCOPE,
                    "stage": CONCEPTS_STAGE,
                    "sub_scope": CONCEPTS_SUB_SCOPE,
