...
| `works/index/<work_id>.json` | Work metadata, required `series_id`, links, downloads and image facts |
...
Every Work belongs to one Series; empty Series remain valid. There is no Catalogue publication filter, primary-Work requirement, Recent projection or Catalogue Search output in this producer. The canonical Gallery registry and membership map are established separately; generated Gallery records and `work.galleries` are the next delivery and are not emitted yet.
