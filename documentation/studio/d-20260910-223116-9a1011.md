...
last_updated: "2026-09-14 20:24:08"
...
Lucide is the preferred family for general interface and document-type icons. Reuse the existing Catalogue Series, Work and Detail designs as standalone SVG assets. Lucide is expected to cover most remaining meanings; confirm coverage when choosing the individual icons. Document-type classification, final storage and Markdown syntax remain open for review. No migration, asset extraction, token implementation or App integration has started.
...
The existing `docs-viewer/runtime/js/management/icons/` contains the VS Code asset and its attribution. A shared location such as `docs-viewer/static/icons/` is proposed for broader use. Public resources would have an explicit tracked site projection; App resources would be packaged from the same canonical artwork. Keep those outputs replaceable and avoid independently maintained drawings in each consumer. The exact directory and small mapping format can be settled in a bounded delivery; this feature does not require a general asset framework.
...
| Display sizes | Review at 16, 20 and 24 CSS pixels or native points. Start with 20 for compact controls and 1em beside document text; the enclosing button owns its hit area. |
...
The 24-unit canvas and 2-unit rounded strokes follow [Lucide's design specification](https://lucide.dev/contribute/icons/specification). The proposed display sizes remain open for review. The existing Catalogue icons already use the same 24-unit canvas, with a 1.75-unit stroke; review their visual weight alongside Lucide before deciding whether to adjust it. Do not redraw an adopted icon merely to change its native viewBox; preserve its aspect ratio and inspect its optical fit.
