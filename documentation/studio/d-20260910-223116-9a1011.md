...
Lucide is the preferred family for general interface and document-type icons. Reuse the existing Catalogue Series, Work and Detail designs as standalone SVG assets. Lucide is expected to cover most remaining meanings; confirm coverage when choosing the individual icons. The [Toolbar Mapping](d-20260919-100234-038395.md) records the implemented toolbar replacements, state variants and remaining visual review. The viewer and report toolbar migrations use the current 20px artwork trial inside 32px buttons, with smaller search-clear controls. Index-row and report-list migration, remaining Catalogue asset extraction, authored icon-token implementation and App integration remain open.
