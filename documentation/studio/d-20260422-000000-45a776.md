...
last_updated: "2026-09-23 19:40:43"
...
The `>` control in the member list's top-right corner expands the list, moves the editor to the right and hides the preview. It becomes `<` to restore the normal layout. At full desktop width the editor keeps its original width and the list takes the released preview space. Expansion remains selected while opening and saving Works, and resets when the page reloads. Toggling changes presentation only: the mounted list, scroll position, selection and unsaved draft remain intact. New and empty editor states continue to omit the preview when the list is restored.
