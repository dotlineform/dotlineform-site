...
Open either authoring modal, select Use document subject, retain or edit the label/image presentation, and insert. Media links capture the exact selected Work, Detail or Series. Images may include an explicit `detail_id`. Save/Build uses the identity already present in Markdown. Subsequent subject changes do not retarget the token.
