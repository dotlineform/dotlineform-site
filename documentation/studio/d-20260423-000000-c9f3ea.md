...
Search is a stage-independent `docs_viewer_search_index_v4` artifact built only in Working. Its header omits stage, its document rows omit result URLs, and its content version excludes stage. Readers use exact document/collection/report-host identity with their configured routes. Prepare Preview captures the existing index, binds its bytes into the plan revision and copies it before recording build completion. Snapshot and repository projection preserve those exact bytes without JSON reserialization, label rewriting or rehashing. Missing/invalid Search fails without a rebuild fallback; a changed index invalidates preparation. Ordinary source freshness and Search membership are separate from prepared document completeness.
...
The builder continues to derive local and public `recent.json` projections independently. [Copy Working Search Into Preview](d-20260924-210855-8b419e.md) implements build/copy ownership first, by explicit user decision. The broader saved Working corpus remains unchanged; Search coverage and inherited draft/unpublishable exclusions are the next separate work under [Collection Search And Site Search](d-20260924-135214-baa9ec.md).
