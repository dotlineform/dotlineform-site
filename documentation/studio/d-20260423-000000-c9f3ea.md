...
Deploy Repo consumes exactly one reviewed Preview revision. It does not read source/generated output, prepare content or apply another document filter. It reconciles configured repository/R2, document-location, Catalogue and publication-lineage projections. Public reader payloads retain the separate `published` identity. The public document, Search and media destinations remain unchanged. Git commit/push and the manually triggered GitHub Pages workflow remain separate gates.
