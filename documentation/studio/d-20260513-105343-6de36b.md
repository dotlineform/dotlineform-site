...
last_updated: "2026-09-13 21:29:09"
...
`docs_media` is the local, read-only media-to-document report hosted by [Docs Media](/docs/?scope=analysis&stage=working&doc=d-20260812-212735-6d9cf3). It loads one selected configured scope through `POST /docs/media-report` and renders sortable Type, File name, and Documents columns. The toolbar owns scope selection, refresh, and immediate file-name or document-title search; File name uses the existing confined local-target action, while Documents preserves exact parent and sub-scope links. The report and its data route remain absent from the public report registry and executable graph. [Media And Asset Handling](/docs/?scope=studio&doc=d-20260514-184303-7914e2) owns the live-row and storage boundary.
