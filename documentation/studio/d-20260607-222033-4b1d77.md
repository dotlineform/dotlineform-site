...
last_updated: "2026-09-10 15:51:19"
...
**Insert doc link** requests the exact mounted Source scope and, for staged authoring, an explicit `stage=working`. The read-only service enumerates the scope-level source collection and every configured sub-scope through their configured source owners. It excludes explicit `publishable: false` targets while retaining drafts and documents without publication metadata. Subject metadata does not filter selection.

The `docs_document_link_targets_v1` response contains `scope`, `stage`, raw `sub_scopes` and `documents`. Each document supplies its title, explicit `{scope, sub_scope, doc_id}` target and ordinary `href`. Child locations use the exact stage's host and child identity. Authored hrefs omit workflow-stage parameters. Unavailable collections and ambiguous child hosts fail visibly without fallback.

The modal filters All documents, Scope-level documents or one exact sub-scope and searches by title or immutable ID. Confirmation inserts the selected title and href through captured, revision-guarded buffer replacement. It does not save, rebuild, change front matter or generate relationships. Public providers expose no local target-lookup capability.
