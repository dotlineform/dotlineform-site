...
last_updated: "2026-09-23 18:28:08"
...
Deleting a Series with assigned Works is blocked by the required-Series rule; reassign those Works first. Deleting a Work removes its entry from `galleries-by-work.json` in the same canonical transaction, then reconciles its generated output and exact obsolete media. The Studio Detail browser, section service, Detail read target and Detail mutation paths are retired. Studio's write context allows only Works, Series and Work Gallery memberships. Gallery creation/title editing and bulk creation of ordinary Works belong to a separate Works/Series editor delivery.
...
Ordinary Studio Catalogue production is available. Save, import and delete maintain Work/Series output and affected Gallery member rows under `$DOTLINEFORM_PROJECTS_BASE_DIR/catalogue/generated/`; private `studio/data/generated/catalogue-lookup/` remains editor-owned. The same output generator reads both canonical Gallery sources, writes individual `galleries/index/<gallery_id>.json` records and embeds Gallery IDs/titles in `work.galleries`. Gallery changes refresh current and former member Works. Staging resolves through `catalogue/media-staging/`, while generated thumbnails use `works/thumbs/`. Docs Viewer reads the generated Gallery lookup and individual records for exact Gallery tokens, search and Media View; Studio Gallery creation/title editing remains a separate later delivery.
