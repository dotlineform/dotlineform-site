...
last_updated: "2026-09-16 18:35:57"
...
New collection validates both stage registrations before writing and commits them together in the workspace config. Working receives the collection ID, title and creation receipt; Pre-publish receives the matching ID and title. The action creates the report host, source/generated roots and payloads in Working only. The ordinary Pre-publish operation owns preparing the collection's eligible contents and generated output. Existing stage-specific customisations and capabilities remain separate.

The New document action for the exact Working `catalogue` collection requires a five-digit `work_id` and a non-blank title. The modal sends both fields in one create request; the service validates them before writing and saves the Work ID as a quoted front-matter string, preserving leading zeros. The Work ID is an authored document association, validated by format. The existing collection build runs once after that source write and creates the document's Links record with the Work ID in `self.subject.key`, `kind: work` and `fields: [work_id]`. Watcher suppression prevents a duplicate rebuild, and the subsequent report refresh and Source opening only read the saved results.
