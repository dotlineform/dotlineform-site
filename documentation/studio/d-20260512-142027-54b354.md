...
`docs_publication_payloads.py` projects Preview document identities and URLs into the configured public section for downstream use. Search v4 bypasses snapshot/repository JSON projection and is copied byte-for-byte, without collection-label rewriting or rehashing. Subject generation remains the original prepared-input receipt. Report-host and child-document IDs remain exact.
