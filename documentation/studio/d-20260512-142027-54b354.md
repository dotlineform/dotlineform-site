...
Each Working and Pre-publish collection requires `report_host_doc_id` beside its collection ID and title. This setting is the authority for collection-host lookup; the lifecycle receipt remains creation provenance. Builders, Links and document-location services use the configured ID directly, without scanning ordinary documents, reading the host document to validate it, or falling back to a discovered host. Configuration loading validates the ID format only. Recent, Search and preparation use the same mapping; Search retains its selected-corpus restriction, and preparation retains its host Draft/Ignore eligibility rule. Collection hosting does not add `parent_id` to member documents.

New collection validates both stage registrations before writing and commits them together in the workspace config. Working receives the collection ID, title, report host ID and creation receipt; Pre-publish receives the matching ID, title and report host ID. The action creates the report host, source/generated roots and payloads in Working only. The ordinary Pre-publish operation owns preparing the collection's eligible contents and generated output. Existing stage-specific customisations and capabilities remain separate.

The New document action for the exact Working `catalogue` collection requires a five-digit `work_id` and a non-blank title. The modal sends both fields in one create request; the service validates them before writing and saves the Work ID as a quoted front-matter string, preserving leading zeros. The Work ID is an authored document association, validated by format. One targeted collection build renders the new document, refreshes collection metadata and creates the document's Links record with the Work ID in `self.subject.key`, `kind: work` and `fields: [work_id]`. Watcher suppression prevents a duplicate rebuild, and the subsequent report refresh and Source opening only read the saved results.
...
Document Build requires `--stage working` or `--stage pre-publish`. `--collection` selects one configured child; `--only-doc-ids` selects document rendering within either the ordinary or named collection. Named-collection authoring writes and watcher changes read and render selected sources, merging their metadata into saved manifests and subject indexes; see [Builder](d-20260423-000000-c9f3ea.md) for preserved outputs and prerequisites. `rebuild_stage_outputs` and the source-write wrappers receive the explicit stage and optional child, with no scope argument. Ordinary saves and watcher changes leave Search explicit. Complete Build still awaits ordinary documents, configured children, requested Search, aggregate Links and prepared Mermaid before writing `docs_build_manifest_v2` through `docs_build_manifest.py`.
...
Draft/Ready buttons submit the intended readiness shown by the control, using a fresh source revision. The source-only save response updates the ordinary sidebar or the mounted collection report's row and cached detail immediately. A collection detail read retains that report readiness while the watcher catches up, and returning to the list uses the updated row. The watcher remains the sole build owner for a Draft change; the UI update performs no fetch, document reload or additional build.
...
Startup validates the existing Docs root without creating replacement storage. The watcher watches Working ordinary and child sources, identifies suppression owners by stage/child, and stops before further writes if a changed workspace config cannot be loaded. After its initial source snapshot, named-collection changes and suppressed writes refresh only changed snapshot entries. It does not start watching Pre-publish authoring sources.
...
`docs_publication_payloads.py` projects Preview document identities and URLs into the configured public section for downstream use. Search v4 passes through stage projection unchanged; only a configured collection-display change recomputes its content version, without regenerating postings. Subject generation remains the original prepared-input receipt. Report-host and child-document IDs remain exact.

Deploy Repo preview/apply require `stage: preview`, use `docs_deploy_repo_preview_v3`, and are bound to both the reviewed Preview revision and destination plan. Working owns Prepare Preview; Preview owns Deploy Repo. Workspace-derived Preview collection destinations supply projection policy, never source/generated content. Deployment preserves `public-reports.json` and uses public-media reconciliation v2 with unchanged R2 locations. Public document-location JSON and legacy Catalogue document-URL refresh are retired; the response has no Catalogue refresh or paused outcome. Retired scope-route creation and source-Delete public-cleanup modules remain removed; Working Delete does not prune the Preview or public snapshot.

The legacy Catalogue document-URL join and site writer are retired. Current Catalogue consumer-file deployment has its separate pending owner in [Catalogue Deployment](d-20260909-205938-3569d6.md). Project State v4 consumes only Working Works, with subject-association v2 and exact stage/collection targets. Media-source evidence v2 likewise requires exact stage/child ownership and permits writes only in Working.
...
Search uses stage-independent `docs_viewer_search_index_v4`, with no header stage or result URLs; the content version excludes stage. Readers resolve exact document/collection/report-host identity through their active route. Working builds its complete index, and Prepare Preview currently builds another from captured eligible input in a temporary workspace. The agreed narrower corpus and Working-only build/copy ownership remain separate work. Tokenization, ranking, fields and explicit rebuild policy remain unchanged. `build_search.py` requires `--stage working|preview`; Preview building requires explicit isolated storage, and no scope or Catalogue selector remains.

The `docs_document_locations_v2` index, public provider/picker and builder are retired. Deploy Repo derives counts and any configured publication-lineage URLs directly from accepted document identities and exact configured report hosts. Navigation and source link insertion retain their own URL helpers. Current Catalogue file deployment remains an independent pending projection into `site/assets/data/catalogue/`, without the retired legacy schema writer or document-URL refresh.
