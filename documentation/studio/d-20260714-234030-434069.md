...
last_updated: "2026-09-23 10:56:00"
...
Media View text links accept an exact five-digit Work ID, an optional canonical Detail suffix belonging to that Work, an exact three-digit Series ID or an exact Gallery ID. Gallery IDs use three digits or at least four digits without a leading zero; they are independent of Series IDs and permit no Detail suffix. The serializer escapes backslash, pipe and closing bracket in the required single-line label; the parser decodes only those escapes. Work image identity also uses five digits. A positive Work Detail ID is normalized to at least three digits; parsed source must already use the canonical form. Canonical Details have been converted to ordinary Works; disposable old Detail references remain unchanged and resolve as unavailable against the current generated data.

Image fields use deterministic percent encoding and canonical field order. Alt text is required. Captioned images also store placement (`full`, `left` or `right`) and boolean `fill_width`; summary is optional. A Detail belongs to its exact Work. Image tokens support Work primaries and Details only. Series-image tokens and the old Series slug policy are removed; Series and Gallery grids open through text links, with no inline group image.

Tokens always include their Work, Series or Gallery identity, including tokens inserted through Use document subject. Gallery selection is independent of document Subjects. Detail media links carry both the owning Work ID and Detail suffix. Labels, alt text, captions and summaries are literal text; `title` and `metadata` are not data-binding commands. The experimental omitted-identity forms are retired without compatibility aliases.
...
Media authoring and Info use generated Catalogue Work/Series/Gallery data through the configured media provider. The shared Catalogue media modal offers Work primary/Detail selection for images and adds Series/Gallery selection for text links. Gallery search reads Studio's generated `galleries/galleries_index.json` through `/docs/catalogue-media-targets`; Gallery resolution reads the selected `galleries/index/<gallery_id>.json` through `/docs/catalogue-gallery`. Neither needs the private legacy target lookup or a Gallery document. Current target validation precedes guarded insertion or Info updates. Info preserves a Detail link's exact owning Work and Detail suffix when its label changes. These paths do not read the usage index.
...
Work, Detail, Series and Gallery text links and Work/Detail images are rendered as HTML markers carrying exact Catalogue identity and authored presentation. A Gallery marker uses `data-docs-media-kind="catalogue-gallery"` and its exact Gallery ID. Build records these occurrences without requiring the private lookup or resolving a media URL. Their usage-row `href` is empty because the media provider resolves the presentation at runtime.
...
Each workspace stage owns one `generated/documents/semantic-tokens/index.json` covering ordinary documents and all configured collections. Rows retain exact `source_stage`, `source_collection` and `source_doc_id`, with an empty collection for ordinary documents. The dataset retains its stage. Raw tokens, source ranges, family/type/identity, occurrence titles and destinations remain intact; repeated occurrences stay separate. `detail_id` explicitly records the selected Detail for both images and links, alongside its owning Work in `target_id`; it is empty for Work primaries, Series and Galleries. The report appends the Detail suffix to the displayed identity.
