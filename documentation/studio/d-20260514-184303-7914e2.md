...
last_updated: "2026-09-09 14:46:28"
...
The local [Docs Media](/docs/?scope=analysis&stage=working&doc=d-20260812-212735-6d9cf3) report joins the selected scope's live provider-neutral ready-media and registered build-source listing to exact parent and sub-scope document references. One row represents one present `{scope, media_type, identity}`. An unreferenced present file remains visible with a blank Documents cell; a referenced identity absent from the live listing produces no row and remains broken-link territory. Build sources do not inherit references from same-basename outputs.

The server supplies the exact media type, root-relative identity, confined role-specific Projects-relative local target, and ordered document targets. The browser does not reconstruct identity or filesystem location. The report is read-only and local: it adds no media registry, source-evidence join, mutation, public projection, missing-reference row, or second storage path. [Reports](/docs/?scope=studio&doc=d-20260513-105343-6de36b) owns its registration and presentation behavior.
