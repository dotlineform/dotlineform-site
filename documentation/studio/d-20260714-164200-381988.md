...
last_updated: "2026-08-31 23:14:43"
parent_id: d-20260720-101826-994dd0
---
...
This Studio-owned document covers repository hosting, local environment, lifecycle, and integration. The [Processing documentation scope](/docs/?scope=processing&doc=d-20260714-092047-06a5db) remains the home for conceptual and technical documentation about the projects themselves.
...
- Follow the project-owned **Javadoc And Function Reference** guidance below, and select Processing-aware Java checks only when the Processing lifecycle defines them.
...
Processing work media and print masters belong beneath the configured external `$DOTLINEFORM_PROJECTS_BASE_DIR/processing/<project-id>/` root. Print masters are important artwork, so their storage and backup policy remains explicit even though they are not committed to Git.
...
A Processing application launched through the GUI cannot be assumed to inherit repository `.env.local` or `DOTLINEFORM_PROJECTS_BASE_DIR`. Each runnable sketch that writes work media must therefore use an explicit portable project-local configuration, validate it before output, report failure visibly, and never fall back to the sketch or repository.

InkEngine is the first configured project. Its tracked `processing/projects/InkEngine/output-config.example.json` has exactly `projects_base_dir` and `project_id`; the example is copied to ignored `processing/projects/InkEngine/data/output-config.local.json`. `projects_base_dir` names the existing absolute readable and writable external base, while `project_id` is the exact visible relative segment `ink-engine`.

InkEngine derives only `<projects_base_dir>/processing/ink-engine`. It may create the missing `processing/` and exact project directory beneath the validated base. Blank, relative, absolute, hidden, traversing, nested, unknown, symlinked, escaping, unavailable, or unwritable project targets fail without a fallback.

The sketch loads this configuration during `setup()`. `F8` deliberately renders a plain evidence frame containing black `䷑` (U+4DD1, WORK ON THE DECAYED), confirms a supporting installed font through `PFont.list()`, loads it with `createFont()`, and calls the existing `exportDrawing()` function. Output filenames include a UTC timestamp and collision token beneath external `processing/ink-engine`. The preset `save` control remains separate and continues to update tracked `data/sliderSettings.json`.

The focused `OutputWorkspaceTest` exercises the standard-Java path helper without introducing a general Processing lifecycle or browser-style interaction harness. Full-sketch acceptance still requires a Processing compile plus one deliberate GUI export and inspection of the resulting glyph frame.

The historical `processing/previews/ink-engine/` repository copies were retired after the user confirmed that the wanted files had been copied into external `processing/ink-engine`. The retirement did not inventory, hash, byte-compare, back up or guarantee recovery of those external files. Selected work media now remains in the external project workspace and reaches public presentation through the existing Catalogue workflow rather than a repository preview copy.
...
2. Small, stable files intended as downloadable companions to a Docs Viewer
   document belong in `docs-viewer/scopes/processing/published/media/files/`.
3. Large inputs, repeated renders, print masters, and bulk project collections belong under `$DOTLINEFORM_PROJECTS_BASE_DIR/processing/<project-id>/`; selected public work media uses the existing Catalogue workflow rather than a repository preview copy.

Standalone images and downloadable files are now published from the active
source editor without creating wrapper documents. This is still not a
general-purpose bulk asset ingest workflow. Existing ZIP, spreadsheet, and CSV
attachments stay in place until their owning documents and reproducibility value
are reviewed.

The I Ching project is the first pressure test: its roughly 150 MB of files and
images should not be published file-by-file merely to make them visible in Docs
Viewer. Before moving it, decide:
...
- which curated files should become scope-owned Docs Viewer media and which
  remain Processing project or archival material
- whether large curated media needs a configured R2 destination without making
  the provider part of ordinary authoring.
...
The Processing Docs Viewer scope should contain hand-written project maps, preset/export instructions, and architectural explanations. Javadoc-generated reference pages can be considered later, after the source comments are useful and the mix of `.pde` and `.java` files is understood.
...
- whether large related projects such as I Ching need their own Docs Viewer
  scope and which selected outputs should become scope-owned media
