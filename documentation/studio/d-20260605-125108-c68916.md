...
last_updated: "2026-09-14 21:20:00"
...
| --- | --- | --- |
| Public publish source | `docs-viewer/scopes/<scope>/published/documents/` | `docs-viewer/scopes/<scope>/published/search/index.json` |
| Public read-only published snapshot | `site/assets/data/docs/scopes/<scope>/` | `site/assets/data/search/<scope>/index.json` |
| Manage/local | `docs-viewer/scopes/<scope>/published/documents/` | `docs-viewer/scopes/<scope>/published/search/index.json` |
...
Recent reads from one scope-level `recent.json`, a small `docs_recent_v1` payload with `basis`, `limit`, and `docs`. It combines ordinary documents and every configured sub-scope in the selected stage. The shared `docs_viewer.recent_limit` setting in `docs-viewer/config/scopes/docs_scopes.json` is 20; generated reader configuration and public runtime defaults carry the same value. Scopes without children use the same builder and retain an ordinary-document list.

Each row carries `doc_id`, title, content URL, one neutral `timestamp`, and optional parent context. Child rows additionally carry `sub_scope`, `report_doc_id` and `collection_title`. The provider supplies scope and stage; child navigation uses the exact report host and `subdoc` ID in that context. Missing or conflicting hosts are errors rather than another visibility filter. Public URL projection preserves the child's configured collection destination. Recent and Search share document-result navigation but read independent indexes.

Every route that enables Recent declares an `added` or `edited` basis. `/docs/` uses edited local projections for every selected scope, and the current public Analysis route uses an edited public projection. The same source scope can therefore have different local and public projections without a UI mode choice.

Only full scope builds generate or update Recent, including the full build within Pre-publish preparation. The builder gathers ordinary metadata from the current build and child metadata from each exact configured source collection, then sorts and limits the complete stage-local candidate set using the existing timestamp rules. Working includes drafts and publishing-excluded documents. Pre-publish includes everything preparation placed there. Recent performs no publishing-eligibility or route-visibility filtering; Publish transfers the prepared payload and deployment projects its URLs without recomputing the list. The existing separately prepared route projection can carry its configured date basis and public collection labels.

Single-document builds, child-only builds and document-deletion cleanup leave Recent output untouched, including when it is missing. This also applies to the separately prepared route projection. Recent may retain an old title or destination after an edit, move or deletion until the next full scope build; accepted/public readers advance with the normal snapshot workflow. Opening Recent reads JSON without rebuilding. No per-sub-scope Recent files, save-time aggregation or Search dependency are involved, and the ordinary Index tree does not acquire timestamp fields for this feature.
...
Manage/local rich flat indexes under `docs-viewer/scopes/<scope>/published/documents/index.json` remain valid for manage/report/tooling consumers.
