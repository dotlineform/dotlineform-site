...
| Recent | unchanged copy of Working `recent.json` | Working generated or copied Preview `recent.json` | One stage-independent payload declares the shared `added` or `edited` basis; readers own navigation. |
...
Prepare Preview validates and captures saved Working Recents alongside Search, binds its exact bytes into the reviewed plan, and copies it before recording the temporary build's completion manifest. The snapshot and Deploy Repo preserve those bytes without URL/label rewriting, reserialization, sorting, limiting or timestamp changes. Missing/invalid Recents fails without regeneration; a changed file invalidates the plan. Source freshness and equality with prepared document membership are not required. The separate `.publish/recent.json` variant is retired; the next full Working build removes that file. [The payload validator](../../docs-viewer/services/docs_recent_payload.py) owns the allowed copied fields and identity checks.
