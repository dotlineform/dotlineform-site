...
Site Search includes eligible ordinary documents plus Works and Processing subdocs. Each configured collection requires an explicit boolean `include_in_site_search`; `DocsStageConfig.site_search_collections` owns the selected set for Search and eventual Recents reuse. Catalogue, Concepts and Moments are excluded. New collection registration records `false`, so registration alone never adds a collection. Ordinary collection landing pages remain ordinary candidates regardless of their subdocs' inclusion policy. Recents adoption of this shared decision is a separate change.
