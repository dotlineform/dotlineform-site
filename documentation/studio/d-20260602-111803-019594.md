...
Every path is beneath `/studio/api/catalogue`. Successful mutations run the shared Catalogue output-completion service after canonical persistence. Work Details folder import uses the Detail-section operations. Responses distinguish saved data from output failure; there are no separate publication, Build or media-publish endpoints.
...
- the write dispatcher to Work, Series, Detail-section, bulk and delete services, followed by shared output completion
