...
last_updated: "2026-09-21 21:48:41"
...
The Working list loads generated `manage-manifest.json`; the public list uses `manifest.json`. These manifests are built from the collection's Markdown documents. The browser retains the loaded list data and DOM, filtering and sorting it locally. Opening a document fetches its generated `by-id/<doc_id>.json` content; returning to an unchanged mounted list reuses its rows as described above. Collection-report search and site Search retain their existing behavior; this feature adds no Search fields or automatic Search rebuild.
