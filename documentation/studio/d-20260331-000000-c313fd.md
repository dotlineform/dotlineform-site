...
last_updated: "2026-09-21 20:41:51"
...
Manage shows the Actions wrench controls in the top toolbar, Index panel and collection report toolbar only in Working. Pre-publish and Published hide these controls, including the entry points that activate document-selection checkboxes.
...
Changed content updates the rendered body and open Info while retaining route, selection and reading position. A collection detail toolbar is created once when opening its detail shell. Content refresh does not create, replace or remove that toolbar. Refresh errors have their own status and cannot reverse or repeat a successful Source Save. Pre-publish, Published and public routes remain read-only and do not join this Working authoring poll.
