...
`site-tools/config/site-code-update.json` explicitly projects shared runtime, CSS and 17 reader SVG assets into `site/docs-viewer/`. Its `docs-viewer-shared-icons` inventory contains `arrow-left.svg`, `bookmark-filled.svg`, `bookmark.svg`, `chart-column-colour.svg`, `chevron-left.svg`, `chevron-right.svg`, `circle-dashed-check.svg`, `clock-9.svg`, `dlf-home.svg`, `expand.svg`, `external-link.svg`, `info.svg`, `moon.svg`, `sprout.svg`, `sun.svg`, `waypoints.svg`, and `x.svg`. `site-tools/site_code_update.py` permits this explicit SVG subset. Management-only mappings and the remaining downloaded assets stay local.
