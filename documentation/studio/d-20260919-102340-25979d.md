...
The selected reader, management, source-editor and content-detail controls, toolbar menus, index header and Info close control now use this foundation. The mapping records the exact consumers and state choices. VS Code retains its 20px brand image inside the shared borderless button with the same hover colour. Index-row artwork, report-owned controls and other unconverted surfaces remain outside this toolbar migration.
...
| Button size | Existing `--docs-viewer-control-height` on `.docsViewer__topRow`, `.docsViewer__mainViewToolbarActions` and `.docsViewer__sidebarHeader` | Button width, minimum width and height, `2rem` (32px at a 16px root font). Independent of artwork size. |
...
An agreed per-control exception can override `--docs-viewer-toolbar-icon-size` on that control. The Info close control uses `1rem` artwork inside its existing `1.8rem` button. Record exceptions in the mapping's Appearance column. Keep dimensions out of artwork classes so one drawing remains reusable at different sizes. Menu items retain their text; their icon column uses the same artwork-size token.

The button-height token deliberately remains scoped to `.docsViewer__topRow`, `.docsViewer__mainViewToolbarActions` and `.docsViewer__sidebarHeader`; moving it to the viewer root would also affect existing controls that use a different size elsewhere. The main-view scope also keeps shared document controls correctly sized in a host without the Public/Manage top row, such as Review. The Info close control supplies its own local override. Any future toolbar outside those scopes must explicitly provide `--docs-viewer-control-height` through its layout owner before using the button class.
...
`site-tools/config/site-code-update.json` explicitly projects shared runtime, CSS and 13 reader SVG assets into `site/docs-viewer/`. Its `docs-viewer-shared-icons` inventory contains `arrow-left.svg`, `bookmark-filled.svg`, `bookmark.svg`, `chevron-left.svg`, `chevron-right.svg`, `clock-9.svg`, `expand.svg`, `external-link.svg`, `info.svg`, `moon.svg`, `sun.svg`, `waypoints.svg`, and `x.svg`. `site-tools/site_code_update.py` permits this explicit SVG subset. Management-only mappings and the remaining downloaded assets stay local.
