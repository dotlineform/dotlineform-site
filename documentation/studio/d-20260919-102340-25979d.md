...
The selected reader, management, source-editor, content-detail and report-toolbar controls, toolbar menus, collection list/detail controls, index header and Info close control now use this foundation. The mapping records the exact consumers and state choices. VS Code retains its 20px brand image inside the shared borderless button with the same hover colour. Index-row artwork, report list/header artwork and other unconverted surfaces remain outside this toolbar migration.
...
Broken Links uses `refresh-cw.svg` when idle and plain disabled `Running...` text with no border or background while busy; its owner supplies `aria-busy` and an accessible running label. Project State's Copy Markdown control and serializer are retired; Copy table retains the existing TSV output. List-row Subject/status/draft artwork and sortable-header indicators remain separately owned and unchanged by these toolbar choices.
