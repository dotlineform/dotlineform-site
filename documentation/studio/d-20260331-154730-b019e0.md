...
Main consumers load it through `studio/shared/python/pipeline_config.py`. Work and Work Detail input paths then use `studio/shared/python/catalogue_work_media_sources.py`; catalogue generation/build media, project-media selection and source-coverage reports must not reconstruct those roots independently.
...
The shared resolver verifies the configured external root, rejects symlinked or escaping path segments, and never falls through from one identity to another. Work Detail media inherits its parent Work identity. The selected root changes only canonical local input lookup: derivative staging, generated asset names and primary URLs retain their existing identities.
...
`studio/services/media/make_srcset_images.py` converts staged source images into the configured primary and thumbnail variants. The normal Work and Detail path is Save output completion, which resolves source images, calls this builder, maintains generated thumbnails and uploads required complete primary sets. Staged sources and primaries live below `catalogue/media-staging/`; final thumbnails live below `catalogue/generated/works/thumbs/` and `catalogue/generated/work_details/thumbs/`. Use the standalone command for explicit media maintenance or debugging, not as a second catalogue workflow:
