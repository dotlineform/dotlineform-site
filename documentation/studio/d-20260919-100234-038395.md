...
last_updated: "2026-09-19 17:58:46"
...
Use this document to choose replacements for Docs Viewer toolbar and report icons. It belongs to [Shared Icons](d-20260910-223116-9a1011.md). The selected viewer-toolbar mappings were implemented on 19 September 2026 and await visual review at the current 20px size. Their previous-icon column retains the migration baseline. The separate Report Toolbars and Report List Icons tables inventory the current implementation; their replacement choices are open.
...
Inspected on 19 September 2026. These are selection rows only; adding this table does not change report runtime code. The new **Current icon or text** column describes what is still rendered. Leave **Replacement SVG**, **Appearance**, and **Decision / gap** open until selecting a replacement or explicitly keeping text/artwork. The earlier viewer-toolbar choices are available for reuse where the meaning agrees.
...
| --- | --- | --- | --- | --- | --- | --- |
| Collection list: sort | selector `[data-docs-collection-sort="title-asc"]` | Sorted by title; click switches to recently updated | 🔤 | arrow-down-a-z.svg | — | — |
| Collection list: sort | selector `[data-docs-collection-sort="last-updated-desc"]` | Sorted by recently updated; click switches to title | 🕒 | clock-3.svg | — | — |
| Collection list: New | selector `[data-docs-collection-new]`; action `new` | Creates a document in the exact collection when available | 📄 | file.svg | — | — |
| Collection list: Regenerate | selector `[data-docs-collection-regenerate]`; action `catalogue-regenerate` | Runs Catalogue regeneration when that contribution is available | 🔄 | refresh-cw.svg | — | — |
| Collection list: Actions | selector `[data-docs-collection-actions]` | Opens collection Actions menu | 🔧 | wrench.svg | — | — |
| Collection Actions menu: package | DOM `docsViewerCollectionPreparePackageButton`; action `prepare-document-package` | Prepares a package for the selected documents | 📦 and text `Prepare package…` | package.svg and text | — | — |
| Collection selection: select all | selector `[data-docs-collection-selection-command="select-all"]` | Selects all eligible documents | Text `Select all` | Text `All` | — | — |
| Collection selection: clear | selector `[data-docs-collection-selection-command="clear"]` | Clears selection | Text `Clear` | Text `Clear` | — | — |
| Collection selection: done | selector `[data-docs-collection-selection-command="done"]` | Ends selection mode | Text `Done` | Text `Done` | — | — |
| Collection list: clear search | `docs_collection`; selector `[data-docs-collection-filters] .docsViewerReport__searchClear` | Clears title search; hidden while empty | × | x.svg | — | small, same as current size |
| Collection detail: return | selector `.docsReportDetail__back` | Returns to the collection list and its saved position | ← | arrow-left.svg | — | — |
| Collection detail: draft readiness | selector `[data-docs-collection-draft]`; action `set-draft` | Draft (`aria-pressed=true`); click marks Ready | 📝 | circle-dashed-check.svg | — | — |
| Collection detail: draft readiness | selector `[data-docs-collection-draft]`; action `set-draft` | Ready (`aria-pressed=false`); click marks Draft | ✅ | circle-check.svg | — | — |
| Collection detail: copy link | selector `[data-docs-collection-copy-link]`; action `copy-link` | Copies the exact detail document's Markdown link | 🔗 | link.svg | — | — |
| Collection detail: Delete | selector `[data-docs-collection-delete]`; action `delete` | Deletes the validated detail when available; same artwork while checking/disabled | 🗑️ | trash.svg | — | — |
| Subject-aware collection detail: assign | selector `[data-docs-projects-assign-subject]`; action `assign-subject` | Opens Subject assignment when configured | Text `Subject` | dlf-subject.svg | — | improve to use dlf-subject-assigned.svg when a subject is set |
| Subject-aware collection detail: Finder | selector `[data-docs-projects-open-folder]`; action `open-project-folder` | Opens a valid Folder subject in Finder; disabled without one | Text `Open in Finder` | folder-open.svg | — | no text |
| Report presentation: expand | selector `.docsViewerReport__detailOpen`; function `createOpenControl()` | Opens a report's expanded view within Docs Viewer | Inline external-link-style SVG | square-arrow-out-up-right.svg | — | This action opens an internal expanded view, not a new browser tab |
| Expanded table: reset | renderer `content-detail-reset-widths` | Resets resized columns; shared with other managed semantic-table details | Text `Reset widths` | unfold-horizontal.svg | — | — |
| Expanded table: copy | renderer `content-detail-copy-table` | Copies the visible semantic table as TSV | Inline overlapping-sheets SVG | copy.svg | — | — |
| Links: refresh | `workspace_links`; `mountWorkspaceLinksReport(): refresh`; selector `[aria-label="Refresh links"]` | Rereads the saved Links report | 🔄 | refresh-cw.svg | — | — |
| Unpublishable: refresh | `unpublishable`; `mountUnpublishableReport(): refresh` | Rereads the publication-ignore list | Text `Refresh` | refresh-cw.svg | — | — |
| Unpublishable: edit source | `unpublishable`; `mountUnpublishableReport(): open` | Opens `unpublishable.json` in VS Code | Text `Open in VS Code` | file-pen.svg | — | — |
| Works in Series: refresh | `series_works`; `mountSeriesWorksReport(): refresh` | Reloads the containing Series' generated member Works | Text `Refresh` | refresh-cw.svg | — | this report is not currently used? |
| Broken Links: run | DOM `docsBrokenLinksReportRun` | Idle; click starts the audit | Text `Run audit` | refresh-cw.svg | — | no text |
| Broken Links: run | DOM `docsBrokenLinksReportRun` | Audit running; disabled | Text `Running...` | — | remove button border, just plain text | Preserve a visible or accessible busy indication if replacing the text |
| Project State: run | DOM `docsProjectStateReportRun` | Runs/refreshes reconciliation | 🔄 | refresh-cw.svg | — | — |
| Project State: group | DOM `docsProjectStateReportGroup`; selector `[data-group-target="series"]` | Currently grouped by Folder; click groups by Series | Inline three-row list SVG; `CONTROL_ICON_MARKUP.list` | dlf-series.svg / folder.svg | — | Change so that icon depicts the active grouping |
| Project State: group | DOM `docsProjectStateReportGroup`; selector `[data-group-target="folder"]` | Currently grouped by Series; click groups by Folder | Inline folder SVG; `CONTROL_ICON_MARKUP.folder` | dlf-series.svg / folder.svg | — | Change so that icon depicts the active grouping |
| Project State: clear search | `project_state`; selector `.docsViewerReport__searchClear` | Clears search; hidden while empty | × | x.svg | — | small, same as current size |
| Project State: copy table | DOM `docsProjectStateReportCopyTable` | Copies the report table | Inline overlapping-sheets SVG; `CONTROL_ICON_MARKUP.copy` | copy.svg | — | — |
| Project State: copy Markdown | DOM `docsProjectStateReportCopyMarkdown` | Copies the report as Markdown | Text `MD` | — | — | this is to be retired |
| Docs Media: run | DOM `docsMediaReportRun` | Runs/refreshes the media inventory | 🔄 | refresh-cw.svg | — | — |
| Docs Media: clear search | `docs_media`; selector `.docsViewerReport__searchClear` | Clears search; hidden while empty | × | x.svg | — | — |
| Works: refresh | DOM `docsWorksReportRefresh` | Refreshes Series documentation coverage | 🔄 | refresh-cw.svg | — | Separate report from the Works collection list and Works in Series |
| Catalogue Works: clear search | DOM `docsCatalogueWorksReportClear` | Clears the canonical Work search | × | x.svg | — | — |
| Catalogue Works: copy table | DOM `docsCatalogueWorksReportCopy` | Copies the filtered report table | Text `Copy table` | copy.svg | — | — |
| Uncataloged Files: run | DOM `docsUncatalogedFilesReportRun` | Runs/refreshes the source-folder audit | 🔄 | refresh-cw.svg | — | — |
| Missing Source Files: run | DOM `docsMissingSourceFilesReportRun` | Runs/refreshes the expected-source-file audit | 🔄 | refresh-cw.svg | — | — |
...
The ten `ui_status` values come from `DOCS_VIEWER_CODE_CONFIG.uiStatuses`. Collection rows render them through `renderRow()` in the default management contribution. That palette is also consumed by the document index and other status presentation, so a future change must decide whether it changes the shared mapping or only the report renderer. Subject icons are a separate classification and must not be inferred from collection membership or status.

| Area / icon | Code identifier | State / meaning | Current icon or text | Replacement SVG | Appearance | Decision / gap |
| --- | --- | --- | --- | --- | --- | --- |
| Collection row: status | `ui_status="done"`; selector `.docsViewer__navStatus` | Done | ✅ | — | — | — |
| Collection row: status | `ui_status="stopped"`; selector `.docsViewer__navStatus` | Stopped | 🚫 | — | — | — |
| Collection row: status | `ui_status="urgent"`; selector `.docsViewer__navStatus` | Urgent | ❗ | — | — | — |
| Collection row: status | `ui_status="review"`; selector `.docsViewer__navStatus` | Review | 👀 | — | — | — |
| Collection row: status | `ui_status="in-progress"`; selector `.docsViewer__navStatus` | In progress | 🔄 | — | — | — |
| Collection row: status | `ui_status="planned"`; selector `.docsViewer__navStatus` | Planned | 🧭 | — | — | — |
| Collection row: status | `ui_status="paused"`; selector `.docsViewer__navStatus` | Paused | ⏸️ | — | — | — |
| Collection row: status | `ui_status="deferred"`; selector `.docsViewer__navStatus` | Deferred | 😴 | — | — | — |
| Collection row: status | `ui_status="report"`; selector `.docsViewer__navStatus` | Report | 📊 | — | — | — |
| Collection row: status | `ui_status="research"`; selector `.docsViewer__navStatus` | Research | 🔬 | — | — | — |
| Collection row: draft | `document.draft === true`; selector `.docsViewer__draftIndicator` | Draft cue beside the title; independent of `ui_status` | 📝 | — | — | — |
...
| Subject cue: Folder | `appendProjectSubjectIcon(parent, "folder")`; selector `[data-project-subject-cue="folder"]` | Folder subject in subject-aware collection rows and Project State document links | 📁 | — | — | — |
| Subject cue: Work | `SUBJECT_ICON_MARKUP.work`; selector `[data-project-subject-icon="work"]` | Work subject in collection rows, Project State and Works document links | Inline square bullet and one horizontal line SVG | — | — | Existing drawing is reusable but has no standalone shared SVG asset yet |
| Subject cue: Series | `SUBJECT_ICON_MARKUP.series`; selector `[data-project-subject-icon="series"]` | Series subject in collection rows, Project State and Works document links | Inline three square bullets and three horizontal lines SVG | — | — | Existing drawing is reusable; Project State's grouping control also contains this drawing |
| Subject cue: Detail | `SUBJECT_ICON_MARKUP.detail`; selector `[data-project-subject-icon="detail"]` | Detail subject in subject-aware collection rows | Inline picture-frame SVG with a zigzag line | — | — | Existing drawing is reusable but has no standalone shared SVG asset yet |
...
| Subject cell: none | selector `[data-project-subject-state="none"]` | No Subject | Text `—`; accessible name `No subject` | — | — | — |
...
| Broken Links header | `docs_broken_links`; selector `[data-report-sort] .docsViewerReport__sortIndicator` | Active ascending sort | Text `asc` | — | — | This owner currently differs from the triangle indicators |
| Broken Links header | `docs_broken_links`; selector `[data-report-sort] .docsViewerReport__sortIndicator` | Active descending sort | Text `desc` | — | — | This owner currently differs from the triangle indicators |
...
The triangle sort indicators are emitted by `docs-index-table-report.js`, `semantic-tokens-report.js`, `project-state-report.js`, `docs-media-report.js`, `catalogue-works-report.js`, `workspace-links-report.js`, and the subject-aware collection `listSortButton()` in `docs-viewer-management-collection-working-subjects.js`. The collection header uses `[data-project-sort]`; several report headers use `[data-report-sort]`. Broken Links has the same empty inactive state but uses `asc`/`desc` text when active.

The inline Subject SVG drawings currently use a `24 × 24` viewBox, `1rem` rendered size, `currentColor` strokes, no fill, and `1.75` stroke width; Folder remains emoji. Their size is independent of the new toolbar-icon token. Record any proposed list size or fill change explicitly; selecting a toolbar asset does not automatically require 20px row artwork. A missing/unrecognised `ui_status` emits no status cue. Status, draft and Subject cues can coexist; they are not alternative names for the same state.
...
- Subject artwork shared across collection lists, Project State and Works: `docs-viewer/runtime/js/reports/project-subject-icons.js`. Status values/emoji: `docs-viewer/runtime/js/shared/docs-viewer-code-config.js`.
...
- Report presentation: `docs-viewer/static/css/docs-viewer-reports.css`, `docs-viewer-local-reports.css` and the report/collection rules in `docs-viewer-manage.css`. The report tables are an inventory of these current owners, not a claim that they already consume the shared toolbar-icon styles.
...
Choose replacements in the **Report Toolbars** and **Report List Icons** tables, then implement the agreed choices as a separate change. Their current controls, artwork and state variants were inspected in source; no report runtime or asset changes were made while adding the inventory. Add a separate **Index** table for document-row/type/status artwork later. Reuse agreed meanings across these areas and record differing state or presentation explicitly.
