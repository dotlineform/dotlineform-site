...
last_updated: "2026-09-20 07:24:22"
...
Use this document to choose replacements for Docs Viewer toolbar and report icons. It belongs to [Shared Icons](d-20260910-223116-9a1011.md). The selected viewer-toolbar and report-toolbar mappings were implemented on 19 September 2026 and await visual review at the current 20px size. Their previous-icon columns retain the migration baseline. The Report List Icons table remains an inventory with open replacement choices.
...
| Unpublishable: edit source | `unpublishable`; `mountUnpublishableReport(): open` | Opens `unpublishable.json` in VS Code | Text `Open in VS Code` | file-pen.svg | — | Implemented; review pending |
...
| Subject cue: Series | `SUBJECT_ICON_MARKUP.series`; selector `[data-project-subject-icon="series"]` | Series subject in collection rows, Project State and Works document links | Inline three square bullets and three horizontal lines SVG | — | — | List drawing remains inline; Project State's toolbar now uses the standalone dlf-series.svg |
...
These 76 SVG filenames are present in `docs-viewer/static/icons/`, including downloaded and custom artwork. Presence is an available choice, not an assigned meaning or adoption decision. The same drawing may serve more than one control when their meanings agree. `bookmark-filled.svg` is a local filled derivative of Lucide's `bookmark.svg`, preserving its path geometry. Non-SVG design files are not runtime assets. VS Code remains separately located as recorded above.
...
- Report presentation: `docs-viewer/static/css/docs-viewer-reports.css`, `docs-viewer-local-reports.css` and the report/collection rules in `docs-viewer-manage.css`. Report-toolbar controls now consume the shared toolbar-icon styles; list/header artwork retains its existing presentation.
...
Review the implemented viewer and report toolbars visually. Choose replacements in **Report List Icons** for a separate implementation, then add an **Index** table for document-row/type/status artwork. Reuse agreed meanings across these areas and record differing state or presentation explicitly.
