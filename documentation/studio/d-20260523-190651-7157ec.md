...
last_updated: "2026-09-15 21:07:58"
...
- [ ] Keep application workflows synchronous from the user's perspective: start A, wait for its complete outcome, then allow B. This applies across the app, including Save, Build, Refresh, Pre-Publish and Publish.
...
- [ ] Every scope's Working publication policy belongs to its configured ordinary `working/source/documents/unpublishable.json` and document `draft` state. Pre-publish excludes exact ignored ordinary IDs and draft subtrees, including collections whose host is excluded. Publish accepts the complete prepared set; Deploy distributes it. Do not restore `publishable`, per-report `access`, inferred report exclusions or downstream eligibility filters.
- [ ] All configured scopes require explicit Working and Pre-publish source/generated targets and share the Published reader. Ordinary source edits use `working/source/documents/`; Pre-publish is read-only preparation output and Published is accepted at the scope root. Works owns Subject specialisation; ordinary scope documents retain authored fields unchanged.
