...
- [ ] Working alone builds Search. Prepare Preview captures the existing index with its plan, copies exact bytes and records them in the completed snapshot; Deploy Repo copies the same file unchanged. Do not rebuild Search during Preview or impose source freshness after ordinary edits. Search corpus/readiness policy remains separately scoped; document completeness does not depend on Search membership.
