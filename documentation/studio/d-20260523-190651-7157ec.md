...
- [ ] Keep application workflows synchronous from the user's perspective: start A, wait for its complete outcome, then allow B. This applies across the app, including Save, Build, Refresh, Prepare Preview and Deploy Repo.
...
- [ ] The single Docs workspace has Working source/generated storage and a read-only `preview/` snapshot directly beneath the existing `DOTLINEFORM_DOCS_BASE_DIR`. Working registers collections and media once; Preview derives configuration. Prepare Preview uses temporary captured input/output, validates before replacement, records completion after byte verification and removes temporary storage. Preview has no source tree or authoring capability. Deploy Repo consumes the reviewed Preview revision; public GitHub Actions deployment remains separate. Works owns Subject specialisation; ordinary documents retain authored fields unchanged.
- [ ] Deploy Repo counts accepted by-ID documents directly. Public document-location JSON, its unused picker and legacy Catalogue document-URL refresh are retired; do not restore the UAC-1 pause or old Catalogue schema writer. Current Catalogue deployment distributes its selected generated consumer files under `site/assets/data/catalogue/` through its separate owner.
