...
- Keep the same media skeleton beneath Working and Pre-publish: `<stage>/source/media/{img,svg,files,html,build-source/mermaid}`. Configured sub-scopes retain their collection-owned source/generated/media beneath each stage. Empty directories are intentional and may be retained.
...
- If the watcher is unavailable, regenerate an ordinary doc-only source change with `$HOME/miniconda3/bin/python3 docs-viewer/build/build_docs.py --stage working --write --only-doc-ids <comma-separated-doc-ids> --skip-media-builds`; add `--sub-scope <child-id>` for a configured child collection. The targeted build still recomputes the collection indexes while preserving unaffected by-ID payloads.
...
- Working `draft` state and `working/source/documents/unpublishable.json` own preparation eligibility. Missing `draft` remains draft; no metadata backfill is implied. Publish accepts the complete prepared set and changes only the external `published/` snapshot. Every downstream consumer uses that same accepted document set. Subject specialisation belongs to the configured Works sub-scope, not ordinary documents.
