...
- Deploy Repo is a revision-bound plan/apply operation over one complete Preview snapshot. It may perform configured destination preparation and reconcile only its owned repository/R2, document-location, Catalogue, and publication-lineage projections; it must not read source or generated output, invoke Build or Prepare Preview, or apply another document filter. Deployed public payloads retain their separate `published` wire identity.
