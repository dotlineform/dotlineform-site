...
- Search JSON is stage-independent: `docs_viewer_search_index_v4` carries document/collection/report-host identity and postings, without stage or result URLs. Readers own stage/index location and build navigation from exact IDs. Snapshot manifests own provenance; do not reintroduce Search stage rewriting or stage-based content hashing. Search coverage and Working-only build/copy ownership remain separately scoped changes.
