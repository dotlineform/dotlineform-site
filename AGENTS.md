...
- For `studio` scope documents, read the current revision and freely replace the complete body when useful; the user does not edit that scope, so avoid extra merge or confirmation friction unless a real revision conflict appears. Treat `notes/tmp` as potentially user-active and do not replace it unless explicitly requested.
