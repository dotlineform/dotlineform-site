...
- Routine docs-source edits are regenerated by the docs watcher running under `bin/local-studio`. The watcher rebuilds document projections only; inspect those outputs and do not rerun the builder solely for idempotence evidence.
- The `studio` scope is the reference scope for live development and maintenance documents.
- If the watcher is unavailable, regenerate an ordinary doc-only source change with `$HOME/miniconda3/bin/python3 docs-viewer/build/build_docs.py --scope studio --write --only-doc-ids <comma-separated-doc-ids> --skip-media-builds`. The targeted build still recomputes the collection indexes while preserving unaffected by-ID payloads.
...
- Docs search has no targeted-postings mode and does not follow ordinary watcher or management writes automatically. Use the Manage Rebuild control to rebuild the active scope's Docs and complete Search index, or run `$HOME/miniconda3/bin/python3 docs-viewer/build/build_search.py --scope studio --write` when a direct search-only refresh is warranted.
