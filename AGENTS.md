...
- Routine docs-source edits are regenerated by docs watcher running under `bin/local-studio`. If the watcher regenerates the intended document and search projections, treat that as the build follow-through: inspect the outputs and do not rerun builders solely for idempotence evidence.
...
- Docs search has no targeted-postings mode. If the watcher is unavailable and a search-relevant source changed, rebuild the complete selected-scope index with `$HOME/miniconda3/bin/python3 docs-viewer/build/build_search.py --scope studio --write`.
