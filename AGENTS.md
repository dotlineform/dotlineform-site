...
- `docs-viewer/config/workspace/docs-workspace.json` is the authority for storage resolution. Working owns source/generated operations and collection/media registration; Preview derives its configuration and has its own snapshot reader. Temporary Preview source/generated inputs require an explicit isolated build workspace. If the configured external root is unavailable, report the workspace as unavailable; do not create a replacement root, infer a repository path, or manufacture a second copy.
- Working keeps `source/media/{img,svg,files,html,build-source/mermaid}` and collection-owned source/generated/media. Prepare Preview captures eligible source and media into a temporary build workspace, writes the finished snapshot, and removes temporary inputs. Preview has no persistent source/generated tree. Empty directories are intentional and may be retained.
...
- If the watcher is unavailable, regenerate an ordinary doc-only source change with `$HOME/miniconda3/bin/python3 docs-viewer/build/build_docs.py --stage working --write --only-doc-ids <comma-separated-doc-ids> --skip-media-builds`. For a configured named collection, add `--collection <child-id>` while retaining `--only-doc-ids`. Named-collection targeted builds read selected source files and merge their metadata into saved manifests, preserving unselected by-ID payloads. Ordinary targeted builds still read their sources to recompute indexes. Omit `--only-doc-ids` only for an intentional complete collection build.
...
- Search JSON is stage-independent: `docs_viewer_search_index_v4` carries document/collection/report-host identity and postings, without stage or result URLs. Working is the sole Search builder. Prepare Preview captures and copies its existing index byte-for-byte, binds it into the preparation plan revision and fails if the file is missing or invalid. Deploy Repo copies the same bytes without label rewriting or rehashing. Readers own route context; snapshot manifests own provenance. Search coverage and inherited readiness filtering remain separately scoped changes; this workflow does not require Search membership to equal the prepared document set or require freshness after ordinary edits.
...
- A complete Working Build replaces its generated files and records completion. Prepare Preview is one awaited operation: select eligible Working source, build the captured set in temporary storage, validate it, replace `preview/`, and record `preview-manifest.json` after byte verification. Review Preview before Deploy Repo. Preparation failure before replacement leaves the previous Preview intact; failure during replacement leaves no valid completion receipt and requires preparation again.
- Working `draft` state and `working/source/documents/unpublishable.json` own preparation eligibility. Every ordinary and collection source document requires an explicit boolean `draft`. Draft and unpublishable exclusions include descendants; an excluded collection host excludes its collection. Prepare Preview captures that complete eligible set. Every downstream consumer uses the same prepared document set. Subject specialisation belongs to the configured Works collection, not ordinary documents.
- Deploy Repo is a revision-bound plan/apply operation over one complete Preview snapshot. It may perform configured destination preparation and reconcile only its owned document/Search repository output, R2 media and publication-lineage projections; it must not read source or generated output, invoke Build or Prepare Preview, or apply another document filter. Deployed public payloads retain their separate `published` wire identity. Public document-location JSON and the legacy Catalogue document-URL refresh are retired. Current Catalogue file deployment is separately owned and pending; do not restore the old schema writer or its paused status.
- Working exposes Prepare Preview; Preview exposes Deploy Repo with its concrete change review. There is no separate local Publish action or combined preparation/deployment modal. Git commit and push remain explicit ordinary user actions. Deploy Public remains the separate manually triggered GitHub Pages workflow over the committed `site/` snapshot.
- Recovery uses ordinary sources and Git history: fix the source, builder, or destination and rerun the owning Build, Prepare Preview, or Deploy Repo operation. Publication lineage requires exact configured Working collection owners; current configuration has no lineage workflow, and the two inactive historical tables were explicitly retired at cutover. The clean Git commit immediately before the Stage 2.3 repository-copy removal remains the recovery point for that retired tracked tree; do not add aliases, shadow copies, retry markers, transactional swaps, or automatic backup directories.
