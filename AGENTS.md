...
- A complete Build replaces the selected stage's `generated/` files and records the completed generated snapshot. Working owns authoring; Pre-publish explicitly prepares eligible Working source and builds its derivative. Review Pre-publish before Publish. Publish replaces the managed files under scope-root `published/` with that prepared snapshot. Empty directories may remain.
- Each scope's Working `draft` state and `working/source/documents/unpublishable.json` own preparation eligibility. Missing `draft` remains draft; no metadata backfill is implied. Publish accepts the complete prepared set and changes only the scope's external `published/` snapshot. Every downstream consumer uses that same accepted document set. Subject specialisation belongs to the configured Works sub-scope, not ordinary scope documents.
