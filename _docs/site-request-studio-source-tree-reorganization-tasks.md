...
- Completed `STSR-003` by adding [Studio Source Tree Reorganization Inventory](/docs/?scope=studio&mode=manage&doc=site-request-studio-source-tree-reorganization-inventory).
- Classified current Studio-related path families as Studio source, public Jekyll source, public generated output, future Docs Viewer extraction candidate, Studio generated/local working output, mixed, or out-of-scope dependency.
- Marked ambiguous path families in the inventory with `STSR-004` so the next session can resolve ownership before any file moves.
- Clarified Docs Viewer ownership: `_docs/` is one Docs Viewer source scope among others, `_docs_library/` and `_docs_analysis/` are Docs Viewer-owned scopes, `_docs_logs/` is Studio-owned change request workflow data, and `_docs_catalogue/` is Studio-owned canonical Catalogue data.
- Clarified report ownership: Docs Viewer owns report documents/config/runtime such as `change-history.md`, but the data a report reads remains with its owning domain, such as Studio-owned `_docs_logs/`.
- Clarified `STSR-004` steering: public site ownership is limited to publishing source, published-page runtime, public assets, and generated runtime data; build scripts, checks, tests, fixtures, search builders, and verification infrastructure are Studio or Docs Viewer owned.

### steer for next task

- Start with `STSR-004`; resolve the inventory rows marked `STSR-004` before moving files.
- Do not move files in `STSR-004`; record ownership decisions in the inventory, parent request, or this tracker as needed.
...
- Treat `assets/studio/data/catalogue_lookup/` as Studio-generated lookup/read-model data for editors, not public page data. `STSR-004` should define lookup versus cache.
