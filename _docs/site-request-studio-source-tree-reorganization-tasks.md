...
| STSR-004 | planned | Resolve ambiguous ownership from the inventory before moving files; update the parent request, inventory, or this table for any path whose owner is unclear, especially canonical catalogue data, `_docs_catalogue/` publishing Markdown, Docs Viewer source scopes, Docs Viewer report source/config/runtime, Studio-owned `_docs_logs/` workflow data consumed by reports, Studio projections, current Docs Viewer code/config/services/assets, generated Docs Viewer public payloads, generated change request log/report artifacts, projection checks, public route JavaScript, and UI Catalogue assets. |
