...
- canonical prose: `_docs_catalogue/moments/<moment_id>.md`
...
- Should `delete_moment.py` delete canonical prose from `_docs_catalogue/moments/`, or remain source-metadata/runtime cleanup only by default?
...
- Documentation needs classification before cleanup. Current workflow docs should describe `moments.json`, `_docs_catalogue/moments/`, and `var/docs/catalogue/import-staging/moments/`; migration/change-log docs may remain as historical context when clearly framed that way.
...
- align delete script docs and behavior with `_docs_catalogue/moments/` as the canonical prose source
