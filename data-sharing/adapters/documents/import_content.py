#!/usr/bin/env python3
"""Normalize trusted Data Sharing document rows into generic Docs Import content."""
...
def package_contract(package_metadata: dict[str, Any]) -> tuple[str, str]:
    ...
    raise ValueError(
        "unsupported Data Sharing documents import contract: "
...
def normalize_documents_import_content(
    ...
    *,
    ...
    if not isinstance(package_metadata, dict):
        raise ValueError("Data Sharing package metadata must be an object")
    package_kind, profile_id = package_contract(package_metadata)
    export_id = clean_text(package_metadata.get("export_id"))
    if not export_id:
        raise ValueError("Data Sharing package export_id is required")
    ...
    selected_doc_ids = package_metadata.get("selected_doc_ids")
    if isinstance(selected_doc_ids, list):
        existing_ids.update(clean_text(doc_id) for doc_id in selected_doc_ids if clean_text(doc_id))
