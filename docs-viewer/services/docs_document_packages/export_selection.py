#!/usr/bin/env python3
"""Source loading and document selection helpers for document-package exports."""
...
from docs_document_packages import source_records
from docs_document_packages.export_common import normalize_text
...
class ExportContext:
    ...
    content_format: str
    config: dict[str, Any]
    source_context: docs_source_context.DocumentPackageSourceContext
...
def load_source_export_context(repo_root: Path, scope: str) -> tuple[docs_source_context.DocumentPackageSourceContext, list[dict[str, Any]]]:
    context = docs_source_context.load_document_package_source_context(repo_root, scope)
...
def selected_docs(
    ...
    *,
    ...
    include_non_viewable: bool | None,
    ...
    for doc_id in requested_ids:
        ...
        if doc is None:
            ...
            continue
        if not effective_include_non_viewable(context.config, include_non_viewable) and doc.get("viewable") is False:
...
def effective_include_non_viewable(config: dict[str, Any], override: bool | None) -> bool:
    selection = config.get("selection", {})
    if override is not None and selection.get("supports_include_non_viewable"):
        return override
    return selection.get("include_non_viewable") is not False
