"""Read one scope's exact Working ordinary-document exclusion list."""
...
def publication_ignore_path(repo_root: Path, scope: str) -> Path:
    """Resolve the exact Working policy file without creating or substituting it."""
    config = load_docs_scope_stage(repo_root, scope, "working")
...
def read_publication_ignore_ids(repo_root: Path, scope: str) -> frozenset[str]:
    """Read fresh exact IDs; missing, unreadable and invalid files fail visibly."""
    path = publication_ignore_path(repo_root, scope)
    try:
...
def working_ignored_doc_ids(repo_root: Path, config: DocsScopeConfig) -> frozenset[str]:
    ...
    if config.stage != "working":
        return frozenset()
    return read_publication_ignore_ids(repo_root, config.scope_id)
