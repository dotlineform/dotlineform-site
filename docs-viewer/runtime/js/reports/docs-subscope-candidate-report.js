...
} from "../shared/docs-subscope-report-filter.js";
...
function resolveReportContribution(context) {
  var contribution = context && Object.prototype.hasOwnProperty.call(
    context,
...
function visibleDocuments(state) {
  var query = normalizeDocsSubscopeFilterValue(state.query);
  var visible = state.docs.filter(function (doc) {
    if (query && !normalizeDocsSubscopeFilterValue(doc.title).startsWith(query)) return false;
 ...
 */
function mountResolvedDocsSubscopeCandidateReport(context, contribution) {
...
export function mountDocsSubscopeCandidateReport(context) {
  var root = context && context.reportRoot;
  if (!root) return Promise.resolve(false);
  return resolveReportContribution(context)
    .then(function (contribution) {
      return mountResolvedDocsSubscopeCandidateReport(
