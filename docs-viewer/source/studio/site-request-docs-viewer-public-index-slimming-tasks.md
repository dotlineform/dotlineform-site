...
| 8 | done | [Batch 8: Nested Tree Payload Correction](/docs/?scope=studio&doc=site-request-docs-viewer-public-index-slimming-batch-8) |
...
- Batch 8 corrected `index-tree.json` to a nested build-time tree payload. The shared runtime adapter derives in-memory parent ids from nested child position, so generated tree nodes no longer carry `parent_id` and no flat adjacency-list compatibility path remains.
...
- 2026-06-05: Batch 7 docs-source updates, cleanup scans, and `git diff --check` passed. Generated Docs Viewer payloads were not rebuilt.
- 2026-06-05: Batch 8 syntax checks, focused pytest, Node adapter/data module check, dry-run generated checks for Library/Analysis/Studio, Studio generated writes, and the Docs Viewer smoke profile passed. Studio `index-tree.json` was rebuilt for the nested contract, and Studio generated docs were regenerated after closeout docs-source edits. Public Library/Analysis tree payloads were checked and did not require writes. Smoke summary: `var/test-runs/20260605-210337/summary.md`.
