...
The app lists available folders and enables user to choose `<folder-id>`.
...
The app should include a Build button for the selected review folder `<folder-id>`.
...
- `GET /docs-review/folders/index-tree?folder_id=<folder-id>`
- `GET /docs-review/folders/payload?folder_id=<folder-id>&doc_id=review-root`
- `POST /docs-review/folders/build` with `{ "folder_id": <folder-id> }`

The frontend should not use normal generated-data runtime until a clean payload-provider abstraction exists.

## Implementation Tasks

### 1. Scaffold the `/docs-review/` app

- create the docs-viewer-review/ folder with copies of the docs-viewer/ modules.

### 2. Remove Normal Docs Viewer from the Review UI
...
- On boot, call the review-folder list endpoint, then load `<folder-id>` index tree when built.
...
- Build action posts `{ "folder_id": <folder-id> }`, then reloads the index tree and selected/default document.
...
- the page can list and select the `<folder-id>` review folder
