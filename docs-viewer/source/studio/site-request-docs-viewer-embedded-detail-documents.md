...
Working request: `site-request-docs-viewer-embedded-detail-documents`.

Completed:
- Step 2: Sub-scope lifecycle actions.
- Step 3: Manual seed content. `analysis/tags` exists and has generated working payloads.
- Step 4: Sub-scope builder.
  - `build_docs.py --scope <scope> --sub-scope <sub-scope>` writes `manifest.json` plus `by-id/*.json`.
  - Parent builds exclude configured sub-scope source roots.
  - Watcher routes configured sub-scope changes to the sub-scope build path.
- Step 5: Publish flow.
  - Parent publish preview/apply now includes configured sub-scope `manifest.json` and `by-id` payloads.
  - Real `analysis` publish preview shows 4 tag payload changes plus `analysis/references/index.json`.
...
- `docs-viewer/source/studio/site-request-docs-viewer-embedded-detail-documents.md`

Generated/dirty context:
- `docs-viewer/generated/docs/studio/by-id/site-request-docs-viewer-embedded-detail-documents.json` was regenerated by watcher after request-doc edits.
- `docs-viewer/generated/docs/analysis/tags/` exists from watcher/sub-scope build.
- User ran real publish UI action for `analysis`.
- `site/assets/data/docs/scopes/analysis/tags/` now present.

Verification run:
- `py_compile` on changed Python files.
- `pytest docs-viewer/tests/python/test_build_docs_python.py` → 16 passed.
- `pytest docs-viewer/tests/python/test_build_search_python.py -k sub_scope` → passed.
- `pytest docs-viewer/tests/python/test_docs_publish_gate.py` → 5 passed.
- `pytest docs-viewer/tests/python/test_docs_management_service.py` → 41 passed.
- `git diff --check` clean.

Next step:
- Step 6: Parent Report Page.
  - Create indexed parent page in `docs-viewer/source/analysis/`.
  - Frontmatter should include:
    - `viewer_report: docs_subscope`
    - `viewer_report_access: public`
    - `viewer_report_subscope: tags`
  - Confirm parent page remains selected in list/detail URLs.
  - Then continue Step 7/8 for public report promotion and generic `docs_subscope` report runtime.
