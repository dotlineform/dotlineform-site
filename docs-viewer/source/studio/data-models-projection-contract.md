...
| --- | --- | --- | --- | --- |
| Catalogue metadata | `studio/data/canonical/catalogue/works.json`, `series.json`, `work_details/<work_id>.json` | fixed public route shells; `site/assets/data/series_index.json`, `site/assets/data/works_index.json`, `site/assets/data/recent_index.json`, `site/assets/series/index/`, `site/assets/works/index/` | retained generated lookup files, catalogue editor API read responses, project-state and thumbnail-quality local reports | [Catalogue Source Model](/docs/?scope=studio&doc=data-models-catalogue-source), [Catalogue Indexes And Payloads](/docs/?scope=studio&doc=data-models-catalogue-indexes) |
...
| Catalogue media metadata | source image fields and confirmed `media_version` values in canonical catalogue JSON plus `DOTLINEFORM_PROJECTS_BASE_DIR` media roots | public route thumbnails under `site/assets/works/img/`, `site/assets/work_details/img/`, `site/assets/moments/img/`; work and nested detail versions in `site/assets/works/index/<work_id>.json`; versioned R2 primary URLs on work/detail routes | external derivative staging under `$DOTLINEFORM_PROJECTS_BASE_DIR/catalogue/media/`; focused work editor reads include the work version | [Scoped JSON Catalogue Build](/docs/?scope=studio&doc=scripts-build-catalogue-json), [Publish Media To R2](/docs/?scope=studio&doc=scripts-publish-media-to-r2) |
...
`media_version` is a deliberate public per-work projection, not a source-path field. It must remain out of aggregate catalogue indexes; those consumers render committed thumbnails rather than independently cached R2 primary images.
...
| --- | --- | --- | --- | --- |
| Studio docs | `docs-viewer/source/studio/*.md` | none by default | `docs-viewer/generated/docs/studio/`, `docs-viewer/generated/search/studio/index.json`, `<DOCS_VIEWER_BASE_URL>/docs/` manage payload reads | [Studio Overview](/docs/?scope=studio&doc=studio-overview), [Docs Viewer Builder](/docs/?scope=studio&doc=scripts-docs-builder) |
...
| Studio config and route UI code | `studio/app/frontend/config/studio-config.json`, Studio frontend route modules, related checked-in config files | public Docs Viewer config and public search policy only where separately generated or copied by their owners | local runtime config JSON and code-owned Studio route copy | [Studio Runtime](/docs/?scope=studio&doc=studio-runtime) |
