...
| Catalogue media metadata | source image fields in canonical catalogue JSON plus `DOTLINEFORM_PROJECTS_BASE_DIR` media roots | public route thumbnails under `site/assets/works/img/`, `site/assets/work_details/img/`, `site/assets/moments/img/`; public page media URLs from generated payloads | external derivative staging under `$DOTLINEFORM_PROJECTS_BASE_DIR/catalogue/media/` | [Scoped JSON Catalogue Build](/docs/?scope=studio&doc=scripts-build-catalogue-json), [Publish Media To R2](/docs/?scope=studio&doc=scripts-publish-media-to-r2) |
