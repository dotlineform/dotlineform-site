...
- `studio/services/catalogue/catalogue_prose_import.py` owns staged moment prose import target normalization, Markdown validation, preview payloads, and draft moment source import application helpers.
...
- current JSON source-write endpoints write only canonical catalogue source JSON under `studio/data/canonical/catalogue/`, including `works.json`, `work_details/<work_id>.json`, and `series.json`
