...
- Completed task 4 Analytics tag source path contract:
  - added `analytics-app/app/server/analytics_app/tag_services/tag_source_paths.py` as the Analytics-owned source path contract
  - retargeted `tag_source_model.py` and `analytics_api.py` to use the contract instead of local tag source constants
  - retargeted catalogue cleanup, catalogue generation, and site consistency audit Python consumers to import the Analytics path contract
  - updated focused catalogue cleanup tests to use the contract for tag assignment paths
  - left JSON files in `studio/data/canonical/analytics/` until task 6 and left catalogue search/projection direct paths for task 5/task 6
  - verified focused pytest and the `quick` profile

### steer for next task

- Start task 5 by removing direct tag reads from Studio-owned catalogue search.
- Public search does not need tag terms, so remove tag path defaults and flags unless a concrete Analytics-specific search surface is identified first.
- Keep task 5 scoped to search ownership; do not move the tag JSON files until task 6.
- After search no longer reads tags directly, task 6 can move canonical tag JSON by changing the Analytics path contract and affected config/tests/docs.
