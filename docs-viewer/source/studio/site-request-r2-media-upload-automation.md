...
- the Docs Viewer media follow-up is implemented across public R2, repo-backed local, and external-local scopes
...
The same credential and upload architecture now supports Docs Viewer images and files without a second security model.

The upload, Work editor integration, cache-safe catalogue URL-version boundary, and Docs Viewer scope-aware media workflow are implemented.
...
- publish scope-owned Docs Viewer media through the same server-owned R2 boundary

## Original Catalogue-Milestone Non-Goals

The original catalogue milestone did not:
...
That separation matters because catalogue primary images and Docs assets have different source roots, but share credentials, safety checks, and upload behavior.
...
It has since become a Docs Viewer scope, so Moments media belongs with the implemented docs-owned asset workflow rather than the catalogue R2 adapter.
...
1. `external_assets` is validated as the only external-local mode, and repo/R2 destinations are rejected there
2. `/docs/media/<scope>/<class>/<filename>` serves confined repo-backed and external-local media through the local service
3. `r2_upload` publishes a complete import record's media set under stable scope-owned keys before its Markdown source is committed
4. `--scope docs --docs-scope <scope> --kind img|files --staged-filename <filename>` publishes one manually staged public Docs asset
5. cache versioning, automatic remote deletion, rollback, and migration automation remain intentionally out of scope
...
- `--kind img|files` with `--scope docs`
- `--id <id>`
- `--docs-scope <scope>`
...
- use concise catalogue logs that show ids, local relative paths, object keys, and statuses
- keep Docs reports narrower by omitting absolute paths, object keys, checksums, ETags, and credentials
...
- docs media handling and image guides for the enabled scope-aware workflow
...
The Docs Viewer media follow-up is complete:

- public Library, Analysis, and Moments scopes use `r2_upload` with deterministic `docs/<scope>/<class>/<filename>` keys
- one import record is fully preflighted, and its source link is committed only after every required upload succeeds or is confirmed unchanged
- remote comparison conflicts block the import record; ordinary Docs Import never treats document overwrite confirmation as media overwrite authority
- R2 credentials and remote object details remain server-side, while browser/result payloads expose only safe scope, class, filename, size, and status fields
- repo-backed local scopes store imported media under their docs source `media/` tree and use the confined local media route
- external-local scopes store media under `$DOTLINEFORM_PROJECTS_BASE_DIR/docs-viewer/media/<scope>/...` and cannot select R2 or repo-public destinations
- the local route rejects traversal/symlink escape and does not serve interactive HTML through the ordinary media boundary
- the CLI supports a dry-run-first exact public Docs scope/class/filename upload from the shared import-staging drop-zone
- Docs media cache versioning, automatic deletion, and an asset registry remain deferred
...
- keeps one secure credential boundary across catalogue and Docs publishing
- gives Docs image publishing an exact shared runner instead of a separate ad hoc script
...
- Docs R2 replacements can remain edge-cached until expiry; immediate replacement still requires a new filename and source-token update
