...
- `data-sharing/services/dispatch.py` owns handler selection and shared operation dispatch for `prepare`, `list_returned`, `review`, and `apply`.
- `data-sharing/workflows/prepare.py`, `list_returned.py`, `review.py`, and `apply.py` expose the headless workflow entry points.
...
- `data-sharing/adapters/documents/adapter.py`
...
Document workflow roots:

- outbound packages: `var/analytics/data-sharing/exports/`
- returned package staging: `var/analytics/data-sharing/import-staging/`
- review artifacts: `var/analytics/data-sharing/import-preview/`
- source root: `docs-viewer/source/library/`
The adapter registry validates the first three roots against the shared Data Sharing path contract; adapters should not add fallback reads for old disposable `var/studio/data-sharing/...` or `var/studio/export-import/...` packages.
...
The tags adapter implementation lives at `data-sharing/adapters/tags/adapter.py` and is resolved through the same Data Sharing registry and workflow dispatcher as the documents adapter.
...
- documents adapter code lives at `data-sharing/adapters/documents/adapter.py`
- Analytics tags adapter code lives at `data-sharing/adapters/tags/adapter.py`
- shared operation dispatch lives at `data-sharing/services/dispatch.py`
- workflow entry points live under `data-sharing/workflows/`
