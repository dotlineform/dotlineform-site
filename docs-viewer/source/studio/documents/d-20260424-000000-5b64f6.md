...
It watches configured document roots only. It does not watch published outputs under:
...
- because the watcher rebuilds from document-source changes only, published output writes do not loop back into new watcher-triggered rebuilds
