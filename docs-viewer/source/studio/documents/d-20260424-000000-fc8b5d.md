...
open Import -> choose target scope and source
    |
    +-- Documents ------------> convert and validate
    |                              |
...
Ordinary document sources must be direct children. A Markdown package is a direct-child folder containing one Markdown file plus its local images or attachments. The picker shows only document formats the server currently accepts; standalone images and downloads belong to the source editor's **Add image…** and **Add file…** actions. [Docs Import Architecture](/docs/?scope=studio&doc=d-20260514-164722-3de65e) points to the code-owned format mapping.
