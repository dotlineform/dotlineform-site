...
Public routes declare whether Recent means `added` or `edited`. The builder prepares that public projection independently and applies public visibility before its limit; Publish copies the prepared projection rather than trimming the working management list. Public Moments currently uses `added`, while Library and Analysis use `edited`.
...
[Scope Lifecycle](/docs/?scope=studio&doc=d-20260515-000000-f43740) creates a public scope by writing its conventional source/config records, rendering a tracked static route shell, adding public and management route records, building working output, and copying the initial public snapshots.
