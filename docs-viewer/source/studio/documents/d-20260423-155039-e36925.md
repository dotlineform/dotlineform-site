...
Place the file beneath the scope source root at `media/img/` or `media/files/`. This is `docs-viewer/source/<scope>/media/` for repo-backed scopes and `$DOTLINEFORM_PROJECTS_BASE_DIR/docs-viewer/source/<scope>/media/` for external-local scopes. The local service resolves the URL through scope configuration and rejects traversal and symlink escapes.
