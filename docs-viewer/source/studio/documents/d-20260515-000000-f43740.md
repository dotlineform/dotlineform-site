...
Preview allocates the first document's immutable identity and shows its source path. Confirmed apply carries that planned identity back to the server, writes the default source document, appends config and manifest records, creates only mode-appropriate route/output paths, and rebuilds the new scope. Local scopes immediately receive materialized `media/`, `media/img/`, and `media/files/` directories even when empty. This uses the same allocator for repo-owned and external-local scopes; the readable scope id is not reused as a document id. A public create also renders the static route shell, updates the public route registry, and copies the initial generated snapshots.

### Rename

Rename is intentionally limited to lifecycle-created external-local scopes. It moves the derived source root, including its `media/` directory, plus the generated-docs and generated-search roots; updates scope config, manifest identity, UI-status keys, and sub-scope paths; then rebuilds under the new id.
...
| --- | --- |
| Repo-backed source root | Created and delete-owned as a directory. Configured `media/{img,files}` directories are kept materialized and are removed with that source root. |
| External-local source root | Created, renamed, and delete-owned by its recorded derived path. Configured `media/{img,files}` directories stay materialized and follow the same lifecycle. |
| External-local generated roots | Created, renamed, and delete-owned by their recorded derived paths. |
