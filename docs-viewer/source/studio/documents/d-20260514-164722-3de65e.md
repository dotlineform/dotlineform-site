...
Ordinary document-source imports produce one replacement record after preview. Collection adapters may produce any of the three intents. This prevents collection planning from inventing replacement content for hierarchy-only records and lets future trusted wrappers reuse document planning without reusing Data Sharing provenance.
...
4. Compare the proposed source identifier with current target-scope state for any explicit overwrite decision.
5. Convert the accepted preview to `ImportContent`; allocate an immutable local identity for create or preserve the canonical target identity for overwrite.
6. Plan the document, materialize media, atomically write source, and invoke the managed rebuild boundary.

The format converter never writes source or performs a rebuild. The Word adapter passes Mammoth's semantic HTML unchanged into the shared HTML converter; temporary HTML and base64 image bodies remain server-side while the public preview contains canonical Markdown and safe media-plan metadata. Filename replacement, overwrite confirmation, current target state, and endpoint authorization stay outside the converter.
