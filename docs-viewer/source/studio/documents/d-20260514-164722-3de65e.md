...
Register the media type's provider, confined path, served prefix, and required capabilities, then extend the focused location adapter only if the provider itself is new. Do not add publication policy to `SourceImporter` or a format converter.
...
- Media publication spans scope config, artifact-location adapters, and focused media services. Looking only at the source registry gives an incomplete picture of import behavior.
