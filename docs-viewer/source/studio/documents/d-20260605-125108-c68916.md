...
| Recent | route-specific published `recent.json` | working `recent.json` | Each payload declares an `added` or `edited` basis; do not add date fields to tree rows to support it. |
...
Every route that enables Recent declares an `added` or `edited` basis. `/docs/` uses edited working projections for every selected scope. Public Library and Analysis use edited published projections; public Moments uses an added published projection. The same source scope can therefore have different working and published projections without a UI mode choice.
