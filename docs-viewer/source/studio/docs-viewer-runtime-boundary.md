...
[Docs Review](/docs/?scope=studio&doc=docs-viewer-review) owns the package workflow and its Data Sharing boundary.
...
[Configuration And Extension Points](/docs/?scope=studio&doc=config-docs-viewer) maps the config, code registry, and capability gates in detail.
...
Views should attach through the code registry, explicit hosted-view context, and panel lifecycle rather than reading broad runtime state or modifying route-shell markup. [Panel Hosts](/docs/?scope=studio&doc=docs-viewer-panel-hosts) owns that focused lifecycle.
...
Use [Runtime Module Ownership](/docs/?scope=studio&doc=docs-viewer-runtime-module-ownership) when an implementation change needs the current grouped module map. Use [Generated Data Contracts](/docs/?scope=studio&doc=docs-viewer-generated-data-contracts) for payload and read authority.
