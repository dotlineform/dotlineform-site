...
- `docs-viewer/runtime/js/docs-viewer-app-shell.js` for public-safe JavaScript-owned shell composition before the entry controller wires route behavior; manage-only shell renderers are supplied by the manage entrypoint rather than imported by the shared shell
- `docs-viewer/runtime/js/docs-viewer-management-shell-composition.js` for the manage-owned renderer bundle that composes management actions, selected-document actions, and management shell markup into the shared app shell during manage boot
- `docs-viewer/runtime/js/docs-viewer-management-shell-renderer.js` for management-only context menu, metadata modal, import modal, settings modal, and import host refs rendered only through manage-owned shell composition
...
- `docs-viewer/runtime/js/docs-viewer-management-hosted-views.js` for manage-owned hosted-view records such as the `markdown-source` source-editor view; public entrypoints do not import it
...
- `docs-viewer/runtime/js/docs-viewer-hosted-views.js` for minimal hosted-view registration, panel-specific listing, access/availability checks, public-safe built-in hosted-view records, and graceful absence
...
- `docs-viewer/runtime/js/docs-viewer-hosted-views.js` owns the minimal hosted-view record shape, lifecycle method defaults, panel-specific listing, access/availability checks, public-safe built-in default hosted-view records through `createDocsViewerDefaultHostedViews()`, and route-config hosted-view filtering through `createDocsViewerRouteHostedViews(...)`.
- `docs-viewer/runtime/js/docs-viewer-management-hosted-views.js` owns manage-only hosted-view records supplied by the manage entrypoint, currently the `markdown-source` source-editor view.
- `docs-viewer/runtime/js/docs-viewer-management-shell-composition.js` owns manage-only shell renderer import composition supplied by the manage entrypoint. Public-safe shared shell code must not import management action, document-action, or shell renderers directly.
...
- `docs-viewer/runtime/js/docs-viewer-route-config.js` fetches that registry and resolves the current `docs_viewer_route_config_v1` snake_case record from a `docs_viewer_route_config_registry_v1` `routes` array; route config resolution no longer reads inline config scripts, legacy `#docsViewerRoot` data attributes, camelCase field aliases, or object-map route registries
