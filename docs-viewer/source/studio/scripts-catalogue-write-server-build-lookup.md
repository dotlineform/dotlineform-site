...
`POST /catalogue/build-preview`, `POST /catalogue/build-apply`, and the shared publication apply path use the scoped JSON catalogue build helper for work, work-detail save/build, and moment media tasks.
...
`POST /catalogue/build-apply` also accepts `media_only: true` for work and moment image refreshes. In media-only mode the helper stages the configured source image, regenerates local thumbnails and staged primary variants, and then stops before page/json generation and catalogue search. Studio sends `force: true` for the remaining work and moment readiness-panel refresh actions so the derivatives are regenerated even when current output timestamps already look fresh.
...
The retired standalone work-detail create/save endpoints are removed:

- `POST /catalogue/work-detail/create`
- `POST /catalogue/work-detail/save`

Future detail editing should use work-scoped section/detail APIs rather than restoring those endpoints.

Bulk work-detail saves now update detail-owned fields only, such as `title` and `project_filename`. Section metadata writes must target section records in the future work-scoped API.

Bulk workbook imports in `work_details` mode create or reuse section records in `work_detail_sections` and write detail-owned fields into `work_details`. Rows whose parent work is still draft are blocked in preview with `parent_work_unpublished`. Detail import rows use `details_subfolder` and `section_title` to create or resolve section records; non-empty legacy `project_subfolder` cells are blocked in preview.
