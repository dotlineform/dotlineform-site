...
- Data Sharing document metadata reads are not generated-data runtime reads; they use a Docs Viewer-owned source metadata helper over scope config and source Markdown.
...
Data Sharing needs for richer document data are covered by `docs_data_sharing.source_metadata`.
That helper reads configured docs source and source-rendered content for Data Sharing workflows.
