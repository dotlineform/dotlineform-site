...
- `docs-viewer/services/docs_data_sharing/source_metadata.py` owns the source-derived Data Sharing docs metadata helper.
- `docs-viewer/services/docs_data_sharing/package.py` reads selectable document records from the source metadata helper.
- `docs-viewer/services/docs_export.py` reads source metadata records and source-rendered content for selection, hierarchy expansion, field mapping, headings, and source-text extraction.
- `docs-viewer/services/docs_import.py` reads source metadata records for returned-package current-doc, viewability, parent, renderability, and current-summary review context.
- Focused tests and adapter fixtures now use source docs and scope config for Data Sharing metadata behavior and guard against generated publication artifact coupling.
...
- close this request after executable checks prove Data Sharing works without public flat docs indexes or generated publication payloads

Completed closeout:

- Durable Data Sharing, adapter config, Docs Viewer source/generated-data, data-model, export-script, and export-config docs describe source metadata as the document Data Sharing metadata source.
- `data-sharing/config/adapters.json` no longer declares generated docs index or by-id roots as document sources.
- Active service/config stale-path scans returned no matches for generated docs index/by-id dependency terms.
- No explicit generated docs write command was run for this request; the local docs watcher regenerated Studio docs/search payloads after source docs edits.
