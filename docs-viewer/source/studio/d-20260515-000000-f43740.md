...
Preview allocates the first document's immutable identity and shows its source path. Confirmed apply carries that planned identity back to the server, writes the default source document, appends config and manifest records, creates only mode-appropriate route/output paths, and rebuilds the new scope. This uses the same allocator for repo-owned and external-local scopes; the readable scope id is not reused as a document id. A public create also renders the static route shell, updates the public route registry, and copies the initial generated snapshots.
