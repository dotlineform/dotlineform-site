...
- Verification for this continuation through `IT-5`: `git diff --check` passed, the document/sidebar/info-panel/config broad-state scans returned no matches, focused lazy management and built-in hosted-view smokes were retargeted to current contracts, and `$HOME/miniconda3/bin/python3 docs-viewer/tests/smoke/docs_viewer_app_shell_modules.py --site-root .` passed.
...
- Continue with route-config fallback, test-retargeting, report/local service, generated-data helper, import-boundary, and durable-doc slices (`IT-6` through `IT-11`) now that the route/search/bookmark/management/hosted-view/document/sidebar/info-panel/config state and command boundary slices (`IT-1`, `IT-2`, `IT-3`, `IT-4`, `IT-5`, `IT-12`, `IT-13`, `IT-14`) are complete.
