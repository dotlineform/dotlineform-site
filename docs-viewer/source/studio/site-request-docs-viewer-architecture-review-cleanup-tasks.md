...
| Focused app-shell smoke tests still assert compatibility surfaces | Resolved 2026-05-28 by `IT-7` for app-session/composition aliases: focused app-shell smoke coverage now asserts state-domain projections, composition public/manage contracts, and the small returned app handle instead of `compatibilityBridge.state` or `composition.state`. | Remaining compatibility-named hosted-view alias coverage is tracked separately by `IT-5`/`IT-11`; broad state removal remains tied to future controller-family narrowing rather than test convenience. | IT-7 done |
| Durable docs still describe compatibility surfaces | `docs-viewer/source/studio/docs-viewer-runtime-boundary.md`, `docs-viewer/source/studio/docs-viewer-overview.md`, and `docs-viewer/source/studio/docs-viewer-javascript-inventory.md` still describe the compatibility runtime, private callback bridges, and built-in compatibility hosted-view alias. Route-config fallback and app-session/composition alias wording was updated by `IT-6` and `IT-7`. | Documentation is doing useful warning work, but compatibility wording should not remain the final architecture description after classification. Remaining compatibility wording needs a current owner or a removal/narrowing task. | Task 8 and IT-11 |
...
| 8 | planned | Review durable Docs Viewer docs for compatibility fields described as current public API. If wording indicates temporary bridges they should be considered migration debt and therefore have tracked cleanup tasks in this document. |
