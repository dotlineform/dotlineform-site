...
- `IT-3` remains the next implementation slice. Route workflow is still the private owner for URL/history/index/payload commands, but the broader route-workflow command contract for management reloads and route state domains has not yet been narrowed.
