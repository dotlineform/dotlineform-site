...
- Verification for this continuation: `git diff --check` passed, the document/sidebar broad-state scan returned no matches, and `$HOME/miniconda3/bin/python3 docs-viewer/tests/smoke/docs_viewer_app_shell_modules.py --site-root .` passed.
...
- Continue with info-panel and config/scope state narrowing (`IT-13`, `IT-14`) now that the route/search/bookmark/document/sidebar state and command boundary slices (`IT-1`, `IT-2`, `IT-3`, `IT-12`) are complete.
