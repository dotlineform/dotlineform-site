...
| Management controller | `docs-viewer-management.js` and child modules | Explicit named-domain consumption, capability/status/control projection, focused-controller composition, one-document action-context handoff, and write orchestration. |
| Action targeting | `docs-viewer-action-definitions.js` | Code-owned action ids, scope/active-document/selection target definitions, primary/all/exactly-one policies, the behavior-preserving one-document context adapter, and pure target resolution. It owns no placement, handlers, workflow state, capability checks, transport, or mutation behavior. |
| Management actions | `docs-viewer-management-actions.js` | Command-specific UI/service workflows using resolved target document ids supplied by the management coordinator. |
...
| Management interactions | `docs-viewer-management-interactions.js` | Manage-only navigation/edit/context-menu/drag event routing and canonical context-menu action-id handoff. It does not own action target policy or multiple-selection state. |
...
| Metadata workflow | `docs-viewer-management-metadata-workflow.js` | Metadata parent-option projection, form validation and payload shaping, explicit document-id modal handoff with active-document fallback, and save-result delegation to the action controller. |
