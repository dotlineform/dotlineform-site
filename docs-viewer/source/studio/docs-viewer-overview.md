...
- validated Docs Review publication uses `docs_review_materialization.py` to build derived source and generated JSON beside the final path, then expose the complete timestamped package with one rename; ordinary review navigation reads that retained JSON, while package-local repair runs only when generated index or payload files are missing or malformed
