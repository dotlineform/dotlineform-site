...
Browser-safe route, display, and search settings live in `docs-viewer/config/defaults/docs-viewer-config.json`.
...
`docs-viewer/config/defaults/docs-viewer-config.json` is generated by `./scripts/build_docs.rb --write`.
...
After changing `docs-viewer/config/scopes/docs_scopes.json`, rerun the docs build for the affected scope or scopes so this generated config stays current.
