...
| STSR-003 | done | Inventory every pre-move Studio-related path and classify it as Studio source, public Jekyll source, public generated output, future Docs Viewer extraction candidate, Studio generated/local working output, mixed ownership, or out-of-scope dependency; include previous path families such as `scripts/`, `assets/studio/`, `assets/docs-viewer/`, `assets/ui-catalogue/`, `_includes/`, `docs-viewer/source/studio/`, `docs-viewer/source/analysis/`, `docs-viewer/source/library/`, `_docs_catalogue/`, `_ui_catalogue_notes/`, `studio/`, tests, checks, local runner files, and generated-output-adjacent data. Inventory recorded in [Studio Source Tree Reorganization Inventory](/docs/?scope=studio&mode=manage&doc=site-request-studio-source-tree-reorganization-inventory). |
