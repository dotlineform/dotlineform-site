...
| ID | status | action |
| --- | --- | --- |
| 1 | planned | Audit current public-scope docs/search output paths, public route data URLs, cross-scope link behavior, broken-links report reads, non-public by-id consumers, manage generated-read behavior, builder tests, watcher behavior, and public route smokes. |
| 2 | planned | Extend docs scope config and config loading to support separate working generated and public publish paths for public scopes. |
| 3 | planned | Change public-scope docs and search builders to write working generated output under `docs-viewer/generated/` while preserving public route URL generation. |
| 4 | planned | Update Docs Viewer scope lifecycle create/delete preview and apply flows so public scopes manage working generated paths and published snapshot paths separately. |
| 5 | planned | Update manage-mode generated reads so public scopes preview working generated docs/search payloads rather than published snapshots. |
| 6 | planned | Implement a publish confirmation/apply service that safely syncs working generated docs/search output to public snapshot roots. |
| 7 | planned | Add Docs management endpoints for publish status, confirmation, and apply. |
| 8 | planned | Add a manage-mode `Publish docs` UI command for public scopes with confirmation and apply. This can be a menu item `🌍 Publish` in the existing Actions button. |
| 9 | planned | Update the live rebuild watcher and docs-management write follow-through so public-scope edits rebuild working generated output without publishing. |
| 10 | planned | Update the Admin Checks target map for the new publish-gate files, routes, and shared dependencies. Keep existing route ids and families if accurate; add a new `publishing` area only if `docs-build`, `management`, and `config` do not describe the workflow clearly. |
| 11 | planned | Add focused tests for config validation, builder output routing, cross-scope link rewriting, broken-links report generated-root reads, scope lifecycle path planning, publish diff/apply behavior, stale published file removal, and unsafe path rejection. |
| 12 | planned | Add focused route smokes proving public routes read only published snapshots, public cross-scope links stay on public routes, and manage mode can see working generated changes. |
| 13 | planned | Update durable Docs Viewer docs and Admin Checks docs for generated-data ownership, builder commands, live watcher behavior, scope lifecycle behavior, public scopes, and target-map coverage. |
| 14 | planned | Run final verification and close the request. |
