...
scope config
    ...
    |       +-- materialized local media/{img,files} where applicable
    |
...
| Local committed | Repo `docs-viewer/source/<scope>/`. | Repo `docs-viewer/generated/`. | `<source root>/media/`. | None. |
| External local | Derived external workspace source root. | Derived external docs/search roots. | `<source root>/media/`. | None. |
...
Ordinary local media is managed with its scope even though the Markdown remains canonical content authority:

- repo-backed and external-local media lives beneath the scope source root at `media/`
- configured local scopes keep `media/`, `media/img/`, and `media/files/` present even when empty
- scope rename and delete move or remove that media with the source root
...
- External Asset Collections are independently owned roots outside `source/<scope>/`; Docs Viewer may inventory them but scope lifecycle does not own them

[Media And Asset Handling](/docs/?scope=studio&doc=d-20260514-184303-7914e2) owns storage, link resolution, and materialization. [External Asset Collections](/docs/?scope=studio&doc=d-20260714-194217-b2bdd7) owns the separate inventory-only boundary.
...
- Canonical source, generated working data, and public snapshots can temporarily disagree until rebuild or Publish completes.
