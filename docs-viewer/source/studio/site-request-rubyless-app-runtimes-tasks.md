...
- Updated check-profile and fixture command expectations for Python app builders. The `docs` run-check profile now invokes `docs-viewer/build/build_docs.py` and `docs-viewer/build/build_search.py`, and the named Docs Viewer/catalogue command-shape tests pass against Python builder expectations.
...
- Removed the retired Ruby Docs Viewer and catalogue search builder entrypoints and updated active command examples to the Python search builders.
- Checked for task-20 leftovers that need explicit cleanup rows. No temporary bridge, stale active command path, retired test, or mixed-runtime switch needed a new task; remaining retired-name scan hits are migration-history text or negative guardrail assertions.
- Updated command/runtime docs so current app-generation paths describe Python Docs Viewer docs/search builders, Python catalogue search, Python catalogue prose rendering, docs watcher behavior, and Ruby/Bundler/Jekyll as public-site preview/build only.
- Updated dependency docs for `markdown-it-py`, the built-in table rule, no external renderer plugins, raw-HTML and sanitization boundaries, catalogue prose usage, and the no-Jekyll/Kramdown-parity fixture policy.
- Updated script inventory and risk docs to record zero Ruby files in the active app script inventory roots, Python ownership for Docs Viewer and catalogue generated builders, remaining Ruby/Bundler/Jekyll as public-site preview/build only, and current risk evidence counts.
- Audited active app runtime surfaces for Ruby/Bundler references. The only command hits are `bin/public-site-preview` and `bin/public-site-build`; app/service hits are backup-bundle naming or negative guardrail tests, not Ruby runtime dependencies.
- Verified no mixed Ruby/Python builder mode remains in active app generation. Docs rebuild orchestration hardcodes `docs-viewer/build/build_docs.py` and `docs-viewer/build/build_search.py`; catalogue build orchestration hardcodes `studio/services/catalogue/search/build_search.py`; fallback branches are full-scope Python rebuild fallbacks, not Ruby fallbacks.
...
| 18 | done | Update check profiles and tests that assert Ruby command shapes. Move `docs-viewer/tests/python/test_docs_write_rebuild.py`, `test_docs_live_rebuild_watcher.py`, `test_docs_import_service.py`, `docs_viewer_management_workflows.py`, and `studio/tests/python/test_catalogue_build_commands.py` to Python command/helper expectations. |
...
| 20 | done | Remove obsolete compatibility scaffolding and mixed-runtime knobs. Delete or retire replaced Ruby builder dispatch options, Bundler override options used only by app builders, stale command examples in active app docs, and tests that exist only to preserve old Ruby command shapes. |
| 21 | done | Add explicit cleanup tasks for any leftovers found during implementation. If a temporary bridge, stale generated path, obsolete helper, old docs reference, or retired test cannot be removed immediately, add a new row to this table before closeout with a concrete owner and removal condition. |
| 22 | done | Update command/runtime docs. Describe Python Docs Viewer generation, Python docs search generation, Python catalogue search generation, Python catalogue prose rendering, app servers, docs watcher behavior, and live rebuilds; remove active-app examples for disabled startup rebuilds and Ruby app builders. |
| 23 | done | Update dependency docs. Describe `markdown-it-py`, enabled renderer plugins, sanitizer boundaries, and the no-Jekyll-parity fixture policy. |
| 24 | done | Update script inventories and risk docs. Record app-facing Ruby removal, remaining public-preview-only Ruby usage, changed ownership for generated builders, and any risk-evidence implications. |

### Phase 5: Audits And Verification

| ID | status | action |
| --- | --- | --- |
| 25 | done | Audit app runtime Ruby references after migration. Use targeted source scans to confirm Studio, Docs Viewer management, Analytics, Data Sharing, catalogue build/search/prose generation, and UI Catalogue no longer depend on Ruby scripts or Bundler. Include at least these patterns: `bundle`, `exec ruby`, `build_docs.rb`, `build_search.rb`, `render_markdown_with_jekyll`, `jekyll_markdown_renderer`, `DOCS_STARTUP_REBUILD_SCOPES`, and `CATALOGUE_STARTUP_LOOKUP_REBUILD`. |
| 26 | done | Verify no mixed builder mode remains. Scan for runtime switches, env flags, fallback branches, compatibility aliases, dual-write paths, or command dispatch that can choose between Ruby and Python app builders. Fail closeout if such a switch remains outside public-preview-only tooling. |
| 27 | done | Verify Studio is JavaScript + Python only. `/studio/`, route shells, runtime config, catalogue editors, audits, activity, Docs links, and APIs should run from Python server + JS frontend + Python builders. |
| 28 | done | Verify Analytics is JavaScript + Python only. Analytics routes, tag workflows, documents Data Sharing apply, runtime config, generated app data, and APIs should not depend on Ruby/Jekyll. |
...
| 33 | done | Final closeout. Confirm there are no unresolved compatibility layers or implicit cleanup tasks, record remaining Ruby usage as public-preview-only or deferred public-site renderer work, update this tracker statuses, and update the parent request if scope/risks changed. |
