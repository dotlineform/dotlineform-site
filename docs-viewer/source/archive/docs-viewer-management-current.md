...
- create, reparent, delete, and searchable metadata edits rebuild docs payloads plus same-scope docs search; archive moves source files into the `archive` scope and rebuilds both the source scope and archive scope; same-parent drag/drop reorder rebuilds docs payloads without a search update
...
- `Archive` requires a configured `archive` scope; if that scope is missing, the action opens a compact notice modal saying `archive scope doesn't exist.`
