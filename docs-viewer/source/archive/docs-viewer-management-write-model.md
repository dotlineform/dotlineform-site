...
- selected docs move from their active source scope into the configured `archive` scope
- `published` does not change automatically
- the moved root doc has its `parent_id` cleared in the archive scope; descendant docs move with it so internal parent references stay valid
- archived docs remain generated under the archive scope; public visibility depends on each doc's own `hidden` value
- if the `archive` scope does not exist, the browser action shows a compact notice modal saying `archive scope doesn't exist.`
...
- special names such as `archive` are ordinary `doc_id` values; the Archive command targets the `archive` scope rather than a same-scope archive parent doc
...
- define the conventional Archive command target scope id, `archive`
...
A: the selected doc moves from the active source scope into the `archive` scope. Descendants move with it so their parent references stay valid. `published` remains true, so visibility is still controlled by each doc's own `hidden` or `viewable` metadata.
...
A: `archive` now matters as a configured Docs Viewer scope for the Archive command. If that scope does not exist, the browser shows a compact notice modal. `_draft` and `_dev` do not carry source-folder semantics in the flat source layout.

Should `archive` be treated as a protected system doc?  
A: no. `archive` can remain an ordinary doc id inside the archive scope, but the command target is the archive scope itself.
