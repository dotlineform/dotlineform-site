...
please proceed with slice 11 defined in the child doc [Public Catalogue Runtime Works Index Route Slice](/docs/?scope=studio&doc=site-request-public-catalogue-runtime-module-architecture-works-index-slice).
...
I’m replacing the command surface next: bin/site-validate validates site/, and bin/site-preview serves site/ directly after validation.


~

> so, why do the site pages have to be generated by a python script? can they not exist as real html pages that are developed in-situ then copied to `_public_site`?

They can exist as real HTML files. There is no hard requirement that every public page shell is generated from Python.

The reason the migration landed on Python-rendered route shells is mostly practical continuity:

- it replaced Jekyll/Liquid with one deterministic static build step
- it kept shared layout/header/footer/version/media config in one place
- it avoided duplicating the same shell markup across `library/index.html`, `analysis/index.html`, catalogue pages, etc.
- it lets the artifact audit know exactly what gets rendered and copied
- it gives GitHub Actions one command to build `_public_site`

But that is a choice, not a law.
...
Then the builder copies those files into `_public_site`, maybe with minimal token substitution or none at all. That would make pages easier to inspect and edit in situ.

The tradeoff is duplication and drift. If each page is “real HTML”, then shared shell changes either need:

- duplicated manual edits across files, or
- a lightweight include/templating/preprocessor system, which starts recreating part of what Jekyll did, or
- web components/client-side shell injection, which changes runtime behavior.

A pragmatic middle ground might be best:

- keep copied static HTML pages for simple/stable shells where direct editing helps
- keep Python rendering only where the page is mostly generated from config/data
- make Docs Viewer public routes data-driven without generating Python source
- keep the final `_public_site` artifact as the thing GitHub Pages deploys
