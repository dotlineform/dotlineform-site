...
last_updated: "2026-07-30 19:59:38"
...
The service resolves `configured_workspace_paths(repo_root).import_staging`, passes that explicit root through the workflow, and reports the marker-rooted drop-zone path without exposing user-specific absolute paths. `docs_import_content.py` defines the wrapper-neutral normalized record, and `docs_import_document_package_content.py` emits that record for import-supported returned packages. Persistent read-only review materialization consumes the same normalizer. The active reviewed collection contract is `document-content`; `document-full-source` is only a potential later Docs Viewer feature and is not enabled. Supported collection requests dispatch to the focused collection planning or apply owners.
...
Ownership: owns the collection apply allowlist, server-issued planned-action confirmation, refreshed-plan response, and confirmed package apply.
