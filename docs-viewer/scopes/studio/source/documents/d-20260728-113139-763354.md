...
summary: define collection identity, shared report ownership, manage-only contributions, selection lifecycle, and explicit function adoption for docs_subscope
ui_status: done
...
`docs_subscope` is the generic collection index for a configured one-level
...
A focused manage-owned controller contributes:

- a detail toolbar mounted only for a validated detail target;
- a list toolbar for New, Import, collection actions, and selection commands;
...
The shared report loads the configured manifest and accepts an optional
...
mount, state, refresh, and unmount events. The Manage contribution implements
Status/non-viewable title prefixes, New, ordinary Import, collection-local
selection, package preparation, and lazy validated-detail Delete. Empty
list/detail hosts are not mounted, and the detail host is offered only after
...
| ordinary Import | explicit collection | shipped create-only |
...
| returned-package Import | package/collection | excluded until separately complete |
...
The list toolbar's 📥 **Import** action consumes the mounted
`{ scope, sub_scope }` collection directly. It is independent of selection
mode and checked IDs.

The management host validates that collection and opens the existing lazy
Import modal with a fixed destination label. Child mode exposes ordinary Word,
HTML, Markdown, text, and Markdown-package sources only; it does not expose
returned packages, overwrite, or a parent-scope fallback.

Every request retains `sub_scope`. Success returns a fresh exact
`{ scope, sub_scope, doc_id }` target, runs the confined child rebuild, refreshes
only the mounted generated manifest, and may open that exact detail. Result
Source actions use the same committed target. Public reports have no Import
contribution or management-module reachability.
...
The shipped checked-ID adoption is export-only. Ordinary Import is a separate
selection-independent workflow. Returned-package Import remains excluded until
its package validation, canonical comparison, and destination contracts all
retain the complete sub-scope identity.
...
- A large flat collection uses report-owned filtering over its supplied list.
  [SSI-4](/docs/?scope=studio&doc=d-20260728-141505-277932) owns that shared
  behavior. Search belongs to a later overall-search adoption
