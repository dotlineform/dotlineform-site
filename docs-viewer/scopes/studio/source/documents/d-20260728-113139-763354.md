...
- a sibling checkbox gutter for each list row;
...
- A large flat collection may need report-owned search or filtering, but those
  ...
  accidentally.
- Duplicate titles are valid and may require a Docs-owned distinguishing label;
  external groups or registry metadata must not be copied into rows as a
  shortcut.
