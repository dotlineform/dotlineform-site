...
Absence of `report_customisation` in sub-scope config selects the default. The
shared engine always supplies title search, title-ascending initial order,
normal empty/error states, and list/detail navigation. Manage adds status and
viewability row treatment, the title/recency sort toggle, New, selection,
Prepare Package, validated-detail Copy Link, and validated-detail Delete.

An optional strict `report_customisation: {id, settings}` selects one known
registry entry. The builder may project only a namespaced `customisation`
root and per-row `customisation` data; the access-specific browser registry
resolves its module. Unknown, unavailable, or mismatched identities fail as
contained report errors and never degrade silently to the default.

Manage composes the default first and the selected customisation second in
separate hosts. No collection ID, custom field, label, or action may become a
condition in the engine or dispatcher. The current `analysis_tags`
customisation is Manage-only and adds ordered group controls/filtering; its
public route therefore remains the uncustomised reader.
