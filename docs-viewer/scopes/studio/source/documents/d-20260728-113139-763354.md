...
implements Status/non-viewable title prefixes only. Empty list/detail hosts are
not mounted, and the detail host is offered only after by-ID identity
validation. Delete, checkboxes, selection, and package workflows remain
unimplemented.
...
7. reconcile the local inventory and return to list state; and
