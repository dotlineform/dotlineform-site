...
SSI-1 now ships this boundary as `subscopeDocumentSource` plus an optional
`subscopeReportContribution`. The contribution receives separate leading-cell
and title-prefix row hosts, list/detail toolbar hosts, and collection-scoped
mount, state, refresh, and unmount events. The current manage contribution
implements Status/non-viewable title prefixes and lazy validated-detail Delete.
Empty list/detail hosts are not mounted, and the detail host is offered only
after by-ID identity validation. Checkboxes, selection, and package workflows
...
7. remove the exact row from the mounted inventory and return to list state,
...
- A large flat collection may need report-owned filtering over its supplied
  list. [SSI-4](/docs/?scope=studio&doc=d-20260728-141505-277932) owns that
  proposed local behavior. Search belongs to a later overall-search adoption
  and must not create a child-specific index or parallel search surface.
