...
See [Docs Viewer Public Route Shell Template](/docs/?scope=studio&doc=d-20260613-115624-3d377c).
...
```json
{
  "scope_id": "notes",
  "title": "Notes",
  "source_root": "docs-viewer/scopes/notes/source",
...
- validates the new scope id, title, publishing mode, source root, and published output locations
- allocates the immutable identity and second-resolution `added_date` for the first document without writing
...
Returned data includes the planned document identity, planned scope config, file changes, rebuild commands, and URLs.
...
- re-runs create-preview validation with the server-issued planned document identity
...
- rebuilds published docs/search output for the new scope
...
- validates that the new scope id and derived external target root are unused
- validates the existing external scope root against the lifecycle-owned path contract
- reports the scope-root move, config/manifest changes, rebuild commands, and resulting management URL
...
- atomically moves the enclosing external `scopes/<scope>/` root to the new scope folder name
- changes the scope id, marker-rooted scope path, media prefix, and matching UI-status key in `docs_scopes.json`
...
- removes each scope-specific local and public search directory implied by its manifest-owned search index, including an empty directory whose index is already missing
