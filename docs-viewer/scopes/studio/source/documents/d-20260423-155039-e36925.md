...
Place the file at the configured published location. Repo-backed local scopes normally use `docs-viewer/published/docs/<scope>/media/<type>/`; external-local scopes use `$DOTLINEFORM_PROJECTS_BASE_DIR/docs-viewer/published/docs/<scope>/media/<type>/`. The local service resolves the URL through scope configuration and rejects traversal and symlink escapes.
