...
The template exists so New Scope can create a new route shell without copying the installed `site/analysis/index.html` route.
...
An installed route may load an additional scope-owned stylesheet when its presentation cannot belong to the portable viewer contract. The generic New Scope template does not add scope CSS automatically.
