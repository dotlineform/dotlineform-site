---
doc_id: d-20260602-210246-394018
title: Semantic References Implementation
added_date: 2026-06-02 21:02:46
...
The proposed replacement feature is documented under
[Semantic Tokens](/docs/?scope=studio&doc=d-20260725-153656-516b61), including
its source-editor UI and Work-token delivery.

This implementation is a pilot. Its parser, renderer, generated lookup, and
local report provide useful evidence, but its file locations, generated
schemas, report shape, and public projection are not the target Semantic Tokens
contract. The former Info-panel picker was retired by Catalogue Token CT-P1;
the new Catalogue add modal temporarily adapts the same registry and lookup
until the later replacement and cleanup. The proposed replacement structure is
documented in
[Semantic Tokens Architecture](/docs/?scope=studio&doc=d-20260714-234030-434069).
...
- provide the temporary lookup/list adapter used by the Catalogue add modal
- provide the read-only [Semantic References](/docs/?scope=studio&doc=d-20260518-214029-eb6598) management report
...
- source-editor target-lookup availability
...
Manage-mode editor assistance uses one generated target lookup artifact:
...
- `href`: current workable target destination
...
The generator reads `docs-viewer/scopes/moments/published/documents/index-tree.json` and per-moment payloads under `docs-viewer/scopes/moments/published/documents/by-id/`.
...
- `catalogue-token-targets.js`: adapts the pilot registry and lookup to the
  accepted Catalogue target shape
- `catalogue-token-contract.js`: serializes the accepted P0 Catalogue token
- `catalogue-token-modal.js`: owns Catalogue search, selection, validation, and
  guarded insertion
- `catalogue-token-contribution.js`: contributes the manage-only source action
  without adding Catalogue branches to source-editor core

The former `semantic-token-editor.js` and `semantic-token-picker-view.js` pilot
modules are retired. The generic target list remains temporarily shared with
the Catalogue modal and is removed or renamed with the rest of the pilot wiring
at the accepted cleanup checkpoint.
...
This is a pilot-era placement carried from the former
`docs-viewer/generated/docs/<scope>/references/` tree when the wider documents
output moved into the scope-first `published/` layout. These relationship files
are reproducible local report inputs, not deliberately published data, so this
is not the accepted target location.

The public publish gate also creates filtered copies beneath
`site/assets/data/docs/scopes/<scope>/references/`. There is no current public
Semantic References report or other accepted public consumer. That projection
should not be treated as a publication contract.
...
The [Semantic References](/docs/?scope=studio&doc=d-20260518-214029-eb6598) report is a read-only Docs Viewer management report.
