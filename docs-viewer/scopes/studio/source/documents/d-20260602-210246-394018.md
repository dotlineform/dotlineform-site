...
```text
docs-viewer/published/semantic-references/target-lookup.json
```
...
The generator reads `docs-viewer/published/docs/moments/index-tree.json` and per-moment payloads under `docs-viewer/published/docs/moments/by-id/`.
...
That is the conventional local published root. Explicit public publication copies the scope-owned reference payloads into its configured public docs snapshot.
