...
The main panel represents rendered documents, search results, recent results, and the public/Manage `content-detail` presentation. Markdown source is a manage-only mode inside `rendered-document`. The document-view coordinator keeps active view/mode, main-view controls, and the default info view synchronized. A Content Detail request carries an immutable adapter-owned target from the current document mount; its `expanded-main` layout temporarily projects index and info presentation away while retaining their state, then Back restores the rendered document, panel projection, scroll, and invocation focus. Navigation releases the target without restoring stale document context.
