...
summary: Local package-rooted API contract for Docs Review capabilities, package reads, explicit Build, assets, retained generated payloads, and exact source opening.
...
Build revalidates the selected package and complete source set, then always runs
the synthetic local-external Docs Viewer builder for that package. It writes
only the selected package's `generated/` folder, even when the previous
generated set was healthy.

The response includes document and asset counts, warnings, diagnostics,
generated path, and summary text. The endpoint never rewrites package
`source/*.md`, advances package transport dates, accepts a configured
scope/sub-scope target, or mutates canonical source. Ordinary generated reads
retain their separate repair-if-missing-or-malformed behavior.

## `POST /docs-review/packages/open-source`
...
The command revalidates the package and complete source set, requires `doc_id` to identify an exact direct package source member, and opens only that `source/<doc_id>.md` file in Visual Studio Code. It accepts no filesystem path, editor choice, canonical target, selected-parent fallback, or confirmation payload.

This is an identity-only external-application handoff. It does not return source content, expose a browser editor, write source, or rebuild retained generated output.
...
The endpoint family contains no package source-read or source-write route.
`source/*.md` is persistent derived build input used by explicit Build and
conditional repair, and may be opened externally only through the confined
identity-based command; it is not a browser-editable service surface.
...
remains the managed `/docs/` import capability. Docs Review has no Import
control or configured-source apply authority. A user may separately stage the
complete rebuilt `source/` folder for managed edited-review-source Import; that
workflow does not add an endpoint here.

The browser enters Docs Review from **Open in Docs Review** on one exact
server-declared reviewable candidate in the app-level Import modal.
`POST /docs/packages/returned/review` owns that management-side
prepare-or-reuse handoff and returns the validated review URL; it is not part
of this independently gated review endpoint family.
