---
doc_id: d-20260713-133928-c786c7
title: Source Documentation and Linting Concept
added_date: 2026-07-13 13:39:28
...
- Use canonical repository vocabulary, ownership boundaries, exclusions, and semantic distinctions. Do not add competing synonyms merely to improve agent retrieval.
- Treat source documentation as a routing and explanation surface, not independent behavior authority. Verify current claims against code and canonical configuration, use tests and schemas as executable evidence, and update affected comments in the same change as their contract.
- Keep source comments local to the responsibility they can remain accurate about. Broader architecture, delivery intent, and historical decisions belong in their durable owners.
...
1. Choose and record a small JSDoc convention using representative modules from the active JavaScript application being adopted.
2. Configure ESLint for the application's real runtime globals, and configure any selected JSDoc consumer separately without broad file-level suppressions.
...
- visual or rendering code with important coordinate, timing, colour, randomness, or layout assumptions
...
Use ESLint with explicit application-owned configuration for browser modules, workers, server-side scripts if any, and tests. Represent each real runtime context rather than enabling generic globals across unrelated files.
...
## Configuration And Tooling Ownership

Shared documentation principles may become repository-wide, but tool configuration, dependency pins, and normal commands should follow the owning application's lifecycle.

- Do not assume one root JavaScript package or one repository-wide `jsconfig.json`.
- Keep generated, vendored, retired, and unrelated application sources outside a pilot's configured surface.
- Derive JavaScript language and module settings from the code's real delivery environment, and derive Python targets from the supported project interpreter.
- Prefer a focused application command and the existing check-profile owner over a new repository-wide mega-command.
- Keep formatter adoption, automatic-fix policy, and broader type checking separate unless the active delivery proves they are required.

The current `processing/` project is Java/Processing and has its own future build, test, documentation, and lint lifecycle. It is not a JSDoc or ESLint adoption surface merely because it is stored in this repository. Any future p5.js project would require its own explicit JavaScript boundary.
...
- the exact JSDoc convention and primary consumer: source/editor help, TypeScript/JSDoc checking, lint validation, or later reference generation
- whether the first JavaScript area is documentation-only, editor-assisted, or selectively type checked
...
- the application-owned configuration, dependency-lock, and command locations
...
- the cleanup leaves durable rules that other Python or JavaScript applications can adopt through their own lifecycle
