...
- [ ] Add the smallest composition callback from the existing theme owner to the manage-owned inline Mermaid adapter without introducing a Mermaid dependency into the theme module.
- [ ] Resolve the accepted semantic token seed from the active viewer root as concrete colors.
- [ ] Change Mermaid initialization from a once-per-session neutral configuration to a once-per-queued-render configuration using the resolved `base` theme variables.
- [ ] Preserve strict security, disabled HTML labels, lazy asset loading, sequential rendering, accessibility metadata, and readable-source failure behavior.
- [ ] Make initial inline rendering use the active light or dark palette.
- [ ] Stop for review after theme ownership, the exact variable seed, and light/dark initial rendering are proven.
