...
**Complete. IMT-P0–P2 shipped and every completion gate passed.**
...
[Mermaid Authoring And Publication](/docs/?scope=studio&doc=d-20260724-190603-860f64) owns the broader forward feature and treats this shipped delivery as the managed-reader prerequisite for later public projection.
...
- [x] Extend the theme-module DOM harness to prove the optional callback receives normalized initial and changed themes without changing storage or toggle behavior.
- [x] Extend the inline Mermaid DOM harness to prove concrete light/dark configuration, retained-source sequential re-rendering, in-place SVG replacement, no-diagram no-op behavior, and failure retention.
- [x] Extend the detail-adapter harness to prove one current URL per inline host, successful replacement ordering, superseded URL revocation, replacement failure retention, and ordinary document cleanup.
- [x] Inspect representative inline flowchart, sequence, and state diagrams in both themes, including explicit source styles and narrow horizontal overflow.
- [x] Confirm **Open diagram in new tab** matches each displayed theme and retains title, description, responsive `viewBox`, and browser zoom.
- [x] Confirm the public route still does not load the manage-owned Mermaid adapter or vendor asset and persistent diagram behavior is unchanged.
- [x] Run the smallest affected module, route, browser, and repository-whitespace checks.
- [x] Update the Theme Restoration parent and Mermaid Authoring And Publication owner with shipped evidence, then close this delivery.

**Closeout evidence:** the pinned Mermaid browser fixture renders accessible flowchart, sequence, and state examples in light and dark, retains a source-authored `classDef`, preserves narrow-layout scroll ownership, replaces only each SVG and current detail target, and verifies standalone title, description, canvas background, and responsive `viewBox`. Real manage-route review confirms the lifecycle diagram remains readable in both themes and receives a replacement detail URL; dedicated detail tests retain ordinary standalone browser zoom by continuing to serve a responsive SVG document. External-local, public read-only, persistent-detail, failure-retention, and cleanup checks pass. The `docs-viewer-smoke` profile passed `20/20`; summary: `var/admin/test-runs/inline-mermaid-theme-p2/summary.md`.
