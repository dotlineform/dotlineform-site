...
last_updated: "2026-07-24 19:36:52"
...
**In progress. IMT-P0 is implemented and at its review gate; IMT-P1 has not started.**
...
- [ ] Retain the exact source and host for each successfully mounted inline diagram.
- [ ] Re-render only those registered diagrams when the theme callback runs; do not scan or reconcile the document.
- [ ] Replace the SVG inside the existing host without rebuilding its frame, viewport, or control.
- [ ] Make the diagram and serialized SVG canvas use the resolved theme background.
- [ ] Give the detail adapter one refresh operation that atomically replaces the current inline Blob target and revokes the superseded target.
- [ ] Retain the previous displayed SVG and detail URL when either re-rendering or replacement-resource creation fails.
- [ ] Keep persistent SVG, public reader, review/detail surfaces, source mode, and exports unchanged.
