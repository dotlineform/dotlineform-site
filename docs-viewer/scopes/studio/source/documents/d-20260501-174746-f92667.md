...
## Docs Viewer Lint Setup

The `docs` profile expects the pinned, isolated Docs Viewer lint tools to be installed:
...
For a narrower source change, both direct commands accept explicit file or directory paths; without arguments they check the proven pilot specimens:

```bash
docs-viewer/bin/lint-python [paths...]
docs-viewer/bin/lint-js [paths...]
```
...
- `docs` — focused Docs Viewer Ruff/ESLint checks, Docs Viewer/Data Sharing Python contracts, and Studio docs/search generation.
