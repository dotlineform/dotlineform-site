...
last_updated: "2026-07-24 13:57:12"
...
- `docs_document_transfer_apply.py` owns the CMV-2 fresh-identity Copy transformation, create-only media/build-source transfer, registered output production, target-only document write/rebuild, result, and partial-target failure evidence. It is not yet connected to a route or UI.
...
**Current state:** CMV-2 is complete. The generalized planner and fresh-identity Copy apply service are implemented and focused-tested, including provider-neutral media and registered build sources. No route or UI has cut over to them; the shipped singular Copy workflow remains current.

**Next checkpoint:** CMV-3 identity-preserving Move apply, only after explicit approval.
