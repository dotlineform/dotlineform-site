...
last_updated: "2026-08-10 17:45:13"
...
viewer_report_access: local
viewer_report_preset: scope_documents_admin
```
...
Changing a source document to `viewer_report_access: public` does not automatically make the report executable on `/analysis/` or another public route.
...
It reads `viewer_report_subscope` from the selected parent document payload, validates that sub-scope against the current scope's browser config, loads the configured `manifest.json`, and renders its ordered document identity/title records as a report-owned list state.
