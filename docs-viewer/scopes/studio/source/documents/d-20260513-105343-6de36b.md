...
`catalogue_works` is the local, read-only published-Works inspection report hosted in the `dotlineform` scope. It reads canonical Works and Series through the Local Studio Catalogue API, requires a non-empty Work or Series search before showing rows, preserves every exact Series membership, and renders Work, Year, Title, Series, and curator-only Storage in one sortable semantic table. Copy table exports the current sorted result as TSV. The report is registered and executable only in the local report graph; public report metadata, loaders, data, and CSS do not expose it. [Catalogue Works](/docs/?scope=studio&doc=d-20260401-000000-ebf14a) owns its durable data, link, editing, and extension contract.
