...
`docs-viewer/config/document-packages/profiles.json` defines configured package shapes for the existing documents adapter family. The documents package engine performs semantic validation; there is no separate profile schema.
