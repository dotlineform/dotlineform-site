...
The report combines the occurrence index with that scope’s document index tree to display Title, Identity, and the owning Document link.
...
- Docs Broken Links scans source Markdown and checks the registry/target lookup independently, because unresolved tokens are necessarily absent from the usage index.
