---
doc_id: d-20260331-000000-cdc571
...
```bash
$HOME/miniconda3/bin/python3 studio/tests/audits/catalogue_site_consistency.py --strict
```

Narrow by check, series, or work when investigating a finding:

```bash
$HOME/miniconda3/bin/python3 studio/tests/audits/catalogue_site_consistency.py \
...
The functions and constants in `catalogue_site_consistency.py` are the exact current inventory. This page deliberately does not duplicate every artifact or query key.
