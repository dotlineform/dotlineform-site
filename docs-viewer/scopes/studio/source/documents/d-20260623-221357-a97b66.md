...
Title and optional group filters remain report-owned and do not change the
selected Docs Viewer document.
...
deriving visible rows from a case-insensitive title prefix. A Manage manifest
may additionally supply ordered document groups; public manifests do not.

In Manage mode, the optional contribution may add collection-local checkbox
selection and Actions. It receives the visible eligible projection so
...
- leaves only the minimal report controls, currently the icon-pill Back control
...
Manage manifest retains local status, viewability, and configured group
...
non-viewable records, plus local list metadata. It is excluded from
publication.
