...
selection and Actions. Checkbox interaction does not navigate; only checked IDs
target a selection action, and unmount or collection change clears that owner.
...
The manifest is an existence, ordering, and list-label contract, not an index tree.
...
- Published sub-scopes must include a minimal manifest so the report can validate, order, and label detail ids without loading a tree, search index, or every document payload.
