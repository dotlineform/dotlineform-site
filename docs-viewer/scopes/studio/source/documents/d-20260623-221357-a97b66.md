...
In Manage mode, the standard contribution adds status/viewability treatment,
...
**Select all** excludes filtered-out and non-viewable rows. Checkbox interaction
...
Manage manifest retains local status, viewability, `last_updated`, and any
...
non-viewable records, plus the standard Manage fields and any registered
...
- the parent remains selected in the index while `subdoc` stays report-owned
  URL state
- list rows come from the complete local inventory and can show the existing
  Status and non-viewable markers
...
Public behavior continues to use only the eligible `{ doc_id, title }`
manifest and compact by-ID payloads. It receives no management inventory,
Status/non-viewable markers, source controls, or metadata actions.
