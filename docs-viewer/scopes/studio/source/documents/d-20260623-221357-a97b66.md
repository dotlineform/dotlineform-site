...
```json
{
  "sub_scope": "tags",
  "title": "Tags",
  "manifest_url": "/docs-viewer/scopes/analysis/published/documents/sub-scopes/tags/manifest.json",
...
The local report also loads a management-only inventory from
`GET /docs/sub-scope-documents`. The inventory contains every configured child,
including non-viewable records, and supplies only `doc_id`, `title`,
`ui_status`, and `viewable`. It is not projected to public routes.
