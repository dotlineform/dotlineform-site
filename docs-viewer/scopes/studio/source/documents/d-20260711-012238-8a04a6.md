...
## Relationship To Edited Review Source Round-Trip

[Edited Docs Review Source Round-Trip](/docs/?scope=studio&doc=d-20260730-121423-78b0f6)
is a separate proposed feature. It treats the complete edited Markdown
`source/` folder of an import-capable validated Docs Review package as the
user-approved content for whole-folder overwrite after an explicit Build and
visual review.

This full-export delivery remains the outbound-fidelity owner:

- it proposes exact canonical Markdown rather than rendered-derived Markdown;
- it inventories and carries source dependencies and assets; and
- it reduces what may be lost before an external editing task begins.

It does not become an input to edited-source-folder Import. The proposed
`document-full-source` profile remains export-only with
`supports_return_import: false`, and no Full Document Export package enters Docs
Review or Import through implication. Making that richer profile reviewable or
returnable would require a later explicit capability and delivery.

The edited-source feature may initially accept the known fidelity limits of an
importable `document-content` review package after a clear warning and user
confirmation. It must not duplicate this delivery's source-faithful export or
asset-discovery work.
...
`canonical_markdown` is the authoritative source field for that row. It contains the complete UTF-8 source file, including front matter, body Markdown, media tokens, semantic tokens, raw allowed embeds, comments, and formatting. JSON escaping is transport encoding only; decoding the string must reproduce the original source bytes used for `source_sha256`.
