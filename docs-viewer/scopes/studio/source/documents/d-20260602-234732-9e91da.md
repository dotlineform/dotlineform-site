...
It does not own API paths, filesystem roots, public-site base URLs, media hosts, pipeline variants, document-package config, or UI copy. Those are server-, owning-app-, pipeline-, or code-owned.
...
The source JSON is not the complete runtime payload. Exact tag service endpoints live in `ANALYTICS_SERVICE_ENDPOINTS`; there is no Data Sharing API projection.
