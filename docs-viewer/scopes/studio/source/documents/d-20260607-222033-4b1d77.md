...
The Manage Source action uses the exact mounted `{scope, sub_scope, doc_id}`
target with `GET /docs/metadata`. A valid Work or Series subject is matched to
the existing semantic target lookup and serialized with its current target
title. A valid Folder subject is serialized only after
`POST /docs/validate-local-target` confirms the one exact normalized relative
path is confined and currently exists; validation does not open the target.

The action replaces the current Source selection through the ordinary buffer
adapter. It does not send a source revision, update front matter, save, rebuild,
or subscribe inserted text to later subject reassignment. Missing, malformed,
conflicting, mismatched, invalid, or unavailable metadata and targets leave the
buffer unchanged.
