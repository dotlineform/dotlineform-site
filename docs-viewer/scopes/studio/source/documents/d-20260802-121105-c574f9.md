...
last_updated: "2026-08-05 22:46:23"
...
## Live Run, Failure, And Legacy Cleanup
...
SSP-2.1 and SSP-2.5 also shipped `var/docs/project-state/folder-lookup.json` as a proposed Resolver consumer product. Resolver Tokens were retired before implementation, so the persisted lookup now has no consumer and is not part of the durable Project State architecture. SSP-2.6 removes its schema, status/input receipt, write path, configuration, tests, and documentation while preserving the live v2 response and report behavior. No new feature should consume or refresh the legacy file.
...
- image discovery, file counts, missing-image analysis, or primary-image auditing;
...
- retirement of the current Studio image-audit workflow, which belongs to SSP-6.
