...
last_updated: "2026-08-02 12:11:05"
...
Project State is a generated relationship artefact with downstream consumers. This document owns its meaning, row model, report projection, resolver lookup, refresh behavior, and failure boundary. Delivery details belong to [PRJ-3](/docs/?scope=studio&doc=d-20260801-133318-15e8fa).
...
PRJ-3.0 freezes the exact schema and state vocabulary. The structural commitments are the normalized folder key, `documents[]`, `series[]`, explicit folder state, generation metadata, and separate exceptional records for evidence that cannot form a valid folder row.
...
It does not publish `Works[]`, Work subjects, catalogue URLs, or Series titles as parallel canonical truth. Each Series item carries the semantic identity `(catalogue, series, canonical id)`. PRJ-4 passes that identity through the existing Semantic Tokens target lookup and link projector, producing one resolver-derived semantic usage row for every rendered Series link.
...
Report source, relationship snapshot, and resolver lookup carry one generation identity. PRJ-3.0 must freeze staging, validation, commit order, and exact-build failure behavior so consumers never treat a mixed generation as complete.
...
PRJ-4 consumes only the versioned folder lookup and Semantic Tokens projection. It does not depend on the saved report's Markdown layout. Project Media Audit may reuse the generated-document lifecycle, but it does not enter the Project State snapshot or lookup.
...
- retirement of the current Studio image-audit route and Markdown output, which belongs to PMA-1.
