...
One normalized row originates from one immediate project directory established by the current scan. Nested directories remain internal project contents. The producer independently joins zero or more Project documents and zero or more canonical Works using the exact Work projection `projects/<normalized project_folder>`. Project State reads only `work_id`, `project_folder`, and `series_ids` from each Work; Studio catalogue/media retains every nested media-location field. After the Work join, the producer groups retained Work identities by current Series ID and derives the count presented beside each Series. A Work belonging to more than one Series contributes once to each applicable membership count, so Series counts do not imply a unique sum.
...
| --- | --- | --- |
| one validated local-folder link | zero or more linked Series labels with their matching canonical Work counts | zero or more exact Project-document links |
...
Every scanned row has a normalized key beginning `projects/` with one project-directory segment. The Folder cell omits that guaranteed prefix and gains a leading slash, so `projects/curve poems (2025)` appears as `/curve poems (2025)`. The stored key, lookup identity, and `dlf-local:` activation target remain unchanged.
...
When a folder contributes Works to two Series, the Series cell contains one linked line per Series, such as `series 1 — 87 works` and `series 2 — 1 work`.

The Series and Docs cells show concise states such as no canonical Works or no document. Together with the fully reconciled row, the table therefore exposes document-only, Works-only, and folder-only states without changing its server-side row owner. Run time, counts, and scan diagnostics may appear above or below the table without adding more relationship columns. Report rows are runtime inspection data and do not enter document Source, automatic exports, bookmarks, or Docs Viewer search; only an explicit clipboard action creates a portable snapshot.
...
| --- | --- | --- |
| Folder | Folder, Series (Works), Docs | one row per scanned real folder; multiple Series remain ordered within the Series cell |
| Series | Series (Works), Folder, Docs | one row per exact Series-folder relationship; a multi-Series folder and its Docs repeat once for each Series |

Folder is the initial projection because the physical scan establishes the authoritative inventory. The Series projection is derived locally from each folder row's exact `series[]` memberships and per-Series Work counts; it does not require a Series-to-folder server index or response-schema change. A folder with no canonical Works remains visible as one explicit `No canonical Works` row. A matched Work with missing, malformed, or unknown Series membership remains a distinct exceptional Series state rather than disappearing or being collapsed into no Works.

Dynamic search filters the current projection locally after every input change, using a case-insensitive substring match over the composed visible text of Folder, Series (Works), and Docs. It does not search arbitrary raw JSON or hidden identities. Typing, clearing the search, switching grouping, changing sort order, and copying the current projection perform no server request.

Every visible column heading supports ascending and descending sorting over its current browser rows. Folder uses natural normalized display-path order. Docs uses the first ordered normalized document title, then exact `doc_id`. Series (Works) uses the applicable Work count, then normalized Series title and canonical Series ID; in Folder view the count is the row's distinct matched-Work count rather than the potentially double-counted sum of Series membership counts. Every comparator falls back to the normalized folder key so the result remains deterministic. Each projection initially sorts by its leading column ascending. Switching grouping preserves the search text and selects the new projection's leading sort; a successful Run/Refresh preserves the selected projection and reapplies its active search and sort.
