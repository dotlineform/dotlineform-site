...
- [ ] State the documentation, linting, type-checking, or reference-generation outcome.
...
- [ ] Treat explicit-path results as evidence only for the submitted paths and maintained-scope results as evidence only for the configured boundary. The completed Docs Viewer pilot remains proving history, not current tool ownership.
- [ ] Keep suppressions rare, local, and justified.
- [ ] Use this capability from the start of major new Python/JavaScript features; historical documentation backfill can remain value-ordered rather than blocking adoption.
