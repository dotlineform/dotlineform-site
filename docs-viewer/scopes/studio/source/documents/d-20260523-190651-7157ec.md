...
last_updated: "2026-08-13 21:58:53"
...
- [ ] For a roadmap delivery, keep the initial readiness step read-only and concise. Exact file inventories, line estimates, frozen code, fixtures, prototypes, generated deltas, and complete test architecture are implementation work, not approval prerequisites.
...
- [ ] When Docs watcher has already generated the intended document and search projections, inspect those outputs and do not rerun the builders solely to prove idempotence.
...
- [ ] For each material implementation step, add a proportional verification-budget table after owner inspection and before adding tests or running broad checks. Name the changed contract or credible risk, lowest adequate evidence, whether a permanent test changes, and the unique regression it can catch.
- [ ] Do not add or run evidence outside the accepted step budget without recording the newly discovered risk and updating the delivery step first. Documentation-only or trivial steps may explicitly require no executable evidence.
...
- [ ] Broad profiles and permanent browser tests are not default closeout gates. Require the concrete cross-owner or browser-only boundary that focused lower-layer evidence cannot prove.
...
- [ ] Review the test diff for duplicated coverage, UI choreography assertions, unnecessary fixtures or abstractions, broad-suite expansion, and checks that cannot name a unique regression. Remove excessive evidence rather than treating test volume as confidence.
