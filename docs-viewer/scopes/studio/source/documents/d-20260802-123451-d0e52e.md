...
last_updated: "2026-08-02 21:01:06"
...
Data Model Maps provide a durable orientation layer without treating every JSON file as a relational table or reproducing every field from the code. The notation is also intended for paired feature-spec diagrams: **before** records the relevant current slice, while **after** is explicitly labelled as a proposal until delivery makes it current.
...
The first delivery should keep only a few useful diagrams authored because their value is semantic classification, not merely enumerating parseable files. Each durable map has a named owner, a small register, and a review trigger after a major delivery changes one of its displayed relationships. Routine changes do not require maintaining an exhaustive repository diagram index.

Feature specifications can carry their own small before/after pair. The before map links to current authorities; the after map must say **proposed** and use the same node names where identity is intended to survive. After delivery, update a durable map only when the changed relationship belongs to one of the retained system explanations.
