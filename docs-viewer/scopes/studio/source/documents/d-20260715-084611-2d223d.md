---
doc_id: d-20260715-084611-2d223d
...
Testing summaries are written by `tests/run_checks.py` under `var/test-runs/`. The Admin route is a read model over those outputs, not their producer.
...
- Checks, audits, and Testing have related but separate artifact/result shapes rather than one normalized operational-result model.
...
- lifecycle issue: `admin-route-state.js` and the shared [Route Ready State](/docs/?scope=studio&doc=d-20260625-162242-b0b6c7).
