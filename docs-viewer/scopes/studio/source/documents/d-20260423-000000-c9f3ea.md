...
3. run configured media producers into published roots, then resolve supported media and semantic tokens
...
The common front matter is `doc_id`, `title`, `parent_id`, `added_date`, `last_updated`, optional `summary`, and optional `ui_status`. An exact collection with a configured public projection may also use optional `publishable`; local and local-external collections reject it. Exact defaults and validation are in the builder/source model.
...
- `publishable: false` keeps a publish-capable document available to management while excluding it, and parent-tree descendants, from the prepared public tree/search/Recent projection.
- local collections carry no publishability field and keep their normal complete local projections.
