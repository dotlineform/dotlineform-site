...
last_updated: "2026-08-07 22:31:15"
...
Scope-wide Publish remains the only public mutation. After successful Publish, it records or clears each affected B's current public URL, time, and generation in the same private table. Public-scope `publishable` means inclusion in the next Publish; it is not proof of current publication and does not exist in local collections. Projects report cues derive 🟠 Pre-publish and 🟢 Published from the exact table, current B state, and successful publication evidence. Analysis may show actual publication separately from `publishable`. Project State icons remain subject-placement provenance and never consume publication lineage.
