...
Returned Work and Series identities reuse Semantic Tokens target and link projection. Generated usage evidence records source scope, collection, document, containing range, effective subject kind/key, lookup generation, and returned registered identity. A selected Manage build containing Project Resolver Tokens refreshes Project State once after current subject products are available and before rendering. Individual token expansion never refreshes again, scans canonical inputs, mutates source, recursively follows returned results, or silently falls back to UI context.
...
flowchart TD
    ...
    selectedBuild["Selected Manage Docs build"] --> tokenPlan{"Selected Project Resolver Token?"}
    tokenPlan -->|"no"| ordinaryBuild["Build selected documents normally"]
    ...
    projectRefresh --> referenceBuild["Render all selected Project Resolver Tokens<br/>against one lookup generation"]
...
A derived failure never rolls back or repeats its source mutation. Each producer retains its previous complete generation and exposes stale or unavailable state. Generated products that must agree carry one generation identity or an explicit compatibility boundary. Project Resolver rendering never silently uses the previous lookup as current after its prerequisite fails. Public builds, builds without selected Project Resolver Tokens, and individual token expansions perform no Project State refresh.
