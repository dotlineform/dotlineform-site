...
```text
front matter
  -> registered collection customisation
  -> typed subject in manifest
     -> collection report
     -> subject-to-document association
     -> focused relationship lookup
        -> Project State report
        -> current references
        -> Registry primary selection
        -> public Work metadata

working sub-scope document
  -> exact collection-aware copy
  -> independent non-viewable editorial document
  -> review and existing Analysis Publish
...
The registered customisation validates source and projects one normalized manifest form:
...
The manifest never infers subject identity from title, filename, body content, current route, selected row, report host, or a neighbouring source. Several documents may declare the same subject. Normal product use may prefer one-to-one associations, but cardinality remains zero-to-many and deterministic.
...
A folder-subject document contributes directly to its declared folder row. A work-subject document contributes through the canonical Work's recorded folder evidence while retaining its association kind and `work_id` in the generated data. Work-keyed lookups may exist for exact consumers without requiring a second visible Work report. Another list is added only when it answers a distinct proven management question.
...
Tag documents decide only that they describe one canonical `tag_id`. Their manifests produce the complete derived `tag_id -> documents[]` association.
...
Canonical document mutation, manifest generation, relationship generation, Resolver rendering, copy, publication, and catalogue payload generation retain separate commit boundaries:

```text
save subject front matter
  -> rebuild exact collection manifest and associations
  -> refresh affected focused relationship rows
  -> rebuild exact current-reference document

publish editorial document
  -> publish public document association
  -> rebuild affected public Work payload
```
