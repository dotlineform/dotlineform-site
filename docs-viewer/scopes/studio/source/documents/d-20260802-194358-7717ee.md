...
last_updated: "2026-08-08 19:54:49"
...
Projects derives no status for a Working document without children, Editorial when current children have no public URL, and Published when any current child has one. One highest non-linking row cue summarizes that state, while focused detail retains every exact child identity and its current presentation evidence without selecting a primary child. Projects has no Folder, Work, or Series row icon; Project State retains those subject-placement icons. Analysis adds no publication-stage cue and keeps only its existing `publishable: false` exclusion icon. A complete orphan-lineage report is unnecessary because lineage ends when Working is deleted.
