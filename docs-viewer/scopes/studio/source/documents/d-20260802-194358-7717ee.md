...
last_updated: "2026-08-10 15:26:37"
...
flowchart LR
    ...
    subgraph inputs["Canonical or independently mutable inputs"]
        ...
        editorialDocs["analysis/works Editorial Markdown<br/>independent identities"]
    ...
    subgraph private["Private derived products"]
        ...
        catalogueLookup["Studio Catalogue lookup projections<br/>Series and Work search"]
        ...
        coverageReport["Catalogue document coverage<br/>local Works report consumer"]
        subjectLink["Insert subject link<br/>Source authoring action"]
        publicationLineage["Persisted publication-lineage table<br/>Working document with current Editorial children"]
    ...
    projectManifest -->|"Normalized subjects and exact document targets"| coverageReport
    catalogueLookup -->|"Published status and current membership"| coverageReport
    projectDocs -->|"Configured New or Replace Copy"| editorialDocs
    projectDocs -->|"Exact Working identity"| publicationLineage
    editorialDocs -->|"Exact Editorial identity"| publicationLineage
...
Docs Viewer Markdown and Studio JSON remain separate canonical authorities. Physical folders are independently mutable evidence. The publication-lineage table is persisted operation-owned state because its exact Working-to-Editorial relationship cannot be reconstructed by inference; private manifests, associations, reports, snapshots, and rendered links remain replaceable producer-owned products. Public products are promoted only through configured Publish; DPD-1 separately makes irreversible canonical document Delete remove the deleted document's exact public projection immediately.
...
| publication lineage | Which exact Editorial documents were created or replaced from one current Working document, and which currently have public projections? | Projects publication-stage cues |
...
All private working notes remain in `dotlineform/projects`, including generic notes, folder-state documents, and documents intended for later Work/Series publication. The normal Manage index supplies equal-width sortable **Doc title** and **Subject** columns rather than another report or collection:

- a valid `folder_path` shows the folder-subject icon and exact stored path as a validated Finder link;
- a valid `work_id` shows the single-item dot-and-line cue, while a valid `series_id` shows the established three-item **Group by Series** list cue; both join the exact `{kind, ID}` to the existing Catalogue target lookup and link its current title to the configured local preview;
- an unavailable Work or Series target falls back to its exact ID rather than the document title;
- no declaration shows an empty Subject value; and
- malformed or conflicting declarations show a non-linking authoring warning without blocking copy or publication.

The Subject projection consumes normalized Manage-manifest metadata plus the existing Catalogue target lookup and never reparses Markdown or infers a target from document title, filename, route, or row context. Subject sorting uses the displayed exact target title or Folder path; missing and warning states remain after valid subjects. The icon remains an authoring hint in Manage indexes and Project State: it is not document status, publication eligibility, or proof that a referenced folder or catalogue target currently exists.
...
Catalogue document coverage is a curator reporting requirement over the same exact Work/Series-to-document associations. The shipped local [Works Report](/docs/?scope=studio&doc=d-20260810-115737-901d0f) owns its presentation as one row per published Series. A Series row contains exact Projects documents declared about that Series plus exact Projects documents declared about any published Work whose current canonical membership includes that Series. One matching document is sufficient Series-level coverage; draft Works contribute no coverage, and the report does not identify or score undocumented member Work IDs. Series subjects reach only their declared row, while one published Work subject may reach several published Series through exact current membership; blank Series rows remain visible.

The report remains an observer composed in the browser: canonical Catalogue JSON owns publication and membership, the existing Studio Series/Work search projections carry that state into local browser reads, document front matter owns the declaration, and the private Projects Manage manifest supplies normalized subjects plus exact document targets. Mount and Run/Refresh each replace the result only after all three complete inputs validate. The report title follows the public-site **Works** vocabulary but the report remains local in scope `dotlineform`; it adds no Works producer or endpoint and does not move presentation into Studio Works, read public Catalogue JSON or `analysis/works`, consume public `doc_url[]`, mutate its inputs, or change public Catalogue pages.
...
The strict private `docs_document_publication_lineage_v3` table stores configured Working and Editorial collection identities once, then deterministically ordered records keyed by exact current Working `doc_id`. A Working document may have zero-to-many current Editorial children, but a record exists only while at least one child remains. Each child stores one exact Editorial `doc_id`, `created_at`, `last_copied_at`, and nullable `published_url`. This is workflow lineage, not content synchronization or document identity rewriting, and it exists only after the configured Copy action records an exact relationship.

For a lineage-enabled configured Copy, the author explicitly chooses **New** or selects one exact current Editorial child for **Replace**. New creates a fresh Editorial identity and child. Replace preserves the selected Editorial document's immutable identity, placement, `added_date`, and current `publishable` gate while replacing its content and transferable metadata through the target transformation. Replace performs no comparison, diff, merge, automatic publication, or fallback to another target.

Scope-wide Publish remains the only positive public promotion. After successful Publish, it records or retains each current publishable Editorial child's public URL and clears it only when explicit `publishable: false` excludes that surviving child or its required public host. Public-scope `publishable` means inclusion in the next Publish; editing or exclusion leaves the current public projection unchanged until Publish. Set Publishable is lineage-neutral. Confirmed Editorial Delete first uses [DPD-1 — Immediate Public Projection Cleanup On Document Delete](/docs/?scope=studio&doc=d-20260808-165751-1f159a) to remove any exact current public projection and affected inventories, then removes only that child and drops an empty Working record. Confirmed Working Delete removes the complete lineage record without deleting independently canonical Editorial or public documents. Out-of-workflow filesystem deletion performs no lineage mutation or repair.

Projects derives no status for a Working document without children, Editorial when current children have no public URL, and Published when any current child has one. One highest non-linking row cue summarizes that state, while focused detail retains every exact child identity and its current presentation evidence without selecting a primary child. The separate Projects Subject column uses Folder, Work, and Series icons to identify the exact authoring subject and activate that target; Project State uses the same icon vocabulary for the distinct route through which a document entered a scanned Folder row. Analysis adds no publication-stage cue and keeps only its existing `publishable: false` exclusion icon. A complete orphan-lineage report is unnecessary because lineage ends when Working is deleted.

The table uses exact `{scope, sub_scope, doc_id}` identities only. It never infers Working-to-Editorial relationships from subject, title, filename, content, dates, public URL, route, or selected UI state and never enters public products. SSP-5 public Work/Series metadata remains independently derived from the final published document's exact subject front matter.
...
flowchart TD
    ...
    accDescr: Canonical mutations retain their owners; Copy records Working-owned Editorial children, Publish records current public evidence, Delete removes obsolete public projections, and public catalogue metadata follows exact published subjects.
    ...
    copyChoice["Configured Copy<br/>New or exact Replace"] --> editorialCommit["Commit independent Editorial document"]
    editorialCommit --> lineageCopy["Commit exact Working-to-Editorial evidence"]

    publish["Publish document through ordinary workflow"] --> publicDocs["Commit public document output"]
    publicDocs --> lineagePublish["Reconcile current Editorial public evidence"]
    deleteEditorial["Delete canonical Editorial document"] --> publicDelete["Remove exact public projection<br/>and affected inventories"]
    publicDelete --> lineageEditorialDelete["Remove exact Editorial child"]
    deleteWorking["Delete canonical Working document"] --> lineageWorkingDelete["Remove complete Working record"]
    lineageCopy --> publicationCues["Refresh Projects stage cues"]
    lineagePublish --> publicationCues
    lineageEditorialDelete --> publicationCues
    lineageWorkingDelete --> publicationCues
...
Copy and Publish use ordinary post-success lineage updates. Confirmed document Delete applies its small Working-owned lineage follow-through only after its ordinary source mutation and any required DPD-1 public cleanup succeed. These paths add no lineage-specific rollback, retry, generation receipt, or generic transaction protocol. Association and public-payload producers retain their existing failure boundaries. Project State shows the current live-run failure without older rows. Insert subject link performs no build or Project State refresh; normal rendering later treats its bytes through the existing semantic-token or local-link owner.
