...
flowchart LR
    ...
    subgraph private["Private derived products"]
        ...
        publicationCues["Projects and Analysis reports<br/>publication-stage cues"]
    end
...
| publication lineage | Which exact editorial B documents were created or replaced from working A, and which currently have public projections? | Projects and Analysis publication-stage cues |
...
[SSP-7 — Document Lineage And Publication State Delivery](/docs/?scope=studio&doc=d-20260805-231829-947705) persists one private row for each exact working-source A to editorial-target B relationship. This row is workflow lineage, not content synchronization or document identity rewriting. A may have several B rows only through deliberate New choices.

For a lineage-enabled configured Copy, the author explicitly chooses **New** or selects one exact existing B for **Replace**. New creates a fresh B and row. Replace preserves the selected B's immutable identity, placement, `added_date`, and current `viewable` gate while replacing its content and transferable metadata through the target transformation. Replace performs no comparison, diff, merge, automatic publication, or fallback to another target.

Scope-wide Publish remains the only public mutation. After successful Publish, it records or clears each affected B's current public URL, time, and generation in the same private table. `viewable` continues to mean inclusion in the next Publish; it is not proof of current publication. Projects report cues derive pre-publish and published stages from the exact table, current B state, and successful publication evidence. Analysis may show actual publication separately from `viewable`. Project State icons remain subject-placement provenance and never consume publication lineage.
...
flowchart TD
    ...
    lineageCopy --> publicationCues["Refresh Projects/Analysis stage cues"]
