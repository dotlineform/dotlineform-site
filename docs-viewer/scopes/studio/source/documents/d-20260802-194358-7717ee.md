...
flowchart LR
    ...
    subgraph private["Private derived products"]
        ...
        publicationLineage["Persisted publication-lineage table<br/>Working A with current Editorial children"]
...
Docs Viewer Markdown and Studio JSON remain separate canonical authorities. Physical folders are independently mutable evidence. The publication-lineage table is persisted operation-owned state because its exact A-to-B relationship cannot be reconstructed by inference; private manifests, associations, reports, snapshots, and rendered links remain replaceable producer-owned products. Public products are promoted only through configured Publish; DPD-1 separately makes irreversible canonical document Delete remove the deleted document's exact public projection immediately.
...
[SSP-7 — Document Lineage And Publication State Delivery](/docs/?scope=studio&doc=d-20260805-231829-947705) shipped exact A-to-B evidence through the New/Replace Copy boundary. [SSP-8 — Revised Model](/docs/?scope=studio&doc=d-20260808-153226-a067d9) now owns its replacement: one record keyed by current Working A with zero-to-many current Editorial B children. This is workflow lineage, not content synchronization or document identity rewriting. A gains children only through deliberate New choices and needs no record when it has none.
...
Scope-wide Publish remains the only positive public promotion. After successful Publish, it records or retains each current publishable B child's public URL and clears it only when explicit `publishable: false` excludes that surviving child or its required public host. Public-scope `publishable` means inclusion in the next Publish; editing or exclusion leaves the current public projection unchanged until Publish. [DPD-1 — Immediate Public Projection Cleanup On Document Delete](/docs/?scope=studio&doc=d-20260808-165751-1f159a) is the next prerequisite and will make irreversible canonical document Delete remove any exact current public projection immediately. Publish will then stop discovering or reporting source deletes and limit its negative plan to publishability exclusions. SSP-8 removes a deleted B child or the complete record for a deleted A; it retains no Published-without-Editorial or Working-less lineage.

Projects derives no status for A without children, Editorial when current children have no public URL, and Published when any current child has one. Its old Folder, Work, and Series row icons remain removed; Project State retains those subject-placement icons. Analysis adds no publication-stage cue and keeps only its existing `publishable: false` exclusion icon. A complete orphan-lineage report is unnecessary because lineage ends when A is deleted.
...
flowchart TD
    ...
    deleteDoc["Delete canonical public document"] --> publicDelete["Remove exact public projection<br/>and affected inventories"]
    publicDelete --> lineageDelete["Remove B child or complete A record"]
    lineageCopy --> publicationCues["Refresh Projects stage cues"]
    lineagePublish --> publicationCues
    lineageDelete --> publicationCues
...
Copy and Publish use ordinary post-success lineage updates. DPD-1 will define immediate public cleanup on canonical document Delete before SSP-8 connects its small Working-owned follow-through. Neither delivery adds a lineage-specific rollback, retry, generation receipt, or generic transaction protocol. Association and public-payload producers retain their existing failure boundaries. Project State shows the current live-run failure without older rows. Insert subject link performs no build or Project State refresh; normal rendering later treats its bytes through the existing semantic-token or local-link owner.
