...
The checkbox-selection foundation is shipped. **Prepare package** is the first shipped selection Action and resolves checked ids as its only target through its compact workflow; its duplicate route is retired and its focused delivery is complete.
...
Actions such as **Copy subtree**, **Edit metadata**, **New child**, and **New sibling** require exactly one checked document if they remain document Actions. Set Actions such as **Prepare package**, **Export**, **Move**, and **Delete** require one or more. No `-selected` action ids, plural labels, compatibility aliases, or parallel workflows are introduced.
...
The first consumer delivery is complete. No other selection Action is approved; Export, Move, or Delete requires its own explicit delivery decision and contract before implementation.
