...
ui_status: done
summary: Record the shipped Prepare package, Copy, Move, and Delete checkbox consumers and the remaining unapproved candidates.
...
The checkbox-selection foundation is shipped. **Prepare package**, **Copy to scope**, **Move to scope**, and **Delete** now resolve checked ids as their only document targets. None of these Actions falls back to the displayed or context-clicked document. **Export** and same-scope group Move remain unapproved candidates rather than missing parts of those shipped workflows.
...
Prepare package, Copy, Move, and Delete are shipped. The remaining candidates retain their provisional value-to-risk order:
...
Prepare package, cross-scope Copy/Move, and Delete are complete. Export and same-scope group Move remain unapproved and require their own product decisions.
