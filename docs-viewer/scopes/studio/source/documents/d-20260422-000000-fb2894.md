...
- confined project-media discovery.

The active browser API is `/studio/api/catalogue/...`; there is no standalone catalogue server or offline write path. [Local Studio APIs](/docs/?scope=studio&doc=d-20260602-111803-019594) is the focused current route inventory.
...
browser command
  ...
  -> optional lookup/build/media follow-through
```

`studio_catalogue_api.py` owns HTTP adaptation plus operations that need Local Studio context: reads, workbook import, and project-media discovery.
...
- Derived lookups, public artifacts, media, and operational logs are outcomes of a canonical operation; they are not alternate source authorities.
...
Project-media operations confine resolved paths below configured roots and return relative catalogue values rather than absolute filesystem paths. Remote media credentials and object details stay server-side.

Operational logs under `var/studio/catalogue/logs/` record compact IDs, fields, states, and errors rather than full submitted records.
...
4. Add the browser constant in `studio-transport.js` and the exact API inventory in [Local Studio APIs](/docs/?scope=studio&doc=d-20260602-111803-019594).
5. Define lookup, public-build, media, cleanup, and operational-log effects explicitly.
...
Use [Catalogue Build And Lookup Refresh](/docs/?scope=studio&doc=d-20260519-202931-0c703e) for the two derived-output planners. Current service code owns exact payload fields; request-contract tests provide focused evidence.
