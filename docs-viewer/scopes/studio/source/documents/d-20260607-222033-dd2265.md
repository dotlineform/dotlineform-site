...
Ownership: owns command shapes, targeted-docs/full-search decisions, diagnostics parsing, and watcher-suppression coordination for management writes.

Responsibilities:

- builds Python commands for `docs-viewer/build/build_docs.py` and `docs-viewer/build/build_search.py`
- supports full same-scope docs and whole-index search rebuilds
- supports targeted docs payload rebuilds with `--only-doc-ids`
- runs one whole-index search build when affected search identities are non-empty
