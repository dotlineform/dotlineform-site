...
![Docs Viewer configuration becomes a browser-safe projection while executable code and live capabilities gate workflow availability.]([[media:docs/studio/svg/docs-viewer-configuration-extension-flow.svg]])
...
| --- | --- | --- |
| `docs-viewer/config/scopes/docs_scopes.json` | Scope roots, scope type, default doc, hierarchy policy, build-media registration, independent public/R2 destinations, sub-scopes, and bounded viewer-wide settings. | Maintainer-edited source config read by builders and services. Local source, documents, search, and omitted-location media paths are fixed descendants of each `scope_root`. |
| `site/docs-viewer/runtime/js/shared/docs-viewer-code-config.js` | Universal metadata statuses and fixed scope-type labels/emoji. | Product-owned browser code. Every scope, including a newly created scope, receives the same vocabulary without a scope-config entry. |
...
| `docs-viewer/config/semantic-references/registry.json` | Reference kinds, identifier rules, public routes, the `data/generated/` target-lookup URL, and source-editor participation. | Data can reuse known normalizers and route shapes; new executable normalization requires code. |
...
Universal display vocabulary follows the same boundary. Metadata statuses and scope-type badges are not user-configurable per-scope data; changing them is a reviewed code-config change.
...
Edit `docs_scopes.json`, then rebuild the browser and generated projections. The accepted schema is `docs_scopes_v3`: every scope declares one repository or external-local `scope_root`; `source/`, `published/documents/`, `published/search/index.json`, and scope-local `published/media/<type>/` are derived. Public projections and R2 media remain explicit independent locations. Do not repeat derived paths or add an old-schema compatibility record.

Use the scope lifecycle workflow when creating or deleting a scope because it also maintains route files, route registries, the scope root, and the scope manifest. Sub-scopes retain only identity, title, and an optional independent public projection; their local paths derive from the parent root.
