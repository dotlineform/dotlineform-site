...
Edit `docs_scopes.json`, then rebuild the browser and generated projections. The accepted schema is `docs_scopes_v4`: one top-level external `media_workspace` owns all private managed media, each scope declares one repository or external-local `scope_root`, and `source/`, `published/documents/`, and `published/search/index.json` derive from that document root. `media.types` and `media.build_sources` derive exact private paths beneath the media workspace. Public documents, search, and per-type repository/R2 media targets remain explicit under `public_projection`. Do not repeat derived paths or add an old-schema compatibility record.
