...
| --- | --- | --- |
| `docs-viewer/config/scopes/docs_scopes.json` | Scope roots, scope type, default doc, hierarchy policy, build-media registration, independent public/R2 destinations, sub-scopes, and viewer-wide display settings. | Maintainer-edited source config read by builders and services. Local source, documents, search, and omitted-location media paths are fixed descendants of each `scope_root`. |
