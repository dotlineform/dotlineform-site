...
The editor also owns Work-level downloads, links, media selection, and detail-section management. Catalogue Work prose and its former source-Markdown workflow are retired. The editor does not provide individual detail-record editing.
...
- Draft Works remain recoverable from [Catalogue Drafts](/docs/?scope=studio&doc=d-20260418-000000-16bf7e).
...
Individual detail create, edit, and delete commands are intentionally absent. The aggregate workflow matches the source structure and avoids a second editor for rare record-level mutations. See [Catalogue Per-Work Detail Source](/docs/?scope=studio&doc=d-20260622-160612-b986f3) for the data boundary.
...
Credentials, local paths, object keys, checksums, and signed requests remain server-side. See [Publish Media To R2](/docs/?scope=studio&doc=d-20260508-093841-3e27b1) for the media-owned publishing contract.
...
The route exposes the shared [Route Ready State](/docs/?scope=studio&doc=d-20260625-162242-b0b6c7) contract on `#catalogueWorkRoot`. `studio/tests/smoke/studio_catalogue_route.py` proves only route/service boot. Use current code, field registry, and service routes for the exact field and endpoint inventory; focused Python tests provide deterministic evidence.
