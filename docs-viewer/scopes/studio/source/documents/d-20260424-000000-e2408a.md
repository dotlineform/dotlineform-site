...
last_updated: "2026-07-27 20:43:41"
...
- `docs-viewer/scopes/<local-scope>/published/documents/semantic-tokens/...`
...
Configured sub-scopes use a separate discovery contract:

- the local builder writes a by-ID payload for every child document, including
  `viewable: false` documents, so manage mode can render and edit them
- the manage-only inventory lists every child with exactly `doc_id`, `title`,
  `ui_status`, and `viewable`
- the public manifest lists only publicly eligible children and contains only
  `{ doc_id, title }`
- the public publish projection omits hidden child by-ID files

The manage browser config points at local sub-scope payloads, while public
browser config points at the public projection. This lets a non-viewable child
remain loadable in manage mode without exposing it publicly.
...
The **non-viewable** checkbox in **Edit metadata** is the only manage UI that
changes a document's viewability. It saves only the explicit rendered target
through `POST /docs/update-metadata`:
...
For a configured sub-scope target, the modal omits Parent and sends
`{ scope, sub_scope, doc_id }` with the existing Title, Summary, Date, Date
display, Status, and `viewable` values. The service rejects `parent_id` on that
target. A visibility change refreshes the child by-ID payload, manage inventory,
and public-manifest eligibility without adding the child to the parent index,
...
Manage-index selection includes viewable and non-viewable documents equally. **Select all** therefore covers the complete manage index, including non-viewable documents and collapsed branches.

The **Document content** package profile includes non-viewable documents by default and offers a modal checkbox to exclude them. The **Document tree** profile always includes them. This affects only package composition; it does not change source metadata, public navigation, search, or the publish gate.

`viewable: false` is not a privacy or external-sharing prohibition. A future rule that some documents must never leave the local workspace would need its own policy field.
