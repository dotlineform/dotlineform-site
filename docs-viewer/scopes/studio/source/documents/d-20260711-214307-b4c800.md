...
last_updated: "2026-07-30 10:52:00"
summary: Durable product, workflow, persistent package, read-only browser rendering, source-open, and authority contract for the local returned-package review route.
...
Package-local `source/*.md` is persistent derived build input. It is inspectable in the external workspace and reproducible from the validated returned package. Docs Review exposes one identity-only source-open command: it validates the active `package_id` and displayed `doc_id`, resolves that exact direct `source/<doc_id>.md` member, and opens it in VS Code without confirmation or canonical fallback.

The browser still exposes no source display mode, source-content provider methods, source-read/write capabilities, revision tokens, or save/rebuild workflow. Editing in the external application is outside Docs Review lifecycle; it does not automatically refresh retained generated preview output.
...
These checks cover validated package discovery, package-root containment, persistent generated reads and repair, inventoried assets, returned summary metadata and hierarchy, absence of a browser source editor and Import control, exact package-source opening, route boot, and canonical comparison without canonical writes.
