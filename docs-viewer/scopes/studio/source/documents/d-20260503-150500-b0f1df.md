...
- optional missing-summary filtering for a supporting profile; the direct service also accepts a supported non-viewable choice
...
- applies the supported missing-summary and non-viewable choices while revalidating explicit ids
...
The Docs Viewer service runs the engine directly through `POST /docs/packages/prepare`; the request uses `scope`, `profile_id`, effective explicit `doc_ids`, `select_all: false`, supported format overrides, `missing_summary_only`, `include_non_viewable`, and `dry_run`. It revalidates but does not broaden the submitted ids. It does not accept generic app/domain/adapter/operation fields or nested selection.
