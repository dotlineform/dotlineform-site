...
Catalogue Series prose and its former source-Markdown workflow are retired.
...
The route exposes the shared [Route Ready State](/docs/?scope=studio&doc=d-20260625-162242-b0b6c7) contract on `#catalogueSeriesRoot`. The retained Catalogue smoke exercises the sibling Work route only; use current code, field registry, and service routes for the exact field and endpoint inventory. Series behavior remains covered by focused Python tests and manual UI acceptance.
