...
An active Manage sub-scope detail may add explicit read-only fields through `managedDocumentTarget` and `metadataInfo`. The target is the validated `{scope, sub_scope, doc_id}` detail, and its matching record replaces the parent report or selected index row for the hosted view. The Projects customisation currently contributes one **Subject** row: **None** or **Folder**, with the portable base-relative `folder_path` as Folder detail. Rendered-document and Markdown-source contexts use that same exact projection.

Projected action eligibility is state for the surrounding Manage contribution. Projects records whether its resolved descriptor declares `authoring_subject`; the detail toolbar owns **Assign subject**, while `metadata-info` remains the passive subject summary.
