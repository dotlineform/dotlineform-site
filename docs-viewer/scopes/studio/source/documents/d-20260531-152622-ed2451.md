...
For `rendered-document`, this includes its breadcrumb, info/bookmark controls, and manage-only document actions such as edit or Markdown source. Mode-specific controls such as source save remain here because the mode is owned by the document view. For `content-detail`, the same surface supplies the public-safe **Back to document** control and presentation label; later Manage-only table tools may contribute beside them without creating another toolbar.
