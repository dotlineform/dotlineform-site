...
def test_python_docs_builder_cli_refreshes_target_lookup_after_written_moments_build(
    monkeypatch,
) -> None:
    moment_doc_id = "d-20260726-000000-a1b2c3"
    with tempfile.TemporaryDirectory() as temp_path:
        root = Path(temp_path)
        monkeypatch.setenv(
            "DOTLINEFORM_PROJECTS_BASE_DIR",
            str(root / "unavailable-projects"),
        )
        write_site_tools_config(root, media_base="")
        ...
        write_docs_scope_config(
            ...
            [
                docs_scope_record(
                    ...
                    default_doc_id=moment_doc_id,
                ),
                docs_scope_record(
                    "private",
                    scope_type="local_external",
                    default_doc_id="d-20260726-000000-ffffff",
                ),
            ],
        )
        write_json(
                        ...
                        "target_types": [
                            {
                                ...
                                "lookup_adapter": "catalogue-moment-target-lookup",
        ...
        write_text(
            root / f"docs-viewer/scopes/moments/source/documents/{moment_doc_id}.md",
            f"""---
doc_id: {moment_doc_id}
title: Lotus Pond
date_display: c. 2024
added_date: 2026-07-26
last_updated: 2026-07-26 20:00:00
parent_id: ""
---
# Lotus Pond
""",
        ...
        dry_run_exit, _, _ = run_cli(root, ["--scope", "moments"])
        before_write = read_json(
            root / "docs-viewer/data/generated/semantic-tokens/target-lookup.json"
        )
        write_exit, stdout, stderr = run_cli(
            root,
            ["--scope", "moments", "--write"],
        )
        after_write = read_json(
            root / "docs-viewer/data/generated/semantic-tokens/target-lookup.json"
        )

    assert dry_run_exit == 0
    assert before_write["targets"] == []
    assert write_exit == 0
    assert stderr == ""
    assert "Semantic target lookup follow-through: wrote (1 targets)" in stdout
    assert after_write["targets"] == [
        {
            "family": "catalogue",
            "target_type": "moment",
            "target_id": moment_doc_id,
            "title": "Lotus Pond",
            "href": f"/moments/?doc={moment_doc_id}",
            "meta": ["c. 2024"],
        }
    ]
