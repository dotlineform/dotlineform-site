#!/usr/bin/env python3
"""Smoke-check Docs Viewer import UI through the Docs Viewer service."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dlf-docs-import-ui-") as tmp_dir:
        ...
        try:
            ...
            imported_path = fixture_root / "docs-viewer/source/studio" / "staged-doc.md"
            ...
            print(f"Docs Viewer service import UI OK: {base_url}/docs/?scope=studio&doc=root-doc")
            return 0
        finally:
            server.shutdown()
            server.server_close()
