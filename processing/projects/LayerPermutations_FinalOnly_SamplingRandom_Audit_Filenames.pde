// LayerPermutations_FinalOnly_SamplingRandom_Audit_Filenames.pde
...
//   • EXPORT SELECTED CSV — saves selected indices, orders, and SOURCE FILENAMES for audit/repro
//
...
void setup(){
  surface.setTitle("Permutation Composites — Final Only (Stride / Seeded Random) + Audit (Filenames)");
...
ArrayList<String> names  = new ArrayList<String>();           // full rendering names
...
ArrayList<String> namesSortedCache = new ArrayList<String>();   // source filenames used in prepareSelection()
...
void drawUI(){
  ...
  text("Permutation Composites — Final Only (Stride / Seeded Random) + Audit (Filenames)", 40, 20);
  textSize(14); fill(60);
  text("Prepare & export selection CSV (with source filenames) for audit/repro. Then EXPORT FINAL when ready.", 40, 44);
...
void prepareSelection(){
  try{
    ...
    selectedIdxCache.clear(); namesSortedCache.clear(); perms.clear();
    ...
    for (File f : files){
      String nm = f.getName().toLowerCase(Locale.ROOT);
      if (endsWithAny(nm, exts)) namesSortedCache.add(f.getName());
    }
    int n = namesSortedCache.size();
...
void exportSelectionCSV(){
  try{
    ...
    // header
    pw.println("perm_index,order,source_files,sampling_mode,stride_n,random_keep_pct,random_seed,first_n_applied,source_dir");
    // rows
    for (int i=0; i<selectedIdxCache.size(); i++){
      int permIndex = selectedIdxCache.get(i);
      int[] order = perms.get(permIndex);
      String orderStr = joinInt(order, "-");
      // map order indices to filenames via namesSortedCache
      StringList srcFiles = new StringList();
      for (int idx : order){
        String nm = (idx>=0 && idx<namesSortedCache.size()) ? namesSortedCache.get(idx) : ("IDX_"+idx);
        srcFiles.append(nm);
      }
      String sourceFilesStr = join(srcFiles.array(), ";");
      ...
      String srcDir = (sourceDirPath==null? "" : sourceDirPath);
      pw.println(csvEscape(str(permIndex))+","+csvEscape(orderStr)+","+csvEscape(sourceFilesStr)+","+
                 csvEscape(mode)+","+csvEscape(strideStr)+","+csvEscape(pctStr)+","+csvEscape(seedStr)+","+csvEscape(firstNStr)+","+csvEscape(srcDir));
    }
    pw.flush(); pw.close();
...
// Blend mode list required by sequentialComposite (forgotten earlier)
final String[] MODES = { "MULTIPLY", "SCREEN", "OVERLAY", "LIGHTEN", "DARKEN", "SOFTLIGHT", "DIFFERENCE" };
