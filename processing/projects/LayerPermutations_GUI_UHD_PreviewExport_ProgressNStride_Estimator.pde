// LayerPermutations_GUI_UHD_PreviewExport_ProgressNStride_Estimator.pde
// Adds an on‑canvas estimator for total composites, time, and memory based on:
//   • Image count (n), Blend strategy, Preview scale, Auto‑Levels, First‑N, and 1‑in‑n stride
//   • Throughput slider (MPix/s) to match your machine
//
// Includes all prior features: preview/export, progress bar, first‑N, stride, UHD greyscale mode,
// CSV layer rules, manifest logging, Auto Levels, and blend modes.
//
// -----------------------------------------------------------------------------
...
void setup(){
  surface.setTitle("Permutations — Stride + First N + Estimator");
  textFont(createFont("Arial", 14));
  setupUI();
  recomputeEstimator(); // initial (no source yet)
}
void draw(){
  background(252);
  drawUI();
...
int PREVIEW_FIRST_N = 100;
int FINAL_FIRST_N   = -1;

// 1-in-n stride (applies to both Preview & Final)
int STRIDE_N = 1; // 1..10
...
int workerConsidered = 0; // permutations considered after stride
...
class Checkbox {
  ...
  boolean isOver(){ return mouseX>=x && mouseX<=x+18 && mouseY<=y+18 && mouseY>=y; }
}
class Cycle {
...
// Integer stepper (for First N and Stride)
class Stepper {
...
Cycle cyStrategy;
Slider slOpacity, slClip, slPreviewScale, slThroughput;
Stepper stPreviewN, stFinalN, stStrideN;
...
void setupUI(){
  ...
  slThroughput   = new Slider("THROUGHPUT (MPix/s)", 50, 500, 150, 40, 650, 260, 20).asInteger();
  ...
  stStrideN  = new Stepper("1‑in‑n (stride)",   780, 280, 200, 32, STRIDE_N, 1, 10);
}

void drawUI(){
  // header
  fill(20); textAlign(LEFT,TOP); textSize(22);
  text("Permutation Composites — Stride + First N + Estimator", 40, 20);
  textSize(14); fill(60);
  text("Pick folders, adjust options, PREVIEW small (limit N / stride), then EXPORT FINAL.", 40, 44);
  ...
  cyStrategy.draw(); slOpacity.draw(); slClip.draw(); slPreviewScale.draw(); slThroughput.draw();
  ...
  // First N + stride steppers
  stPreviewN.enabled = (phase!=Phase.RUNNING);
  stFinalN.enabled   = (phase!=Phase.RUNNING);
  stStrideN.enabled  = (phase!=Phase.RUNNING);
  stPreviewN.draw(); stFinalN.draw(); stStrideN.draw();
  fill(80);
  text("Tips: 0 = all permutations. 1‑in‑n keeps every n‑th permutation.", 340, 320);

  // Status
  fill(30);
  String pfx = (phase==Phase.RUNNING)?"Working…":(phase==Phase.DONE?"Done:":(phase==Phase.ERROR?"Error:":"Status:"));
  text(pfx+" "+statusMsg, 340, 350, width-380, 60);

  // Progress bar
  if (phase==Phase.RUNNING && workerConsidered>0){
    int barX = 340, barY = 420, barW = width-380, barH = 24;
    ...
    text(which + "  " + workerDone + " / " + (workerLimit>0?min(workerLimit, workerConsidered):workerConsidered) + " composites (stride="+STRIDE_N+")", barX, barY+barH+8);
  }

  // Estimator Panel
  drawEstimatorPanel(340, 480, width-380, 300);
}
...
void mousePressed(){
  ...
  if (btnSource.isOver()) selectFolder("Select the SOURCE folder (images)", "sourceSelected");
    ...
    slOpacity.startDrag(); slClip.startDrag(); slPreviewScale.startDrag(); slThroughput.startDrag();
...
void mouseDragged(){
  if (phase==Phase.RUNNING) return;
  slOpacity.drag(); slClip.drag(); slPreviewScale.drag(); slThroughput.drag();
...
void mouseReleased(){
  slOpacity.stopDrag(); slClip.stopDrag(); slPreviewScale.stopDrag(); slThroughput.stopDrag();
...
void recomputeEstimator(){
  ...
  if (sourceDir==null || !sourceDir.isDirectory()){
    EST_DIM_KNOWN = UHD_MODE; // if UHD on, we know dimensions; else unknown
    return;
  }
  ...
  if (EST_N>0){
    if (UHD_MODE){
      ...
      // Probe first image to infer dimensions
...
void drawEstimatorPanel(int x,int y,int w,int h){
  ...
  text("Estimator (not exact — tweak MPix/s to your machine)", x+12, y+10);
  textSize(14); fill(40);

  // n and permutations
  text("Images found (n): "+EST_N, x+12, y+40);
  String factStr = (EST_N<=12) ? factorialString(EST_N) : "very large ("+EST_N+"!)";
  text("Total permutations (n!): "+factStr, x+12, y+60);

  // Dimensions & scale
  String dimStr = (EST_DIM_KNOWN? (EST_W+"×"+EST_H) : "unknown (assuming UHD)");
  text("Base size: "+dimStr, x+12, y+80);
  text("Preview scale: "+nf(PREVIEW_SCALE*100,0,0)+"%", x+12, y+100);
  float scale = workerPreview ? PREVIEW_SCALE : PREVIEW_SCALE; // same control applies to preview; final uses 1.0
  float pixPerImage = (float)EST_W * (float)EST_H * scale * scale;

  // Stride + First N
  int totalPerms = safeFactorialInt(EST_N, 2000000000); // clamp if huge
  int considered = (STRIDE_N<=1 ? totalPerms : (totalPerms + (STRIDE_N-1))/STRIDE_N); // ceil(total/stride)
  int firstNPreview = (PREVIEW_FIRST_N<0? considered : min(PREVIEW_FIRST_N, considered));
  int firstNFinal   = (FINAL_FIRST_N<0? considered : min(FINAL_FIRST_N, considered));
  text("1‑in‑n stride: "+STRIDE_N+"  → considered ≈ "+considered, x+12, y+120);
  text("First N (Preview): "+(PREVIEW_FIRST_N<0?"all":str(PREVIEW_FIRST_N))+"  |  First N (Final): "+(FINAL_FIRST_N<0?"all":str(FINAL_FIRST_N)), x+12, y+140);

  // Time model
  float mpixps = max(1, THROUGHPUT_MPIXPS);
  float passes = max(1, (BLEND_STRATEGY.equals("AVERAGE") ? 1 : 1)); // keep simple: ~1 pass of L channel ops; folded by L below
  float autoMultiplier = AUTO_LEVELS ? 1.2 : 1.0;
  int L = max(1, EST_N);
  double secondsPerCompositePreview = ((double)L * (double)pixPerImage) / (mpixps*1e6) * autoMultiplier * passes;
  double secondsPerCompositeFinal   = ((double)L * ((double)EST_W*(double)EST_H)) / (mpixps*1e6) * autoMultiplier * passes;

  // Totals
  double totalSecondsPreview = secondsPerCompositePreview * firstNPreview;
  double totalSecondsFinal   = secondsPerCompositeFinal   * firstNFinal;

  text("Throughput: "+int(mpixps)+" MPix/s   |   Auto‑Levels: "+(AUTO_LEVELS?"on (+20%)":"off"), x+12, y+160);
  text("Est. seconds/composite (Preview): "+nf((float)secondsPerCompositePreview,1,2), x+12, y+180);
  text("Est. seconds/composite (Final):   "+nf((float)secondsPerCompositeFinal,1,2), x+12, y+200);
  text("Est. total time (Preview set): "+fmtHMS(totalSecondsPreview), x+12, y+220);
  text("Est. total time (Final set):   "+fmtHMS(totalSecondsFinal), x+12, y+240);

  // Memory
  double bytesPerPixel = 4.0;
  double mibPerUHD = (3840.0*2160.0*bytesPerPixel) / (1024.0*1024.0); // ~31.7 MiB
  double mibPerImageAtScale = ((double)EST_W*(double)EST_H*bytesPerPixel)/(1024.0*1024.0)*scale*scale;
  double ramMinPreview = (L+1) * mibPerImageAtScale;
  double ramMinFinal   = (L+1) * ((double)EST_W*(double)EST_H*bytesPerPixel)/(1024.0*1024.0);
  text("RAM est. (Preview): ~"+nf((float)ramMinPreview,1,1)+" MiB   |   RAM est. (Final): ~"+nf((float)ramMinFinal,1,1)+" MiB", x+12, y+270);
}
...
void runGeneration(){
  try{
    ...
    // First N
    int firstN = workerPreview ? PREVIEW_FIRST_N : FINAL_FIRST_N;
    workerLimit = (firstN<0 ? workerConsidered : min(firstN, workerConsidered));
    ...
    statusMsg = which + ": processing "+workerLimit+" of "+workerTotal+" permutations (stride="+STRIDE_N+")…";
    int produced = 0;
    for (int p=0; p<workerLimit; p++){
      ...
      String strideTag = "__stride_"+STRIDE_N;
      String fname = String.format("perm_%s__blend_%s__auto_%s%s%s.png", ordStr, BLEND_STRATEGY, autoStr, scaleTag, strideTag);
