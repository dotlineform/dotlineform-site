// LayerPermutations_GUI_UHD_PreviewExport_ProgressNStride.pde
// Dual‑res preview/export GUI with:
//  • Progress bar
//  • First‑N limit (Preview & Final)
//  • NEW: 1‑in‑n permutation sampling (stride) — render only one out of every n permutations (n=1..10)
//
// Everything else included: folder pickers, toggles/sliders, CSV rules, manifest, Auto Levels.
//
// -----------------------------------------------------------------------------
...
void setup(){
  surface.setTitle("Permutations — Preview/Export + Progress + First N + 1-in-n");
...
// NEW: 1-in-n stride (applies to both Preview & Final)
int STRIDE_N = 1; // 1..10
...
void drawUI(){
  ...
  text("Permutation Composites — Progress, First N, and 1‑in‑n Sampling", 40, 20);
  textSize(14); fill(60);
  text("Pick folders, adjust options, PREVIEW small first (limit N / stride), then EXPORT FINAL.", 40, 44);
  ...
  fill(80);
  text("Tips: 0 = all permutations. 1‑in‑n reduces workload by keeping every n‑th permutation.", 340, 320);
  ...
  // Current settings
  fill(50); text("Current settings:", 340, 480);
  String s="";
  ...
  s += "FIRST N (Preview)="+(PREVIEW_FIRST_N<0?0:PREVIEW_FIRST_N)+"   FIRST N (Final)="+(FINAL_FIRST_N<0?0:FINAL_FIRST_N)+"   1‑in‑n="+STRIDE_N;
  text(s, 340, 500);
}
...
void runGeneration(){
  try{
    ...
    // 1-in-n stride selection: consider only indices i where (i % STRIDE_N == 0)
    ArrayList<Integer> selectedIdx = new ArrayList<Integer>();
    for (int i=0; i<workerTotal; i++){
      if (i % STRIDE_N == 0) selectedIdx.add(i);
    }
