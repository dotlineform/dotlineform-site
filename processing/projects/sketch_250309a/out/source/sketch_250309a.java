/* autogenerated by Processing revision 1297 on 2025-04-29 */
...
public class sketch_250309a extends PApplet {
...
class InkParameters {
  ...
  public float noiseScale = 0.05f;      // Default noise scale
  public float noiseScaleMin = 0;
  public float noiseScaleMax = 1;
  
  public float inkFraction = 0.1f;      // the fraction (0 to 1) of the stroke drawn at full strength.
...
public float computeScalingFactor(PVector start, PVector pt) {
  PVector v = PVector.sub(pt, start);
  float s = 1.0f;  // Default: if point is inside, no scaling needed
...
public CurvePoints offsetCurve(CurvePoints cp, float offsetX, float offsetY) {
...
public PVector randomPointInCircle(float r, PVector center) {
...
public void cacheInkParametersFields() {
...
public void cacheInkParametersFieldSetters() {
...
public void createDesign_Settings() {
...
public void generateSettingButtons(int firstColumnWidth, int topY) {
...
public void loadDefaultInkParameters() {
...
public void createDesign_InkTypes(){
  ...
  for (String inkKey : inkTypes.keySet()) {
    ...
    int groupHeight = PApplet.parseInt(maxGroupHeight);
  ...
  Boundary compositeBoundary = boundaryFromRect(col4X, compositeHeaderY, width - col4X - margin, PApplet.parseInt(maxGroupHeight));
  ...
  currentY += PApplet.parseInt(maxGroupHeight) + groupSpacing;
}

public void updateCurvePointsOffset() {
...
public void updateDecaySelection(String inkKey, String selectedDecay) {
...
public void drawBoundary(Boundary b) {
  stroke(200);
  noFill();
  ...
  // If arcAngle is greater than 0 (but less than 360), draw an arc and radial lines.
    ...
    float halfArc = radians(b.arcAngle) / 2.0f;
...
public void loadInkParameters(String settingName) {
  String fileName = "sliderSettings.json";
  String fullPath = dataPath(fileName);
  if (!fileExists(fileName)) {
    println("No slider settings file found at " + fullPath);
...
public CurvePoints defineCurve() {
...
public CurvePoints defineCurve(Boundary boundary) {
  ...
  float halfArc = radians(boundary.arcAngle) / 2.0f;
  ...
  float r1 = random(0.2f, 0.5f) * boundary.arcRadius;
  float r2 = random(0.2f, 0.7f) * boundary.arcRadius;
  float r3 = random(0.4f, 1.0f) * boundary.arcRadius;
...
public CurvePoints randomCurvePoints(Boundary boundary) {
  ...
  if (boundary.arcRadius == 0) {
    ...
    float cp1Min = 0.2f * radiusMax;
    float cp1Max = 0.3f * radiusMax;
    float cp2Min = 0.3f * radiusMax;
    float cp2Max = 0.5f * radiusMax;
    float endMin = 0.5f * radiusMax;
...
public void drawCurve_Particle(InkParameters inkParams, CurvePoints cp) {
  ...
  for (int i = 0; i < numParticles; i++) {
    float t = i / PApplet.parseFloat(numParticles - 1);
    ...
    pos.add(PVector.random2D().mult(0.5f));  // Slight random offset for natural variation
    positions[i] = pos;
  }
  ...
  for (int i = 0; i < numParticles; i++) {
    ...
    if (t < inkFraction) {
      ...
      if(decayType.equalsIgnoreCase("linear")) {
        ...
        factor = (1 / (1 + exp(slopeNorm * (t - 0.5f)))) * initialMultiplier;
...
public void drawCurve_Particle_Texture(InkParameters inkParams, CurvePoints cp) {
  ...
  for (int i = 0; i < numParticles; i++) {
    ...
    if (t < inkParams.inkFraction) {
      ...
      if (inkParams.decayType.equalsIgnoreCase("linear")) {
        ...
        factor = (1 / (1 + exp(slopeNorm * (xVal - 0.5f)))) * inkParams.initialMultiplier;
...
public void drawCurve_Particle_Diffuse(InkParameters inkParams, CurvePoints cp) {
  ...
  float blurAmount = 1.0f;         // Blur radius for each iteration.
...
public void drawCurve_VectorField(InkParameters inkParams, CurvePoints cp) {
  ...
  for (int i = 0; i < numPoints; i++) {
    float t = i / PApplet.parseFloat(numPoints - 1); // Normalized parameter (0 to 1)
    float x = bezierPoint(cp.start.x, cp.control1.x, cp.control2.x, cp.end.x, t);
    float y = bezierPoint(cp.start.y, cp.control1.y, cp.control2.y, cp.end.y, t);
  ...
  float staticTime = 1.0f;      // Static time value for noise (for a static drawing) - not required for a 2D drawing but kept for possible future use
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    float noiseX = noise(base.x * noiseScale, base.y * noiseScale, staticTime * 0.01f);
    float noiseY = noise(base.y * noiseScale, base.x * noiseScale, staticTime * 0.01f);
  ...
  for (int i = 0; i < numPoints - 1; i++) {
    float t1 = i / PApplet.parseFloat(numPoints - 1);
    float t2 = (i + 1) / PApplet.parseFloat(numPoints - 1);
    float avgT = (t1 + t2) / 2.0f;  // Midpoint parameter
...
public void createDroplets(int numDroplets, DisplayOptions displayOptions, DropletRanges dropletRanges, DrawingOptions drawingOptions) {
  ...
  float marginX = width * drawingOptions.drawingMargin / 100.0f;
  float marginY = height * drawingOptions.drawingMargin / 100.0f;
  ...
  while(dropletsCreated < numDroplets && attempts < maxAttempts) {
    ...
    dropletOptions.size = PApplet.parseInt(random(dropletRanges.sizeMin, dropletRanges.sizeMax));
    dropletOptions.complexity = PApplet.parseInt(random(dropletRanges.complexityMin, dropletRanges.complexityMax + 1));  // +1 so max is inclusive
    dropletOptions.spread = PApplet.parseInt(random(dropletRanges.spreadMin, dropletRanges.spreadMax));
    
    // Compute scale factor and outer ellipse dimensions based on size
    float scaleFactor = dropletOptions.size / 100.0f;
    ...
    float dropletRadius = (outerWidth + outerHeight) / 4.0f;
    ...
    for (DropletOptions placed : placedDroplets) {
      float placedScaleFactor = placed.size / 100.0f;
      float placedOuterWidth = width * placedScaleFactor;
      float placedOuterHeight = height * placedScaleFactor;
      float placedRadius = (placedOuterWidth + placedOuterHeight) / 4.0f;
...
public void createDroplet(DropletOptions dropletOptions, DisplayOptions displayOptions) {
  ...
  float innerWidth = outerWidth * (spread / 100.0f);
  float innerHeight = outerHeight * (spread / 100.0f);
  
  // Determine the number of curve points based on complexity (mapping 1 -> 10 points, 100 -> 100 points)
  int numPoints = PApplet.parseInt(lerp(10, 100, (complexity - 1) / 99.0f));
  
  // Compute semi-dimensions for inner and outer ellipses
  float a_inner = innerWidth / 2.0f;
  float b_inner = innerHeight / 2.0f;
  float a_outer = outerWidth / 2.0f;
  float b_outer = outerHeight / 2.0f;
  ...
  float compFactor = (complexity - 1) / 99.0f;
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    float t = lerp(0.5f, random(1), compFactor);
...
pixelDensity(displayDensity()); }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_250309a" };
    if (passedArgs != null) {
