...
final class BlendViewModel: ObservableObject {
    ...
    func run() async {
        ...
        do {
            ...
            let args = buildArgs(plan: plan, outURL: outURL, settings: settings)
            try await runProcess(launchPath: settings.ffmpegPathOrDefault() ?? "/usr/bin/ffmpeg", arguments: args)
    ...
    func probeAll() async throws {
        ...
        let streams = try await probe.inspect(url: v1, ffprobePath: settings.ffprobePathOrDefault() ?? "/usr/bin/ffprobe")
    ...
    private func buildArgs(plan: FilterPlan, outURL: URL, settings: AppSettings) -> [String] {
        ...
        // Color tag passthrough from plan.colorMeta (derived from video1)
        if let meta = plan.colorMeta {
            if let r = meta.range { args += ["-color_range", r] }
            if let s = meta.space { args += ["-colorspace", s] }
            if let t = meta.trc { args += ["-color_trc", t] }
            if let p = meta.primaries { args += ["-color_primaries", p] }
        }
