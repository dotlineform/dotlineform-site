...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Video Blend GUI (FFmpeg) — v3.3")
                .font(.title2).bold()
            ...
            GroupBox("Slow-motion & Interpolation (optional)") {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        ...
                        Picker("Quality", selection: $vm.state.interpQuality) {
                            ForEach(InterpQuality.allCases, id: \.self) { q in
                                Text(q.label).tag(q)
                            }
                        }.frame(width: 180)
                        Text("minterpolate settings").foregroundStyle(.secondary)
                    }
                }
            }
            ...
            GroupBox("Encoding") {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack {
                        ...
                        Toggle("Preserve original audio format", isOn: $vm.state.preserveAudioFormat)
                        Spacer()
                    }
                }
            }
