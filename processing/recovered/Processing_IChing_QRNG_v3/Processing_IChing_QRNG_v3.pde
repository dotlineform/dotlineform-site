// QRNG Divination — v3
// Adds: mo lineage table (CSV) loading and on-screen guidance in mo mode.
// Keys: SPACE=new, R=auto, S=screenshot, M=mode, L=logging, Q=question
...
HashMap<Integer, String[]> moTable = new HashMap<Integer, String[]>();
...
void setup(){
  size(860, 800);
  surface.setTitle("QRNG Divination — I Ching & mo (v3)");
  textAlign(LEFT, TOP);
...
void draw(){
  ...
  drawHeader();
  if(mode==Mode.ICHING){
    drawHexagram(120, 200, yang, change, "Primary", hexIndex);
    if(hexIndexResult >= 0){
      drawHexagram(520, 200, yangResult, new boolean[6], "Resulting", hexIndexResult);
    }
    drawFooterIching();
  } else {
    drawMoPanel();
...
void drawMoPanel(){
  ...
  // guidance box
  float gx = x + 240, gy = y+10, gw = 420, gh = 150;
  stroke(220); fill(255); rect(gx, gy, gw, gh, 12);
  fill(20); textSize(15);
  text("Lineage guidance", gx+12, gy+10);
  String[] row = moTable.get(dieSum);
  textSize(13); fill(40);
  if(row!=null){
    text("Outcome: " + row[0], gx+12, gy+34);
    text("Advice: " + row[1], gx+12, gy+58, gw-24, 60);
    text("Remedy: " + row[2], gx+12, gy+118, gw-24, 60);
  } else {
    text("No entry for this sum. Edit data/mo_table.csv to add it.", gx+12, gy+34);
  }

  textSize(12); fill(60);
  text("Tip: edit data/mo_table.csv (sum 2–12) to match your lineage table.", x, y+140);
}
...
void loadMoTable(){
  moTable.clear();
  try{
    Table t = loadTable("mo_table.csv", "header");
    for(TableRow r : t.rows()){
      try{
        ...
        moTable.put(s, new String[]{ outcome, advice, remedy });
      }catch(Exception ex){}
    }
    println("Loaded mo_table.csv with " + moTable.size() + " entries.");
  }catch(Exception e){
    println("mo_table.csv not found or invalid; using empty table.");
  }
}
