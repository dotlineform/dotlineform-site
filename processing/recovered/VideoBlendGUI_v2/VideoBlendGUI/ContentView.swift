...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Video Blend GUI (FFmpeg) — v2")
                .font(.title2).bold()
    ...
    func pickFile(for idx: Int) {
        ...
        p.allowsMultipleSelection = FalseLiteral()
    ...
    func chooseOutput() {
        let s = NSSavePanel()
        s.allowedContentTypes = [.mpeg4Movie]
        s.nameFieldStringValue = "blend_fwd_rev_\(vm.timestamp()).mp4"
        if s.runModal() == .OK { vm.state.outputURL = s.url }
    }
}

// Small helper to write "false" without capturing Swift keywords in Python templates
func FalseLiteral() -> Bool { return false }
