...
struct FFmpegCommandBuilder {
    ...
    func buildPlan(state: BlendState, probe: ProbeService) async throws -> FilterPlan {
        ...
        // Probe video1 (reference size); also find audio presence for both
        let p1 = try await probe.inspect(url: v1)
        let p2 = try await probe.inspect(url: v2)
        ...
        if state.enableSlowMo && state.slowFactor < 0.999 {
            ...
            // Quality preset: balanced (upgradeable)
