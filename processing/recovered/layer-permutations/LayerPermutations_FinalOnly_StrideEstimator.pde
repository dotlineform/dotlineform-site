// LayerPermutations_FinalOnly_StrideEstimator.pde
// Final-only build: Preview removed (no preview button, no preview scale).
// Features kept: folder pickers, UHD/strict/greyscale toggles, blend strategies,
// Auto Levels/Contrast (percentile), CSV layer rules, manifest logging,
// 1-in-n stride, First-N cap (Final only), progress bar, final-only estimator.
//
// -----------------------------------------------------------------------------
...
void setup(){
  surface.setTitle("Permutation Composites — Final Only");
...
Cycle cyStrategy;
Slider slOpacity, slClip, slThroughput;
...
void setupUI(){
  ...
  stFinalN   = new Stepper("FIRST N (Final)",   340, 280, 200, 32, FINAL_FIRST_N<0?0:FINAL_FIRST_N,   0, 1000000);
  stStrideN  = new Stepper("1‑in‑n (stride)",   560, 280, 200, 32, STRIDE_N, 1, 10);
}

void drawUI(){
  // header
  fill(20); textAlign(LEFT,TOP); textSize(22);
  text("Permutation Composites — Final Only", 40, 20);
  textSize(14); fill(60);
  text("Pick folders, adjust options, then EXPORT FINAL (with stride / First‑N).", 40, 44);
  ...
  if (phase==Phase.RUNNING && workerConsidered>0){
    ...
    fill(30);
    text("FINAL  " + workerDone + " / " + (workerLimit>0?min(workerLimit, workerConsidered):workerConsidered) + " composites (stride="+STRIDE_N+")", barX, barY+barH+8);
  }

  // Estimator Panel
  drawEstimatorPanel(340, 480, width-380, 200);
}
...
void mousePressed(){
  ...
  if (btnSource.isOver()) selectFolder("Select the SOURCE folder (images)", "sourceSelected");
    ...
    slOpacity.startDrag(); slClip.startDrag(); slThroughput.startDrag();
...
void mouseDragged(){
  if (phase==Phase.RUNNING) return;
  slOpacity.drag(); slClip.drag(); slThroughput.drag();
  LAYER_OPACITY = slOpacity.v;
  AUTO_CLIP_PERCENT = slClip.v; // fraction
  THROUGHPUT_MPIXPS = slThroughput.v;
}
void mouseReleased(){
  slOpacity.stopDrag(); slClip.stopDrag(); slThroughput.stopDrag();
...
void drawEstimatorPanel(int x,int y,int w,int h){
  ...
  int considered = (STRIDE_N<=1 ? totalPerms : (totalPerms + (STRIDE_N-1))/STRIDE_N); // ceil
  int firstN = (FINAL_FIRST_N<0? considered : min(FINAL_FIRST_N, considered));
  text("1‑in‑n stride: "+STRIDE_N+"  → considered ≈ "+considered, x+12, y+100);
  text("First N (Final): "+(FINAL_FIRST_N<0?"all":str(FINAL_FIRST_N))+"  → to render: "+firstN, x+12, y+120);
...
void runGeneration(){
  try{
    ...
    statusMsg = "FINAL: processing "+workerLimit+" of "+workerTotal+" permutations (stride="+STRIDE_N+")…";
    int produced = 0;
    for (int p=0; p<workerLimit; p++){
      ...
      String fname = String.format("perm_%s__blend_%s__auto_%s%s.png", ordStr, BLEND_STRATEGY, autoStr, strideTag);
