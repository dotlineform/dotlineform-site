// LayerPermutations_AutoLevels_CSV_Manifest.pde
// Static composites for ALL permutations, with Auto Levels/Auto Contrast,
// CSV-driven per-layer rules, and a per-output MANIFEST CSV.
//
// MANIFEST (output/manifest.csv) columns:
// filename,order,strategy,modes,auto_levels,source_files
//
// - filename: saved PNG filename
// - order: indices order (e.g., 0-2-1) corresponding to load order from data/input
// - strategy: SEQUENTIAL or AVERAGE
// - modes: per-layer modes used (SEQUENTIAL) or "N/A" (AVERAGE)
// - auto_levels: AutoLevels | AutoContrast | None
// - source_files: comma-separated original filenames in the order applied
//
// Place source images in "data/input". Optional rules in "data/layer_rules.csv".
// Outputs + manifest are written to "output/".

import java.io.*; // for manifest append
...
// manifest writer
File manifestFile;
boolean manifestInitialized = false;

void settings() { size(800, 800); }

void setup() {
  println("=== Layer Permutations + Auto Levels + CSV Rules + Manifest ===");
  ...
  // preserve deterministic order by sorting by name
  Arrays.sort(files);
  for (File f : files) {
  ...
  for (int p = 0; p < perms.size(); p++) {
    ...
    if (BLEND_STRATEGY.equals("AVERAGE")) {
      ...
      // sequential returns modes used for manifest along with image
    ...
    // Build source files list in applied order
    StringList srcFiles = new StringList();
    for (int idx : order) srcFiles.append(names.get(idx));
    String srcJoined = join(srcFiles.array(), ",");

    // Append to manifest
    appendManifestRow(fname, ordStr, blendStr, modesStrForManifest, AUTO_LEVELS ? (AUTO_LEVELS_PER_CHANNEL ? "AutoLevels" : "AutoContrast") : "None", srcJoined);
  }

  println("✔ Done. Wrote " + produced + " composite(s) to: " + sketchPath(OUTPUT_DIR));
  println("✔ Manifest: " + manifestFile.getAbsolutePath());
  exit();
}
...
// ------------------------------------
void initManifest() {
  manifestFile = new File(sketchPath(OUTPUT_DIR + "/manifest.csv"));
  if (!manifestFile.exists()) {
    ...
    try {
      ...
      println("Failed to initialize manifest: " + e.getMessage());
    }
  } else {
    manifestInitialized = true; // assume header already present
  }
}
...
void appendManifestRow(String filename, String order, String strategy, String modes, String autoLevels, String sourceFiles) {
  ...
  try {
    PrintWriter pw = new PrintWriter(new FileWriter(manifestFile, true));
    // simple CSV escaping: wrap fields that may contain commas in quotes
    filename = csvEscape(filename);
    order = csvEscape(order);
    strategy = csvEscape(strategy);
    modes = csvEscape(modes);
    autoLevels = csvEscape(autoLevels);
    sourceFiles = csvEscape(sourceFiles);
    pw.println(filename + "," + order + "," + strategy + "," + modes + "," + autoLevels + "," + sourceFiles);
    pw.flush();
    pw.close();
  } catch (IOException e) {
...
PImage sequentialComposite(int[] order, StringList usedModesOut) {
  ...
  usedModesOut.append("BASE"); // placeholder for layer 1 (no blend)
