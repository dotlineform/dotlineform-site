...
final class BlendViewModel: ObservableObject {
    ...
    func run() async {
        ...
        do {
            ...
            try await runProcess(launchPath: settings.ffmpegPathOrDefault(), arguments: args)
    ...
    func probeAll() async throws {
        ...
        let streams = try await probe.inspect(url: v1, ffprobePath: settings.ffprobePathOrDefault())
    ...
    private func buildArgs(plan: FilterPlan, outURL: URL) -> [String] {
        ...
        // Color tag passthrough using mapped tokens
