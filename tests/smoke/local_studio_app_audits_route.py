...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            audit_service_requests: list[str] = []
            page.on("console", lambda message: console_errors.append(message.text) if message.type == "error" else None)
            page.on("pageerror", lambda error: page_errors.append(str(error)))
            page.on(
                "request",
                lambda request: audit_service_requests.append(request.url)
            ...
            page.route(
                ...
                lambda route: route.fulfill(
                    ...
                    body='{"ok": false, "error": "audit service unavailable"}',
                ),
            )
            ...
            expect(root).to_have_attribute("data-studio-mode", "unavailable", timeout=10_000)
            ...
            expect(page.locator("[data-run-audit]").first).to_be_disabled(timeout=10_000)
            ...
            if not audit_service_requests:
                raise AssertionError("audits route did not probe the audit service")
