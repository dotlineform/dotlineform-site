...
def create_fixture_repo(target_root: Path) -> None:
    ...
    for scope, default_doc in {
        ...
        source_root = {
            ...
            "analysis": "_docs_analysis",
        }[scope]
        write_doc(target_root / source_root / f"{default_doc}.md", doc_id=default_doc, title=default_doc.replace("-", " ").title())
    write_doc(target_root / "_docs" / "archive.md", doc_id="archive", title="Archive", sort_order=9000)
    write_doc(target_root / "_docs" / "sibling-doc.md", doc_id="sibling-doc", title="Sibling Doc", sort_order=2000)
    write_doc(target_root / "_docs" / "child-doc.md", doc_id="child-doc", title="Child Doc", parent_id="root-doc", sort_order=1000)
