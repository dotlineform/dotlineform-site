#!/usr/bin/env python3
"""Smoke-check the local Studio Analytics dashboard route shell."""
...
ROUTE_ID = "studio_analytics"
ROUTE_PATH = "/studio/analytics/?mode=manage"
ROOT_SELECTOR = "#studioAnalyticsDashboardRoot"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            expect(root).to_have_attribute("data-studio-route", "studio-analytics", timeout=10_000)
            ...
            expect(page.locator('[data-studio-metric="tag-count"]')).not_to_have_text("--", timeout=10_000)
            ...
            if nav_link != ROUTE_PATH:
                raise AssertionError(f"Analytics dashboard nav link is not manage-mode: {nav_link!r}")
            ...
            if not EXPECTED_LINKS.issubset(hrefs):
                raise AssertionError(f"Analytics dashboard links missing expected targets: {sorted(hrefs)!r}")
        ...
        print(f"local Studio Analytics dashboard route OK: {base_url}{ROUTE_PATH}")
        return 0
    finally:
        server.shutdown()
        server.server_close()
