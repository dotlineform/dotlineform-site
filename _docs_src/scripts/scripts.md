...
- `./scripts/build_catalogue.py`
  - plans workbook-backed generation and canonical source-media changes from `var/build_catalogue_state.json`, prunes stale repo-owned generated artifacts for removed rows, then orchestrates copy -> srcset -> generation for works, work details, and moments, then rebuilds catalogue search
