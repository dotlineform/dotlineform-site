...
| MSC-005 | Front-matter metadata extraction from moment prose | `parse_front_matter` in `scripts/moment_sources.py`; retired source-entry/update helpers | no | partial | Implemented for active/scanner paths: source-entry, front-matter update, scalar update, and workbook-ish row helpers were removed. `parse_front_matter` remains only because later slices still own standalone delete and deprecated preflight cleanup. | Remove `parse_front_matter` when delete/preflight no longer import it. Do not keep it as migration-only by default. | Source validation, Moment editor import, scoped moment builds, and publish/unpublish flows pass without reading metadata from prose front matter. |
