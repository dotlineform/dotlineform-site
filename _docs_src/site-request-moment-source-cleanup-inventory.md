...
| MSC-008 | Workbook preflight moment front-matter scan | `scripts/catalogue_preflight.py` | no | removed | Implemented: the deprecated workbook preflight no longer scans external moment Markdown or parses moment front matter for status validation. | Keep workbook preflight limited to workbook-shaped checks while it remains in the repo; do not reattach external moment source validation to it. | Deprecated workbook preflight syntax passes, and active JSON-source checks do not depend on external moment source scans. |
