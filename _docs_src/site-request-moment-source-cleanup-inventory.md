...
| MSC-013 | Stale implementation/testing docs | `_docs_src/studio-implementation-plan.md`; `_docs_src/studio-e2e-checklist.md` | no or mixed | `docs-stale` | These docs still include front-matter-owned metadata or source preview assumptions that no longer match the current moment workflow. The standalone delete script doc has been removed. | Rewrite or archive stale current-workflow guidance during documentation cleanup. | Docs search for moment front-matter workflow terms should not return stale pages as current guidance. |
...
1. Done: remove front-matter detection and rejection from active moment preview/import paths. Retired source-entry and front-matter update helpers were also removed; `parse_front_matter` remains only for later delete/preflight cleanup slices.
2. Done: remove generator fallback scanning of external moment source files and moment source manifest support.
3. Done: remove `scripts/delete_moment.py` and `_docs_src/scripts-delete-moment.md` so Studio delete remains the single moment deletion workflow.
