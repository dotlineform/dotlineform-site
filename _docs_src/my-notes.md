...
- In `assets/studio/js/tag-studio.js`, load repo assignments first, then overlay any staged row for the current series before `buildState(...)`.
...
- Update chip rendering in `assets/studio/js/tag-studio.js` and styling in `assets/studio/css/studio.css`.
...
- Extend `studio/series-tags/index.md` with a template-owned section above the list.
- In `assets/studio/js/series-tags.js`, render session summary, `Copy JSON`, `Download JSON`, `Clear session`, and later `Import assignments` when server is available.
...
- In `scripts/studio/tag_write_server.py`, add:
...
- Add new `ui_text` entries in `assets/studio/data/studio_config.json` for:
