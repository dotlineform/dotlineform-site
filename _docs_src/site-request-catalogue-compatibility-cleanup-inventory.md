...
| 3. Remove file/link compatibility maps | CCI-006, CCI-010 | Planned. | May need separate source, lookup, delete, activity, and validation checks because affected-source display can lag behind source-model cleanup. |
| 4. Stale docs cleanup | CCI-003, CCI-009 | Planned. | Some narrow deprecated-command wording may remain if a retained command must explain why it exits. |
