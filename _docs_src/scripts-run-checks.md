...
- `quick`
  Runs diff whitespace checks, lightweight Python syntax checks, catalogue cleanup, invalidation, lookup-refresh, prose-import, route, and transaction tests, Studio activity tests, Studio backup-retention tests, the Studio ready-state audit, and Studio config/activity JSON parsing. The syntax check includes core catalogue/source helpers, the Studio audit service script, and core lightweight test modules.
- `catalogue`
