...
That is useful as a safe interim behavior, but it is not enough for the intended external-organization workflow: an external service may create a better hierarchy by introducing new parent nodes that do not exist in `_docs_library_src/` yet.
...
- new docs are source-controlled Library docs under `_docs_library_src/`
...
Create missing parent docs under `_docs_library_src/` before applying selected child parent changes, with backups and source-write allowlists.
