...
{% assign works_items = site.works_curator %}
{% if works_items and works_items != empty %}
  {% assign works_sorted = works_items | sort_natural: "series_id" | sort_natural: "title" %}
  ...
  <div class="index worksList worksList--curator">
    ...
    <ul class="worksList__list" id="worksList">
      {% for w in works_sorted %}
        ...
        {% assign work_doc = site.works | where: "work_id", w.work_id | first %}
        {% assign series_label = w.series_title | default: series_doc.title | default: w.series_id %}
        ...
        {% if series_doc and series_doc.sort_fields %}
          ...
          {% elsif sort_token == "storage" or sort_token == "storage_location" %}
            {% assign series_primary_sort = "storage" %}
          {% endif %}
        {% endif %}
        <li
          class="worksList__item"
          data-cat="{{ w.work_id | default: '' | downcase | strip | escape }}"
          data-series-sort="{{ work_doc.series_sort | default: w.work_id | downcase | strip | escape }}"
          ...
          data-storage="{{ w.storage_location | default: '' | downcase | strip | escape }}"
          data-series-id="{{ w.series_id | default: '' | downcase | strip | escape }}"
          data-series-label="{{ series_label | strip | escape }}"
        >
          <a class="worksList__cat" href="{{ w.url | relative_url }}?from=works_curator_index">{{ w.work_id }}</a>
          <span class="worksList__year">{{ w.year_display | default: w.year }}</span>
          <a class="worksList__title" href="{{ w.url | relative_url }}?from=works_curator_index">{{ w.title }}</a>
          <a class="worksList__series" href="{{ series_href | relative_url }}">{{ series_label }}</a>
          <span class="worksList__storage">{{ w.storage_location | default: "—" }}</span>
        </li>
      {% endfor %}
    </ul>
