...
def test_runtime_config_exposes_admin_home_and_planned_routes() -> None:
    ...
    assert runtime["data_paths"]["testing"]["runs"] == "var/test-runs"
...
def test_admin_testing_api_reads_repository_run_summaries(tmp_path) -> None:
    run_dir = tmp_path / "var" / "test-runs" / "sample-run"
    run_dir.mkdir(parents=True)
    (run_dir / "summary.json").write_text(
        '{"profiles":["quick"],"status":"passed","run_dir":"var/test-runs/sample-run","results":[]}\n',
        encoding="utf-8",
    )
    ...
    assert payload["runs_root"] == "var/test-runs"
    assert payload["runs"][0]["run_id"] == "sample-run"
    assert payload["runs"][0]["profiles"] == ["quick"]
    assert payload["runs"][0]["summary_path"] == "var/test-runs/sample-run/summary.md"
