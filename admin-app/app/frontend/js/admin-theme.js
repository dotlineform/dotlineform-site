...
function renderToggle(button, theme) {
  ...
  button.querySelectorAll("[data-admin-theme-icon]").forEach((icon) => {
    if (icon.dataset.adminThemeIcon === activeTheme) {
...
export function initAdminThemeToggle(options = {}) {
  ...
  const buttons = Array.from(root.querySelectorAll("[data-admin-theme-toggle]"));
...
initAdminThemeToggle({ root: document });
