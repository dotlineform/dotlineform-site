...
export async function loadAdminConfig() {
  if (!adminConfigPromise) {
    adminConfigPromise = fetch(resolveAdminConfigUrl(), { cache: "default" })
      .then((response) => {
...
function resolveAdminConfigUrl() {
  const configuredUrl = readConfiguredAdminConfigUrl();
  if (configuredUrl) {
    return buildAssetUrl(resolveSitePath(configuredUrl));
  }
  throw new Error('Admin runtime config meta tag is required: meta[name="dlf-admin-config-url"]');
}

function readConfiguredAdminConfigUrl() {
  if (typeof document === "undefined") return "";
  const meta = document.querySelector('meta[name="dlf-admin-config-url"]');
...
export function getAdminRouteRegistry(config) {
  const routes = pathValue(config, ["app", "routes"]);
  return routes && typeof routes === "object" && !Array.isArray(routes) ? routes : {};
}

export function getAdminRouteEntry(config, key) {
  const normalizedKey = normalizeRouteKey(key);
  const routes = getAdminRouteRegistry(config);
  const entry = routes[normalizedKey];
  return entry && typeof entry === "object" && !Array.isArray(entry) ? entry : null;
}

export function getAdminRoute(config, key) {
  const routeEntry = getAdminRouteEntry(config, key);
...
export function buildAdminRouteUrl(config, key, params = {}) {
  const route = getAdminRoute(config, key);
  if (!route) return "";
...
function deriveSiteBasePath(importUrl) {
  ...
  const marker = "/admin/app/frontend/js/";
